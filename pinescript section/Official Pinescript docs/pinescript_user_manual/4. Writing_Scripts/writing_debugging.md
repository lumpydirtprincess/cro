![](../4. Writing_Scripts/writing_debugging.md)

# [Debugging](../4. Writing_Scripts/writing_debugging.md#debugging)

## [Introduction](../4. Writing_Scripts/writing_debugging.md#introduction)
...
## [Common debug outputs](../4. Writing_Scripts/writing_debugging.md#common-debug-outputs)
...
Pine [drawings](../4. Writing_Scripts/writing_debugging.md#pine-drawings) display visuals in the main chart pane or the script’s separate pane. Although they do not output results in other locations, such as the Data Window or Pine Logs pane, drawings provide convenient ways to visualize a script’s data and logic within global or local scopes. [Labels](../2. Visuals/visuals_text-and-shapes.md#labels) are the most flexible drawings for debugging, because they can display colored shapes with formatted text and tooltips at any available chart location, for example:

![image](../images/Debugging-Common-debug-outputs-2.CKqA9BHP_ZbEfay.webp)
...
![image](../images/Debugging-Common-debug-outputs-3.CzcytxBb_GcDAe.webp)
...
The [bgcolor()](../../reference manual/functions/bgcolor.md) function displays colors in the background of the main chart pane or the script’s pane. The [barcolor()](../../reference manual/functions/barcolor.md) function colors the main chart’s bars or candles. Although these outputs are less flexible than Pine Logs, drawings, and plots, they provide a quick way to inspect calculated colors and visualize conditions from the global scope:

![image](../images/Debugging-Common-debug-outputs-4.B7ERq33__ZV9NIy.webp)
...
## [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs)
...
Pine Logs do not appear on the chart or in the Data Window. Instead, scripts print logged messages with prefixed date and time information in the dedicated _Pine Logs pane_. The [inspection](../4. Writing_Scripts/writing_debugging.md#inspecting-logs) and [filtering](../4. Writing_Scripts/writing_debugging.md#filtering-logs) options in the Pine Logs pane help users analyze and navigate logs efficiently.
...
![image](../images/Debugging-Pine-logs-1.BKWDgKSp_Z1B7xuP.webp)

### [Creating logs](../4. Writing_Scripts/writing_debugging.md#creating-logs)

Scripts create Pine Logs by calling the functions in the `log.*` namespace: [log.info()](../../reference manual/functions/log.info.md), [log.warning()](../../reference manual/functions/log.warning.md), or [log.error()](../../reference manual/functions/log.error.md). All these logging functions have the following two signatures:
...
- The second overload creates a [formatted string](../1. Concepts/concepts_strings.md#formatting-strings) based on its `formatString` and additional arguments, similar to [str.format()](../../reference manual/functions/str.format.md), then displays the resulting text inside the pane.
...
- The [log.info()](../../reference manual/functions/log.info.md) function creates a message with the “info” level (gray text).
- The [log.warning()](../../reference manual/functions/log.warning.md) function creates a message with the “warning” level (orange text).
- The [log.error()](../../reference manual/functions/log.error.md) function creates a message with the “error” level (red text).

This simple script demonstrates the difference between all three `log.*()` functions. It calls [log.info()](../../reference manual/functions/log.info.md), [log.warning()](../../reference manual/functions/log.warning.md), and [log.error()](../../reference manual/functions/log.error.md) on the first chart bar to print the values of three [literal strings](../1. Concepts/concepts_strings.md#literal-strings) in the Pine Logs pane:

![image](../images/Debugging-Pine-logs-Creating-logs-1.FAyoXrvo_4ir5m.webp)
...
- The Pine Logs pane can _filter_ messages by their logging level using the menu accessible from the rightmost icon above the logs. See the [Filtering logs](../4. Writing_Scripts/writing_debugging.md#filtering-logs) section to learn more.
...
During historical executions, scripts log a new message _once_ for each `log.*()` call on any bar. During realtime executions, scripts can call the `log.*()` functions to log messages for _any available tick_, regardless of whether the bar is confirmed. The logs created on realtime ticks are _not_ subject to [rollback](../3. Language/language_execution-model.md#executions-on-realtime-bars). All logs remain available in the Pine Logs pane until the script restarts.

The example script below calculates the average ratio of each bar’s `close - open` value to its `high - low` range. When the range is nonzero, the script prints the values of the calculation’s variables in the Pine Logs pane using [log.info()](../../reference manual/functions/log.info.md) if the bar is confirmed or [log.warning()](../../reference manual/functions/log.warning.md) if the bar is still open ( _unconfirmed_). If the bar’s range is zero, making the calculated ratio _undefined_, the script logs an “error” message using [log.error()](../../reference manual/functions/log.error.md):

![image](../images/Debugging-Pine-logs-Creating-logs-2.BvPUb-vU_q9wDE.webp)
...
- Programmers can use [barstate.isconfirmed](../../reference manual/variables/barstate.isconfirmed.md) in the conditions that trigger `log.*()` calls to allow logs for any realtime bar only once, on its _closing tick_, as shown in the example code.
...
- The Pine Logs pane can display the most recent 10,000 logs for historical bars. If a programmer needs to view earlier logs, they can add logic in the code to filter specific `log.*()` calls. See the [Custom code filters](../4. Writing_Scripts/writing_debugging.md#custom-code-filters) section for an example.

The following sections use the example script above to demonstrate the Pine Logs pane’s log [inspection](../4. Writing_Scripts/writing_debugging.md#inspecting-logs) and [filtering](../4. Writing_Scripts/writing_debugging.md#filtering-logs) features.

## [Inspecting logs](../4. Writing_Scripts/writing_debugging.md#inspecting-logs)

When a script generates a log by calling any `log.*()` function call, the Pine Logs pane automatically prefixes the logged message with an [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601#Combined_date_and_time_representations) _timestamp_ representing the log’s assigned time, expressed in the [chart’s time zone](../1. Concepts/concepts_time.md#time-zones). The timestamp prefixed to a log on a historical bar represents the bar’s _opening time_, whereas the timestamp for a realtime log represents the _system time_ of the log event:

![image](../images/Debugging-Pine-logs-Inspecting-logs-1.8xpvIK8H_1Yjrq2.webp)
...
![image](../images/Debugging-Pine-logs-Inspecting-logs-2.jaYYYDxx_ZmMSDl.webp)
...
![image](../images/Debugging-Pine-logs-Inspecting-logs-3.Dxr9Y90y_10pluC.webp)
...
![image](../images/Debugging-Pine-logs-Inspecting-logs-4.CnvC925K_Z1uNlcm.webp)
...
![image](../images/Debugging-Pine-logs-Inspecting-logs-5.ClRTwv_F_ZO3LGY.webp)

## [Filtering logs](../4. Writing_Scripts/writing_debugging.md#filtering-logs)
...
To help users navigate high volumes of logs efficiently, the pane includes _filters_ that isolate logs based on [logging level](../4. Writing_Scripts/writing_debugging.md#logging-level), [start date and time](../4. Writing_Scripts/writing_debugging.md#start-date), or [search queries](../4. Writing_Scripts/writing_debugging.md#character-and-pattern-search). Users can apply these log filters individually or in combination to show only the messages that meet specific criteria. The filters are accessible from the icons below the “x” in the top-right portion of the pane:

![image](../images/Debugging-Pine-logs-Filtering-logs-1.Qt-I6kcm_2tz9QU.webp)

For custom filtering options, programmers can use conditional logic to activate specific `log.*()` calls selectively across a script’s executions. See the [Custom code filters](../4. Writing_Scripts/writing_debugging.md#custom-code-filters) section below to learn more.

### [Logging level](../4. Writing_Scripts/writing_debugging.md#logging-level)
...
![image](../images/Debugging-Pine-logs-Filtering-logs-Logging-level-1.CqbnhwLx_2oymTa.webp)

Note that:

- Deactivating logging levels in this menu hides the relevant messages but _does not_ stop the execution of those `log.*()` calls in the code. For instance, a [log.info()](../../reference manual/functions/log.info.md) call still executes and adds to the historical log count even when the “Info” option is unchecked.

### [Start date](../4. Writing_Scripts/writing_debugging.md#start-date)
...
![image](../images/Debugging-Pine-logs-Filtering-logs-Start-date-1.IZNVl8zX_1Uv1pc.webp)
...
![image](../images/Debugging-Pine-logs-Filtering-logs-Start-date-2.BDbP45dS_Lsj8M.webp)

### [Character and pattern search](../4. Writing_Scripts/writing_debugging.md#character-and-pattern-search)
...
![image](../images/Debugging-Pine-logs-Filtering-logs-Character-and-pattern-search-1.D-1crEOA_Z1Kv6AJ.webp)
...
The rightmost icon in the search bar opens a dropdown menu containing three options to adjust the search filter’s behavior: [Match case](../4. Writing_Scripts/writing_debugging.md#match-case), [Whole word](../4. Writing_Scripts/writing_debugging.md#whole-word), and [Regex](../4. Writing_Scripts/writing_debugging.md#regex):

![image](../images/Debugging-Pine-logs-Filtering-logs-Character-and-pattern-search-2.CGclA-al_ZjsR2g.webp)

#### [Match case](../4. Writing_Scripts/writing_debugging.md#match-case)
...
![image](../images/Debugging-Pine-logs-Filtering-logs-Character-and-pattern-search-Match-case-1.BprO1bQS_Z1b1395.webp)
...
#### [Whole word](../4. Writing_Scripts/writing_debugging.md#whole-word)
...
![image](../images/Debugging-Pine-logs-Filtering-logs-Character-and-pattern-search-Whole-word-1.-3nPjDxx_S2aPh.webp)
...
#### [Regex](../4. Writing_Scripts/writing_debugging.md#regex)
...
![image](../images/Debugging-Pine-logs-Filtering-logs-Character-and-pattern-search-Regex-1.CnCNzCEk_Z2mOm3N.webp)

The more advanced search query below specifies that the logs must contain prefixed timestamps representing any time of day equal to or after 09:30
and before 16:00
...
```\
\
![image](../images/Debugging-Pine-logs-Filtering-logs-Character-and-pattern-search-Regex-2.BubI1o13_Z1FguP1.webp)\
\
For more information about regular expressions, consult the [Regex syntax reference](../1. Concepts/concepts_strings.md#regex-syntax-reference) in this manual’s [Strings](../1. Concepts/concepts_strings.md) page. Most of the described syntax works the same within the Pine Logs search filter, with a few notable differences:\
\
- The strings used as `regex` arguments in [str.match()](../../reference manual/functions/str.match.md) calls require _two_ consecutive backslashes (` `) for specifying escape sequences in the pattern (e.g., `" w"` means the regex matches a character from the `\w` class). In contrast, the Pine Logs search filter requires only a _single_ backslash for escape sequences. Double backslashes in the search bar match the literal `\` character.\
...
\
### [Custom code filters](../4. Writing_Scripts/writing_debugging.md#custom-code-filters)\
\
If the filtering options in the Pine Logs pane are not sufficient, programmers can control specific `log.*()` calls using [inputs](../1. Concepts/concepts_inputs.md) and conditional logic.\
\
The script below calculates an RMA of [close](../../reference manual/variables/close.md) prices and creates a compound condition from four distinct individual conditions. It plots the RMA on the chart and highlights the background when the `compoundCondition` value is `true`. For debugging, the script uses [log.info()](../../reference manual/functions/log.info.md) to display a formatted string representing the `close` and `rma` values, the values of all the “bool” variables that form the compound condition, and the final `compoundCondition` value.\
\
The `filterLogsInput`, `logStartInput`, and `logEndInput` variables define a custom _time filter_ for generating logs. When `filterLogsInput` is `true`, the script uses the [time inputs](../1. Concepts/concepts_inputs.md#time-input) assigned to `logStartInput` and `logEndInput` to filter the [log.info()](../../reference manual/functions/log.info.md) calls, allowing a new log only when the bar’s [time](../../reference manual/variables/time.md) is within the specified range:\
\
![image](../images/Debugging-Pine-logs-Filtering-logs-Custom-code-filters-1.BDemxPLh_Zj1SX.webp)\
\
...
- The `formatString` argument of the [log.info()](../../reference manual/functions/log.info.md) call uses the Em Space character (U+2003) to align the represented values vertically in the logged text. In contrast to the standard space and tab characters, leading or repeated Em and En spaces are _not_ removed from the Pine Logs pane’s displayed messages.\
\
## [Pine drawings](../4. Writing_Scripts/writing_debugging.md#pine-drawings)\
\
Pine’s [drawing types](../3. Language/language_type-system.md#drawing-types) create chart drawings with specified properties. Scripts can place drawings at any valid chart location during code executions on any bar. Programmers can use these types in a script’s global or local scopes to visualize numeric data, conditions, colors, and strings on the chart. The flexibility of Pine drawings makes them helpful for debugging scripts when other methods do not suffice, namely when a programmer wants to inspect information graphically outside the [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) pane.\
\
However, before debugging a script using drawings, it is crucial to note the following limitations:\
\
- The `expression` argument of a `request.*()` call _cannot_ depend on code that creates or modifies drawings. Likewise, an indicator that specifies another context in its declaration statement cannot create drawings from _anywhere_ in the code. To debug code that executes on requested data, use [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) instead.\
...
\
The sections below explain some simple debugging methods using [labels](../2. Visuals/visuals_text-and-shapes.md#labels) and [tables](../2. Visuals/visuals_tables.md). These drawings, especially _labels_, are the most effective for on-chart debugging because they can use dynamic [strings](../1. Concepts/concepts_strings.md) to express information from other data types as custom text.\
\
### [Labels](../4. Writing_Scripts/writing_debugging.md#labels)\
\
[Labels](../2. Visuals/visuals_text-and-shapes.md#labels) display colored shapes and text at specified chart coordinates. In contrast to the outputs of the [plotshape()](../../reference manual/functions/plotshape.md) and [plotchar()](../../reference manual/functions/plotchar.md) functions, labels can display text from “series string” values that change across script executions. Programmers often use labels to visualize the logic of [conditional structures](../3. Language/language_conditional-structures.md) and show text representing information from a script’s global or local scopes.\
\
...
\
#### [Drawing on successive bars](../4. Writing_Scripts/writing_debugging.md#drawing-on-successive-bars)\
\
When inspecting values of varying magnitudes or different types across bars, a simple approach is to create [formatted strings](../1. Concepts/concepts_strings.md#formatting-strings) containing the necessary debug information and display them in [labels](../2. Visuals/visuals_text-and-shapes.md#labels) on each bar requiring analysis.\
\
In this example, we’ve modified the “Average bar ratio” script from the [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) section above. Instead of creating formatted text and displaying information using `log.*()` function calls, this script formats the values separately, then calls [label.new()](../../reference manual/functions/label.new.md) to show the results on the chart within labels anchored to each bar’s [high](../../reference manual/variables/high.md):\
\
![image](../images/Debugging-Pine-drawings-Labels-Drawing-on-successive-bars-1.Y0IdYgw-_ZgHKix.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- The [label.new()](../../reference manual/functions/label.new.md) calls include `force_overlay = true`, meaning the labels always appear on the main chart pane.\
- Unlike the example in the [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) section, this script’s outputs **are** subject to _rollback_, meaning the information shown on a bar reflects only the bar’s _latest_ data. The script does **not** show information for _all_ realtime bar updates.\
\
The above example allows users to inspect the script’s confirmed values or latest updates on any bar that has a [label](../../reference manual/types/label.md) drawing. However, each bar’s results are legible only when the labels do not _overlap_.\
\
An alternative, more _compact_ way to display text with labels on successive bars is to utilize the [label.new()](../../reference manual/functions/label.new.md) function’s `tooltip` parameter instead of the `text` parameter, as labels show their tooltips only when the mouse pointer _hovers_ over them.\
\
In the script version below, we changed all the [label.new()](../../reference manual/functions/label.new.md) calls to use `debugText` as the `tooltip` argument instead of the `text` argument. Now, we can view a specific bar’s information without visual clutter from other nearby labels:\
\
![image](../images/Debugging-Pine-drawings-Labels-Drawing-on-successive-bars-2.oXxEu_CI_es7hX.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
```\
\
Below, we added a condition to the script that draws a label only when the bar’s [time](../../reference manual/variables/time.md) is between the [chart.left\_visible\_bar\_time](../../reference manual/variables/chart.left_visible_bar_time.md) and [chart.right\_visible\_bar\_time](../../reference manual/variables/chart.right_visible_bar_time.md) values. This logic restricts the drawings to _visible_ chart bars, allowing us to scroll through the chart and inspect labels on any bar:\
\
![image](../images/Debugging-Pine-drawings-Labels-Drawing-on-successive-bars-3.DxYYTLXs_Z150tJv.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- The script _restarts_ each time the [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) of the [chart.left\_visible\_bar\_time](../../reference manual/variables/chart.left_visible_bar_time.md) or [chart.right\_visible\_bar\_time](../../reference manual/variables/chart.right_visible_bar_time.md) variables change after the user scrolls or zooms on the chart.\
\
#### [Drawing at the end of the chart](../4. Writing_Scripts/writing_debugging.md#drawing-at-the-end-of-the-chart)\
\
When debugging information does not change frequently across executions, or only the information from a specific execution requires inspection, programmers often display it using [labels](../2. Visuals/visuals_text-and-shapes.md#labels) anchored to the _end_ of the chart.\
\
...
\
![image](../images/Debugging-Pine-drawings-Labels-Drawing-at-the-end-of-the-chart-1.5mqpohZu_ZVOkdF.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- The `printLabel()` function draws _one_ label per function call instance. The label’s `x` property is the maximum of the [last\_bar\_time](../../reference manual/variables/last_bar_time.md) and [chart.right\_visible\_bar\_time](../../reference manual/variables/chart.right_visible_bar_time.md) values, ensuring it appears above the last available bar.\
- On each execution of a `printLabel()` instance, the label’s `text` property updates to reflect the latest `info` value.\
- The [label.new()](../../reference manual/functions/label.new.md) call in the `printLabel()` function includes `force_overlay = true`, meaning the drawing always appears in the main chart pane.\
...
\
### [Tables](../4. Writing_Scripts/writing_debugging.md#tables)\
\
[Tables](../2. Visuals/visuals_tables.md) display text within cells arranged in columns and rows at fixed locations in the chart pane’s visual space. In contrast to other [drawing types](../3. Language/language_type-system.md#drawing-types), which create visuals on the chart at specified _coordinates_, tables appear at one of nine unique, _bar-agnostic_ locations defined by the `position.*` constants.\
\
...
\
This example contains a `printTable()` function that calls [table.new()](../../reference manual/functions/table.new.md) and [table.cell()](../../reference manual/functions/table.cell.md) to create a single-cell table that displays dynamic text in a relative location on the main chart pane. The script uses a single call to this function to display the same chart information shown by the example script from the [previous section](../4. Writing_Scripts/writing_debugging.md#drawing-at-the-end-of-the-chart):\
\
![image](../images/Debugging-Pine-drawings-Tables-1.D2zJtwRb_1tJ10H.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
- This script calls `printTable()` only on the last historical bar and all realtime bars because updating tables on each historical bar is an unnecessary use of runtime resources. See the [Reducing drawing updates](../4. Writing_Scripts/writing_profiling-and-optimization.md#reducing-drawing-updates) section of the [Profiling and optimization](../4. Writing_Scripts/writing_profiling-and-optimization.md) page for more information.\
\
## [Plots and chart colors](../4. Writing_Scripts/writing_debugging.md#plots-and-chart-colors)\
\
The built-in `plot*()` functions display results from a value’s series in up to four locations: the chart pane, the script’s status line, the Data Window, and the price scale. Programmers often use these output functions as a quick way to display the history of a script’s numeric values, conditions, and colors. Two other functions, [bgcolor()](../../reference manual/functions/bgcolor.md) and [barcolor()](../../reference manual/functions/barcolor.md), color a chart pane’s background and the main chart’s bars or candles. Although not as versatile as other output functions, they offer a quick way to display conditions and colors on the chart.\
\
All these functions, especially [plot()](../../reference manual/functions/plot.md), [plotchar()](../../reference manual/functions/plotchar.md), and [plotshape()](../../reference manual/functions/plotshape.md), can serve as helpful tools for debugging a script’s calculations and logic. For instance, the outputs of a single [plot()](../../reference manual/functions/plot.md) call can show the complete available history of a script’s series on the chart and provide information for any bar in other locations.\
\
Before using plots or chart colors for debugging, it is important to note the following limitations:\
\
- Unlike [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) or [drawings](../4. Writing_Scripts/writing_debugging.md#pine-drawings), these outputs _cannot_ display results for values that are accessible from local scopes only. Scripts must [extract](../4. Writing_Scripts/writing_debugging.md#extracting-data-from-local-scopes) values from local scopes into the _global scope_ to debug them with plots or chart colors.\
- The only `plot*()` functions that can display _text_ on the chart — [plotchar()](../../reference manual/functions/plotchar.md) and [plotshape()](../../reference manual/functions/plotshape.md) — require “const string” values. Therefore, they cannot display dynamic strings or calculated [string conversions](../1. Concepts/concepts_strings.md#string-conversion-and-formatting) of other types.\
...
- The maximum _plot count_ for any script is 64. Each call to these functions contributes a different number to the total, depending on its arguments. See the [Plot limits](../4. Writing_Scripts/writing_limitations.md#plot-limits) section of the [Limitations](../4. Writing_Scripts/writing_limitations.md) page to learn more.\
\
### [Plotting numbers](../4. Writing_Scripts/writing_debugging.md#plotting-numbers)\
\
One of the simplest methods to inspect global numeric series (“int” or “float” values) is to plot them using the [plot()](../../reference manual/functions/plot.md), [plotchar()](../../reference manual/functions/plotchar.md), or [plotshape()](../../reference manual/functions/plotshape.md) function. The outputs on the chart pane provide a graphical view of the series’ history. The other possible output locations (status line, price scale, and Data Window) show formatted numbers representing the values calculated on a specific bar.\
\
Let’s look at a simple debugging example. The following script calculates a custom oscillator whose value is the average of three separate oscillators. It displays the `oscillator` value in four output locations using a [plot()](../../reference manual/functions/plot.md) call:\
\
![image](../images/Debugging-Plots-and-chart-colors-Plotting-numbers-1._9pJe3gC_Z14bL5r.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
``\
\
The above script’s outputs allow inspection of the final `oscillator`, but not the three constituent oscillators that determine its value. Because the script calculates all three series in the global scope, we can inspect them using additional plots. Here, we add three [plot()](../../reference manual/functions/plot.md) calls to the script to display each oscillator, allowing us to verify the script’s calculated values and understand how they affect the final result:\
\
![image](../images/Debugging-Plots-and-chart-colors-Plotting-numbers-2.DpbWG2QO_Zn4nwY.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
\
#### [Plotting without affecting the scale](../4. Writing_Scripts/writing_debugging.md#plotting-without-affecting-the-scale)\
\
...
\
![image](../images/Debugging-Plots-and-chart-colors-Plotting-numbers-Plotting-without-affecting-the-scale-1.DCLyJ8at_GSYGc.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- This script includes `precision = 5` in the [indicator()](../../reference manual/functions/indicator.md) declaration statement, which specifies that it plots numbers with five fractional digits instead of using the chart’s default precision setting.\
\
Suppose we want to inspect _all_ the values in the `average` calculation using plots. If we use `plot*()` functions with the default `display` argument ( [display.all](../../reference manual/constants/display.all.md)), the plotted results appear in all possible locations, including the chart pane. Unlike the example script from the [Plotting numbers](../4. Writing_Scripts/writing_debugging.md#plotting-numbers) section, this script’s visuals become hard to read in the pane because each plot has a significantly different range:\
\
![image](../images/Debugging-Plots-and-chart-colors-Plotting-numbers-Plotting-without-affecting-the-scale-2.JCRilku2_2mPLac.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
``\
\
We can change the `display` argument in each debug [plot()](../../reference manual/functions/plot.md) call to view all the calculated values while preserving the chart’s scale. Below, we set the argument to `display.all - display.pane`, meaning all the debug plots show information in all locations _except_ the chart pane. Now, we can visualize how the calculated values affect each bar’s `average` result without distorting the scale:\
\
![image](../images/Debugging-Plots-and-chart-colors-Plotting-numbers-Plotting-without-affecting-the-scale-3.BdsQvxR3_Z103C4I.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- The `display.*` constants support _addition_ and _subtraction_ operations for customized display settings. This script uses subtraction to remove [display.pane](../../reference manual/constants/display.pane.md) from the output locations allowed by [display.all](../../reference manual/constants/display.all.md). Operations that remove valid display constants more than once do not cause errors. For instance, this script produces the _same_ outputs if it subtracts [display.pane](../../reference manual/constants/display.pane.md) once, twice, or more times in the `debugLocations` expression.\
\
### [Plotting and coloring conditions](../4. Writing_Scripts/writing_debugging.md#plotting-and-coloring-conditions)\
\
Programmers can inspect a script’s conditions (“bool” values) with the `plot*()`, [bgcolor()](../../reference manual/functions/bgcolor.md), and [barcolor()](../../reference manual/functions/barcolor.md) functions in several ways, including:\
\
- Using the “bool” condition as the `series` argument in a [plotshape()](../../reference manual/functions/plotshape.md) or [plotchar()](../../reference manual/functions/plotchar.md) call. The call creates a shape/character with specified text on the chart when the condition is `true`, and it shows a _numeric text_ representation of the condition in the status line and Data Window (`1` for `true` and `0` for `false`).\
- Creating a logical expression that returns different “int” or “float” values for the condition’s `true` and `false` states, then using the result as the `series` argument in a `plot*()` call. When using [plotchar()](../../reference manual/functions/plotchar.md) or [plotshape()](../../reference manual/functions/plotshape.md), note that these functions show visuals on the chart only when the `series` value is not [na](../../reference manual/variables/na.md) or 0.\
- Creating a logical expression that returns different “color” values based on the condition’s `true` or `false` state, then using the result to color the chart with [bgcolor()](../../reference manual/functions/bgcolor.md) or [barcolor()](../../reference manual/functions/barcolor.md), or to color a plot or fill.\
\
The following example uses the above methods to debug a simple condition. The script calculates an RSI with an input length and defines a `crossBelow` condition that is `true` when the RSI crosses 30. It uses [plotshape()](../../reference manual/functions/plotshape.md), [plotchar()](../../reference manual/functions/plotchar.md), and [bgcolor()](../../reference manual/functions/bgcolor.md) calls to visualize the `crossBelow` condition in different ways:\
\
![image](../images/Debugging-Plots-and-chart-colors-Plotting-and-coloring-conditions-1.BhI2H3mP_cKpDe.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- The `plot*()` functions that display text or shapes on the chart — [plotshape()](../../reference manual/functions/plotshape.md), [plotchar()](../../reference manual/functions/plotchar.md), and [plotarrow()](../../reference manual/functions/plotarrow.md) — _do not_ display data in the price scale.\
- The [plotshape()](../../reference manual/functions/plotshape.md) call uses `crossUnder` as its `series` argument. The chart pane shows a shape at the top when the condition occurs. The status line and Data Window show 1 when the `series` is `true` and 0 when it is `false`.\
- The [plotchar()](../../reference manual/functions/plotchar.md) call plots the result of a ternary expression that returns the `rsi` when `crossUnder` is `true` and [na](../../reference manual/variables/na.md) otherwise. It shows the character U+2930 at the `rsi` location when the expression does not evaluate to [na](../../reference manual/variables/na.md). Because the `series` argument is a “float” value, the number in the status line and Data Window represents that value directly.\
- The [bgcolor()](../../reference manual/functions/bgcolor.md) call highlights the chart’s background when `crossUnder` is `true`, but it does not display information in the status line or Data Window.\
\
The [plotshape()](../../reference manual/functions/plotshape.md) and [plotchar()](../../reference manual/functions/plotchar.md) functions have a `text` parameter that adds “const string” text to the plotted shapes/characters. When debugging multiple global conditions, it is often helpful to call these functions with `text` arguments to label each condition for simple on-chart inspection. The arguments can contain the newline character (`\n` [escape sequence](../1. Concepts/concepts_strings.md#escape-sequences)), allowing scripts to plot multiple shapes in identical locations with non-overlapping text.\
\
Let’s explore a debugging example using this approach. The script below calculates an RSI and its median over `lengthInput` bars. Then, it creates five singular conditions and uses them to form a _compound condition_. The script plots the `rsi` and `median` values with the `plot()` function, and it colors the background with [bgcolor()](../../reference manual/functions/bgcolor.md) when the `compoundCondition` is `true`:\
\
![image](../images/Debugging-Plots-and-chart-colors-Plotting-and-coloring-conditions-2.1wtCT3lE_Z17TAu2.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
``\
\
To verify that the script’s logic works as intended, we can inspect each of the conditions that affect the final `compoundCondition` value. Below, we added five [plotchar()](../../reference manual/functions/plotchar.md) calls to display information for these conditions, each with the same `location` argument. To label the conditions on the chart, each [plotchar()](../../reference manual/functions/plotchar.md) call uses a string containing newline characters (`\n`) and a digit from 1 to 5 as the `text` argument. With these outputs, we can see which sets of conditions trigger each `compoundCondition` occurrence:\
\
![image](../images/Debugging-Plots-and-chart-colors-Plotting-and-coloring-conditions-3.STxZruzR_Z2mTbLW.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- The `char` argument of each [plotchar()](../../reference manual/functions/plotchar.md) call is an empty string, meaning the function displays its `text` value without a character above it.\
- Because each [plotchar()](../../reference manual/functions/plotchar.md) call outputs results at the same relative location (`location.top`), we included different numbers of leading `\n` sequences in the `text` arguments to move the displayed numerals down and ensure they do not overlap.\
- The `title` argument of each [plotchar()](../../reference manual/functions/plotchar.md) call contains the condition number to distinguish it in the Data Window.\
- The [plotshape()](../../reference manual/functions/plotshape.md) call’s title describes the compound condition’s structure in the Data Window.\
\
To learn more about the [plotshape()](../../reference manual/functions/plotshape.md) and [plotchar()](../../reference manual/functions/plotchar.md) functions and how their outputs differ from [labels](../2. Visuals/visuals_text-and-shapes.md#labels), refer to the [Text and shapes](../2. Visuals/visuals_text-and-shapes.md) page.\
\
## [Tips and techniques](../4. Writing_Scripts/writing_debugging.md#tips-and-techniques)\
\
The following sections explain several additional tips and helpful techniques for effective Pine Script debugging.\
\
### [Decomposing expressions](../4. Writing_Scripts/writing_debugging.md#decomposing-expressions)\
\
One of the best practices for efficient debugging is to _split expressions_, especially those with multiple calculations or logical operations, into _smaller parts_ assigned to separate variables. Decomposing expressions enables programmers to inspect each critical part individually, making it easier to verify calculations or logic and isolate potential issues in the code. Additionally, complex code broken down into smaller parts is typically simpler to read, maintain, and [profile](../4. Writing_Scripts/writing_profiling-and-optimization.md#profiling-a-script).\
\
The following script calculates a custom oscillator representing the smoothed median change in the differences between the [close](../../reference manual/variables/close.md) price and two EMAs over different lengths. The script performs all the calculations in a single expression assigned to the `osc` variable. Then, it creates a compound condition in another expression assigned to the `upSignal` variable and uses that variable to trigger [order placement commands](../1. Concepts/concepts_strategies.md#order-placement-and-cancellation). The script plots the `osc` series as columns with different colors based on the `upSignal` value:\
\
![image](../images/Debugging-Tips-and-techniques-Decomposing-expressions-1.DPZlbAAY_EhdjF.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
\
The script version below declares several extra variables to hold _different parts_ of the original `osc` and `upCondition` expressions. With this expanded structure, we can inspect each part of the calculations and logic step-by-step using various outputs. In this script, we included a single [log.info()](../../reference manual/functions/log.info.md) call at the end that displays [formatted text](../1. Concepts/concepts_strings.md#formatting-strings) containing each variable’s information in the [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) pane:\
\
![image](../images/Debugging-Tips-and-techniques-Decomposing-expressions-2.CWIhg_Os_Z21t52k.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
- The script calls [log.info()](../../reference manual/functions/log.info.md) only when [barstate.isconfirmed](../../reference manual/variables/barstate.isconfirmed.md) is `true`, preventing unnecessary logs on the ticks of unconfirmed bars.\
- All the placeholders with the `number` modifier in the [log.info()](../../reference manual/functions/log.info.md) call’s formatting string include the `0.00000` pattern, which forces the formatted numbers to always show five fractional digits. Refer to the [Formatting strings](../1. Concepts/concepts_strings.md#formatting-strings) section of the [Strings](../1. Concepts/concepts_strings.md) page for more information.\
- The Pine Logs pane displays up to 10,000 historical logs. To view earlier logs, add another condition to the [if](../../reference manual/keywords/if.md) structure that limits the [log.info()](../../reference manual/functions/log.info.md) call to specific bars. See the [Custom code filters](../4. Writing_Scripts/writing_debugging.md#custom-code-filters) section above for an example that restricts `log.*()` calls using [time inputs](../1. Concepts/concepts_inputs.md#time-input).\
\
### [Extracting data from local scopes](../4. Writing_Scripts/writing_debugging.md#extracting-data-from-local-scopes)\
\
...
\
All identifiers declared outside [user-defined functions](../3. Language/language_user-defined-functions.md), [methods](../3. Language/language_methods.md#user-defined-methods), [loops](../3. Language/language_loops.md), [conditional structures](../3. Language/language_conditional-structures.md), or [user-defined type](../3. Language/language_type-system.md#user-defined-types) and [enum type](../3. Language/language_type-system.md#enum-types) declarations belong to the _global scope_. Identifiers in the global scope are accessible to most _inner_ (local) scopes after declaration. Every Pine script has exactly **one** global scope.\
\
...
\
The following sections explain techniques for extracting data from local scopes using [return expressions](../4. Writing_Scripts/writing_debugging.md#extraction-using-return-expressions) and [reference types](../4. Writing_Scripts/writing_debugging.md#extraction-using-reference-types). We demonstrate these techniques on the following script, which contains a `customMA()` function that calculates a custom adaptive moving average of a `source` series based on the distance from its current value to its 25th and 75th percentiles over `length` bars. The script contains a local function scope, and a nested block scope from the [if](../../reference manual/keywords/if.md) structure that sets the `outerRange` value:\
\
![image](../images/Debugging-Tips-and-techniques-Extracting-data-from-local-scopes-1.7noUNshc_24uJM8.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
``\
\
#### [Extraction using return expressions](../4. Writing_Scripts/writing_debugging.md#extraction-using-return-expressions)\
\
In Pine Script, any [user-defined function](../3. Language/language_user-defined-functions.md) or [method](../3. Language/language_methods.md#user-defined-methods) call, [loop](../3. Language/language_loops.md), or [conditional structure](../3. Language/language_conditional-structures.md) _returns_ the result of the _final_ expression or nested structure within its local scope. Scripts can use these structures’ returned results, excluding [void](../3. Language/language_type-system.md#void), by assigning them to variables declared in the _outer_ scope.\
\
When debugging functions and conditional structures that contain multiple local variables, a common technique to extract data from their scopes is to return [tuples](../3. Language/language_type-system.md#tuples) containing the data that requires inspection.\
\
Here, we’ve modified the [previous](../4. Writing_Scripts/writing_debugging.md#extracting-data-from-local-scopes) example script’s `customMA()` function to return a tuple containing values calculated from the local scopes. With this change, the script can call the function with a _tuple declaration_ to make all the data available to the global scope. The script plots the `q1Dbg` and `q3Dbg` values, highlights the background when `alphaDbg` is 0, and uses [log.info()](../../reference manual/functions/log.info.md) to display a formatted string containing all the extracted data in the [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) pane:\
\
![image](../images/Debugging-Tips-and-techniques-Extracting-data-from-local-scopes-Extraction-using-return-expressions-1.gNLzCXLM_Rj8IR.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- We added a tuple at the _end_ of the [if](../../reference manual/keywords/if.md) structure’s block to _return_ the `upperRange` and `lowerRange` values from its local scope. The function assigns the result to a two-variable tuple in its main scope, enabling it to include the [if](../../reference manual/keywords/if.md) structure’s local values in the return expression.\
\
#### [Extraction using reference types](../4. Writing_Scripts/writing_debugging.md#extraction-using-reference-types)\
\
_Reference types_, including all [special types](../3. Language/language_type-system.md#types) and [user-defined types (UDTs)](../3. Language/language_type-system.md#user-defined-types), serve as structures for creating _objects_. Each object has an associated _reference_ that distinguishes it and provides access to its data. Unlike fundamental types, variables of reference types **do not** store values directly. Instead, they hold the references for specific objects in memory.\
\
An advanced, flexible way to extract data from local scopes is to initialize reference-type objects — such as instances of [collections](../3. Language/language_type-system.md#collections) or UDTs — in the global scope and store local variable data in their elements or fields.\
\
This technique is especially useful for extracting data from [user-defined functions](../3. Language/language_user-defined-functions.md) and [methods](../3. Language/language_methods.md#user-defined-methods). Although functions can access global variables, they cannot _reassign_ them like global [conditional structures](../3. Language/language_conditional-structures.md) and [loops](../3. Language/language_loops.md) can. Consequently, they _cannot update_ the data held by global variables of fundamental types. However, scripts do not modify reference types by reassigning their variables; they access objects via their references and use _methods_ or _field reassignments_ to update their data. As such, scripts _can_ update global collections or UDT instances from inside function scopes.\
\
For example, this modified version of our [initial script](../4. Writing_Scripts/writing_debugging.md#extracting-data-from-local-scopes) declares a global `debugData` variable that holds the reference of a [map](../../reference manual/types/map.md) with “string” keys and “float” values. Each [map.put()](../../reference manual/functions/map.put.md) call inside the `customMA()` scope modifies the map by adding a _key-value pair_ containing a local variable’s _name_ and _value_. After calling `customMA()`, the script uses [map.get()](../../reference manual/functions/map.get.md) calls on `debugData` to retrieve the stored information for its debugging outputs:\
\
![image](../images/Debugging-Tips-and-techniques-Extracting-data-from-local-scopes-Extraction-using-reference-types-1.BdAF7qQM_Z1fHdXL.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- The script declares `debugData` with the [var](../../reference manual/keywords/var.md) keyword, meaning the assigned map reference _persists_ across script executions.\
...
- Because the [map.put()](../../reference manual/functions/map.put.md) calls in `customMA()` assign keys to the map that do not change across executions, each `customMA()` call _replaces_ the `debugData` map’s existing data. Programmers can _preserve_ data from specific executions with this technique by making a _copy_ of the global collection after the function call.\
\
### [Inspecting loops](../4. Writing_Scripts/writing_debugging.md#inspecting-loops)\
\
[Loops](../3. Language/language_loops.md) are structures that execute a local code block repeatedly based on a _counter_ ( [for](../../reference manual/keywords/for.md)), the _contents_ of a [collection](../3. Language/language_type-system.md#collections) ( [for…in](../../reference manual/keywords/for...in.md)), or a _condition_ ( [while](../../reference manual/keywords/while.md)). These structures allow scripts to perform repetitive tasks without redundant lines of code.\
\
Because loops can execute their local code multiple times, programmers must use techniques to track local variables _across iterations_ to debug them effectively. As with other structures, there are many ways to inspect loops. These sections cover two helpful techniques: [collecting loop information](../4. Writing_Scripts/writing_debugging.md#collecting-loop-information) and [tracing loop executions](../4. Writing_Scripts/writing_debugging.md#tracing-loop-executions).\
\
#### [Collecting loop information](../4. Writing_Scripts/writing_debugging.md#collecting-loop-information)\
\
One of the most effective loop inspection techniques is to use [collections](../3. Language/language_type-system.md#collections) or [strings](../1. Concepts/concepts_strings.md) to gather information from the local scope on each iteration requiring inspection, then use the information in output functions _after_ the loop terminates.\
\
Let’s look at a simple loop debugging example using this technique. The following script calculates the average rate of change in the [close](../../reference manual/variables/close.md) price over lengths from 1 to `lookbackInput` bars inside a [for](../../reference manual/keywords/for.md) loop. It declares an `aroc` variable in the global scope, sums the rates of change inside the loop, and then divides the sum by the `lookbackInput` to calculate the average:\
\
![image](../images/Debugging-Tips-and-techniques-Inspecting-loops-Collecting-loop-information-1.CbIJsIuS_1RptDD.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
``\
\
To debug the script’s loop and ensure it works as intended, we can collect data from the local scope on each iteration and pass the result to the available output functions after the loop ends. In the script version below, we demonstrate two extraction methods. The first declares a global `logText` variable and concatenates [formatted strings](../1. Concepts/concepts_strings.md#formatting-strings) containing each loop iteration’s `length` and `roc` values. The second declares a global `rocArray` variable and pushes each iteration’s `roc` value into the referenced [array](../../reference manual/types/array.md).\
\
After terminating the loop, the script calls [log.info()](../../reference manual/functions/log.info.md) to display the `logText` in the [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) pane if the bar is confirmed. It then displays a “string” representation of the `rocArray` inside [label](../../reference manual/functions/label.md) tooltips. Lastly, it shows the array’s _first_ and _last_ element values in all possible plot locations with the [plot()](../../reference manual/functions/plot.md) function:\
\
![image](../images/Debugging-Tips-and-techniques-Inspecting-loops-Collecting-loop-information-2.CS9aDiYb_ZPNykR.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- Scripts can generate [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) and [drawings](../4. Writing_Scripts/writing_debugging.md#pine-drawings) directly from within a loop’s local scope. However, because loops usually execute their local code _more than once_, calling `log.*()` or [label.new()](../../reference manual/functions/label.new.md) functions inside the scope can result in numerous logs or [labels](../2. Visuals/visuals_text-and-shapes.md#labels) per bar. Logging on each iteration helps trace execution patterns, but it also limits the number of _historical_ bars with available debug data. See the next section, [Tracing loop executions](../4. Writing_Scripts/writing_debugging.md#tracing-loop-executions), for an example.\
- [Strings](../1. Concepts/concepts_strings.md) can contain up to _40,960_ characters, and large strings or repeated [concatenation](../1. Concepts/concepts_strings.md#concatenation) can impact a script’s performance. Therefore, extracting loop information with string concatenation is suitable for relatively _small_ loops or inspecting specific variables. To extract large amounts of data from loops, use [collections](../3. Language/language_type-system.md#collections) instead.\
\
#### [Tracing loop executions](../4. Writing_Scripts/writing_debugging.md#tracing-loop-executions)\
\
An alternative way to inspect a loop, without collecting information for use in the _outer scope_, is to add `log.*()` calls directly to the loop’s _local block_. Each iteration that activates the call results in a new message in the [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) pane, allowing programmers to trace the loop’s execution pattern in detail.\
\
This simple script calculates a pseudorandom sample from a [binomial distribution](https://en.wikipedia.org/wiki/Binomial_distribution) using a [for](../../reference manual/keywords/for.md) loop. The plotted `sample` series represents the number of [math.random()](../../reference manual/functions/math.random.md) calls across `trialsInput` iterations that return a value not exceeding the `probabilityInput` value. On each iteration where `success` is `false`, the loop _skips_ the rest of its block and moves to the next iteration. On other iterations, it increments the `sample` value by one:\
\
![image](../images/Debugging-Tips-and-techniques-Inspecting-loops-Tracing-loop-executions-1.KSzq3fPt_ap77Q.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
``\
\
Below, we added `log.*()` function calls to generate [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) at specific points in the loop’s local block across iterations. Each loop iteration creates two new logs. The first log shows formatted text containing the local `trial`, `randValue`, and `success` variables’ values. The second log depends on the [if](../../reference manual/keywords/if.md) statement. When the statement’s local code executes, the log is a `"CONTINUE"` message with the “warning” level. Otherwise, the second log is an “info” message containing the current iteration’s `sample` value:\
\
![image](../images/Debugging-Tips-and-techniques-Inspecting-loops-Tracing-loop-executions-2.f9od3pWY_Z2dUjRM.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- The script includes [log.warning()](../../reference manual/functions/log.warning.md) calls before and after the loop to mark its start and end in the Pine Logs pane. The message marking the start of the loop also displays the current [bar\_index](../../reference manual/variables/bar_index.md) value.\
- The Pine Logs pane shows only the most recent 10,000 logs created on historical bars. Because this script creates multiple logs per bar, the earliest message in the pane is from less than 10,000 bars back. Programmers can use conditional logic that limits `log.*()` calls in order to inspect a loop’s execution flow on earlier bars with this technique. See the [Custom code filters](../4. Writing_Scripts/writing_debugging.md#custom-code-filters) section to learn more.\
\
### [Debugging collections](../4. Writing_Scripts/writing_debugging.md#debugging-collections)\
\
[Collections](../3. Language/language_type-system.md#collections) are data structures that store values or references as _elements_, which scripts access using indices or keys, depending on the type. These structures can contain a lot of information, as the maximum number of elements across all instances of each collection type is 100,000.\
\
...
\
- Creating a “string” representation of the collection with [str.tostring()](../../reference manual/functions/str.tostring.md) and displaying the result using [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) or other text outputs.\
- Retrieving specific elements from the collection, then creating [formatted strings](../1. Concepts/concepts_strings.md#formatting-strings) for logging, or using the element values or references in other output processes.\
\
#### [Displaying collection strings](../4. Writing_Scripts/writing_debugging.md#displaying-collection-strings)\
\
The simplest way to inspect the data of [arrays](../3. Language/language_arrays.md) and [matrices](../3. Language/language_matrices.md) of “int”, “float”, “bool”, and “string” types is to generate “string” representations with the [str.tostring()](../../reference manual/functions/str.tostring.md) function, then display the results using [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) or other “string” outputs.\
\
The following script calls [request.security\_lower\_tf()](../../reference manual/functions/request.security_lower_tf.md) to retrieve a “float” [array](../../reference manual/types/array.md) containing [close](../../reference manual/variables/close.md) prices for each lower-timeframe bar within the current chart bar, which it uses to calculate an average intrabar price. Then, it calculates the ratio of the difference between the bar’s price and the intrabar average to the bar’s total range. The script plots the resulting ratio and its EMA in a separate pane:\
\
![image](../images/Debugging-Tips-and-techniques-Debugging-collections-Displaying-collection-strings-1.C-bZsQtI_14WMLV.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
\
The script version below declares a `debugText` variable that holds a [formatted string](../1. Concepts/concepts_strings.md#formatting-strings) representing the `intrabarPrices` array, the array’s size, and the `avgPrice` value. The script calls the `log.*()` functions to display the `debugText` value for each bar in the [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) pane:\
\
![image](../images/Debugging-Tips-and-techniques-Debugging-collections-Displaying-collection-strings-2.DKihV-eb_HIwbl.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- The script calls [log.info()](../../reference manual/functions/log.info.md) on confirmed bars and [log.warning()](../../reference manual/functions/log.warning.md) on the open bar. Users can [filter](../4. Writing_Scripts/writing_debugging.md#filtering-logs) the logs by [logging level](../4. Writing_Scripts/writing_debugging.md#logging-level) to inspect confirmed and unconfirmed bars’ logs separately.\
- For larger collections whose “string” representations _exceed_ 40,960 characters or cause excessive memory use, programmers can split them into _smaller parts_ and convert them to strings separately. Alternatively, they can inspect [individual elements](../4. Writing_Scripts/writing_debugging.md#inspecting-individual-elements) via the `*.get()` method or [for…in](../../reference manual/keywords/for...in.md) loops.\
\
#### [Inspecting individual elements](../4. Writing_Scripts/writing_debugging.md#inspecting-individual-elements)\
\
Collections of “color” or non-fundamental types (e.g., [labels](../2. Visuals/visuals_text-and-shapes.md#labels)) _do not_ have built-in “string” representations. Consequently, the technique described in the [Displaying collection strings](../4. Writing_Scripts/writing_debugging.md#displaying-collection-strings) section does not work for them.\
\
To inspect a collection that does not have a built-in “string” format, programmers can retrieve elements individually within [for…in](../../reference manual/keywords/for...in.md) loops or using methods such as `*.get()`, then use those elements in custom “string” constructions or other output routines.\
\
Consider the following example, which calculates the ratio of [close](../../reference manual/variables/close.md) changes to the overall [close](../../reference manual/variables/close.md) range over `lengthInput` bars. It plots the resulting `osc` in a separate pane, and it draws a [label](../../reference manual/types/label.md) on the main chart pane each time the variable’s absolute value is 1:\
\
![image](../images/Debugging-Tips-and-techniques-Debugging-collections-Inspecting-individual-elements-1.DejiM-hQ_ZKsXyw.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
``\
\
When a script creates [labels](../2. Visuals/visuals_text-and-shapes.md#labels), it _automatically_ maintains an [array](../../reference manual/types/array.md) containing each active label’s reference. Programmers can access this array using the [label.all](../../reference manual/variables/label.all.md) variable, and thus inspect each individual label’s properties on any bar.\
\
In the version below, the script executes a [log.info()](../../reference manual/functions/log.info.md) call to display the current [bar\_index](../../reference manual/variables/bar_index.md) and the size of the [label.all](../../reference manual/variables/label.all.md) array for the latest bar. Then, it iterates through the array with a [for…in](../../reference manual/keywords/for...in.md) loop. On each iteration, the script calls [log.info()](../../reference manual/functions/log.info.md) to log formatted text containing the array index and the corresponding label’s `x`, `y`, and `text` properties. Additionally, the script plots the oldest and newest active labels’ y-coordinates on each bar:\
\
![image](../images/Debugging-Tips-and-techniques-Debugging-collections-Inspecting-individual-elements-2.CxLrVvEC_ZACjuz.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- It is not possible to obtain _all_ properties from drawing objects. For example, there is no built-in method to retrieve a label’s color. Some other types, such as [table](../../reference manual/types/table.md), do not have `*.get_*()` methods. If an object’s properties are not directly accessible, programmers can create separate variables for the arguments of the drawing’s `*.new()` or `*.set_*()` function, and then use those variables for debugging.\
- In the above image, the logs show that the [label.all](../../reference manual/variables/label.all.md) array contains 55 elements. By default, Pine limits the number of labels to approximately 50, but the _precise_ number of active labels varies. Programmers can increase the label drawing limit using the `max_labels_count` parameter of the [indicator()](../../reference manual/functions/indicator.md) or [strategy()](../../reference manual/functions/strategy.md) declaration statement.\
\
### [Debugging objects of UDTs](../4. Writing_Scripts/writing_debugging.md#debugging-objects-of-udts)\
\
[User-defined types (UDTs)](../3. Language/language_type-system.md#user-defined-types) define the structures of [objects](../3. Language/language_objects.md). Objects contain a fixed set of _fields_, where each field can hold a separate value or reference to another specified type, even to another instance of the _same_ user-defined type.\
\
...
\
The following example defines a custom `Data` type with three fields. The first two fields reference [arrays](../3. Language/language_arrays.md) that hold successive price and time values. The third field specifies the number of bars between each new data sample. The script creates a new object of this type with a randomized `length` field on the first bar, then updates its arrays on bars whose [bar\_index](../../reference manual/variables/bar_index.md) values are divisible by that field.\
\
The script uses [array.covariance()](../../reference manual/functions/array.covariance.md) and [array.variance()](../../reference manual/functions/array.variance.md) on the object’s `prices` and `times` arrays to calculate a time-based slope of the collected data, and then plots the result on the chart:\
\
![image](../images/Debugging-Tips-and-techniques-Debugging-objects-of-udts-1.BNdDHvUW_4GfGk.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
``\
\
To verify and understand the script’s calculations, we can _extract_ information from the `Data` object’s _fields_ and inspect the data with [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) or other outputs.\
\
The script version below includes a [log.info()](../../reference manual/functions/log.info.md) call inside the [if](../../reference manual/keywords/if.md) structure. The call displays [formatted text](../1. Concepts/concepts_strings.md#formatting-strings) representing information from the `Data` object’s `prices`, `times`, and `length` fields in the Pine Logs pane. Now, we can view each change to the object’s data to confirm the script’s behavior:\
\
![image](../images/Debugging-Tips-and-techniques-Debugging-objects-of-udts-2.DboJuwuM_nQEyE.webp)\
\
[Pine Script®](https://tradingview.com/pine-script-docs)\
Copied\
\
...
Note that:\
\
- The script calls [log.info()](../../reference manual/functions/log.info.md) on confirmed bars and [log.warning()](../../reference manual/functions/log.warning.md) on open bars, allowing users to [filter](../4. Writing_Scripts/writing_debugging.md#filtering-logs) the results by [logging level](../4. Writing_Scripts/writing_debugging.md#logging-level) in the Pine Logs pane.\
\
### [Organization and readability](../4. Writing_Scripts/writing_debugging.md#organization-and-readability)\
\
...
\
Below are a few helpful coding recommendations based on our [Style guide](../4. Writing_Scripts/writing_style-guide.md) and best practices:\
\
- Follow the [script organization](../4. Writing_Scripts/writing_style-guide.md#script-organization) guidelines. Organizing scripts based on this structure makes different parts of the code simple to locate and inspect.\
- Use _identifiers_ that you can read, distinguish, and understand. When a code contains unclear identifiers, it is often harder to debug efficiently. See our [Naming conventions](../4. Writing_Scripts/writing_style-guide.md#naming-conventions) to learn our recommended identifier format.\
...
- Document the code using comments and [compiler annotations](../3. Language/language_script-structure.md#compiler-annotations) (`//@function`, `//@variable`, etc.). The Pine Editor’s autosuggest displays the text from annotations when the mouse pointer hovers over identifiers, making it simple to recall what different parts of the code represent.\
\
[Previous 
**Style guide**](../4. Writing_Scripts/writing_style-guide.md) [Next 
**Profiling and optimization**](../4. Writing_Scripts/writing_profiling-and-optimization.md)