![](../5. Errors_And_Warnings/errors_re10139.md)

## [Memory limits exceeded](../5. Errors_And_Warnings/errors_re10139.md#memory-limits-exceeded)

The most common cause of this error is the retrieval of custom [objects](../3. Language/language_objects.md#objects) and [collections](../3. Language/language_type-system.md#collections) from `request.*()` functions such as [request.security()](../../reference manual/functions/request.security.md). Other possible causes include unnecessary drawing updates, excess historical buffer capacity, or inefficient use of [max\_bars\_back()](../../reference manual/functions/max_bars_back.md).

The sections below explain these issues and potential solutions to fix them.

### [Requesting collections and other objects with ​`request.*()`​ calls](../5. Errors_And_Warnings/errors_re10139.md#requesting-collections-and-other-objects-with-request-calls)

The “Memory limits exceeded” error most often occurs when a script calls [request.security()](../../reference manual/functions/request.security.md) or other `request.*()` functions to create custom [objects](../3. Language/language_objects.md#objects) or [collections](../3. Language/language_type-system.md#collections) on the dataset for another symbol or timeframe.
...
For example, the script below calls [request.security()](../../reference manual/functions/request.security.md) to request the result of a [user-defined function](../3. Language/language_user-defined-functions.md) call evaluated on the “1D” timeframe. The custom function (`dataFunction()`) creates a “float” [array](../3. Language/language_arrays.md) and assigns its ID to a persistent variable declared [using the \`var\` keyword](../3. Language/language_arrays.md#using-var-and-varip-keywords). On each bar, the function pushes a new [balance of power (BOP)](https://www.tradingview.com/support/solutions/43000589100-balance-of-power-bop/) value into its array and returns the array’s ID. Each time that the [request.security()](../../reference manual/functions/request.security.md) call evaluates the `dataFunction()` call, it creates a _new copy_ of the array and returns that copy’s ID. Storing a new copy of the array on each bar requires a lot of memory. Consequently, this script can _exceed_ the memory limits when it runs on datasets that have a lengthy history:
...
### [How do I fix this?](../5. Errors_And_Warnings/errors_re10139.md#how-do-i-fix-this)
...
#### [Return the last state only](../5. Errors_And_Warnings/errors_re10139.md#return-the-last-state-only)

If a script requires only the _latest_ state of a requested collection, use a [conditional structure](../3. Language/language_conditional-structures.md) or expression with [barstate.islast](../../reference manual/variables/barstate.islast.md) as the condition to limit retrieving a copy of that collection to the _last_ bar where the latest state is available.

For example, suppose we want our [initial script](../5. Errors_And_Warnings/errors_re10139.md#requesting-collections-and-other-objects-with-request-calls) to plot the BOP value and display the current average in a [table](../2. Visuals/visuals_tables.md). Users cannot see the past results displayed by a table as the script loads on historical bars. Only the _final_ result as of the latest bar is visible to users. Therefore, to minimize the script’s memory demands, we can structure the script’s request to return a usable ID on the last bar only.

Below, we modified the script’s custom function to return a two-item [tuple](../3. Language/language_type-system.md#tuples). On the last bar, the function returns the BOP value and the array’s ID. On all previous bars, it returns only the BOP value and an [na](../../reference manual/variables/na.md) array ID. The script requests the result of a call to this revised function. It plots the retrieved BOP value on each bar, then calculates and displays the requested array’s average in a table on the last bar if the retrieved array ID is not [na](../../reference manual/variables/na.md):

![image](../images/RE10139-1.CwspkT8__Eg4BM.webp)
...
#### [Return IDs on some bars](../5. Errors_And_Warnings/errors_re10139.md#return-ids-on-some-bars)

If a script needs to access a requested collection or other object in its main context, but _not_ on _every bar_, structure the request to use [conditional structures](../3. Language/language_conditional-structures.md) or expressions that return the object’s ID on only necessary bars, and [na](../../reference manual/variables/na.md) on other bars. The logic in the script’s main context can then handle the [na](../../reference manual/variables/na.md) gaps in the requested series as necessary.

Suppose we want to modify our [initial script](../5. Errors_And_Warnings/errors_re10139.md#requesting-collections-and-other-objects-with-request-calls) to calculate the average, high, and low daily BOP for each _completed month_. We can structure the script’s request to collect BOP values across the days in each month and return an array ID for the necessary calculations only after a month ends. The `dataFunction()` function in the following script version pushes BOP values for each bar in a month into an array and returns a two-item tuple. The tuple contains the BOP value and the array’s ID only on the first bar of a month. On other bars, the tuple contains the BOP value and an [na](../../reference manual/variables/na.md) ID. The script uses a call to this updated function in its data request. On bars where the requested array ID is not [na](../../reference manual/variables/na.md), the script calculates the average, high, and low values from that array, then assigns the results to persistent variables for plotting:

![image](../images/RE10139-2.hkUJzj7s_lgT0n.webp)
...
#### [Return calculated results](../5. Errors_And_Warnings/errors_re10139.md#return-calculated-results)
...
### [Other possible error sources and their fixes](../5. Errors_And_Warnings/errors_re10139.md#other-possible-error-sources-and-their-fixes)
...
#### [Minimize ​`request.*()`​ calls](../5. Errors_And_Warnings/errors_re10139.md#minimize-request-calls)
...
This memory consumption is especially substantial for scripts that request data from [lower timeframes](../1. Concepts/concepts_other-timeframes-and-data.md#lower-timeframes) with [request.security\_lower\_tf()](../../reference manual/functions/request.security_lower_tf.md), because the function returns the IDs of [arrays](../3. Language/language_arrays.md) that contain intrabar data for _each_ bar in the script’s main dataset. For example, a script that requests one-minute data from a “1D” chart creates a new array containing hundreds of intrabar data points on each bar. The system must allocate sufficient memory to store each of those large arrays so that the script can access them later in the main context. Maintaining that much data in memory requires a significant amount of resources.
...
See the [Minimizing `request.*()` calls](../4. Writing_Scripts/writing_profiling-and-optimization.md#minimizing-request-calls) section of the [Profiling and optimization](../4. Writing_Scripts/writing_profiling-and-optimization.md) page to learn more about optimizing data requests.

#### [Use ​`max_bars_back`​ only when necessary](../5. Errors_And_Warnings/errors_re10139.md#use-max_bars_back-only-when-necessary)

The `max_bars_back` parameter of an [indicator](../../reference manual/functions/indicator.md) or [strategy](../../reference manual/functions/strategy.md) script sets the size of the [historical buffers](../3. Language/language_execution-model.md#historical-buffers) for _all_ series in a script. Each buffer determines the number of historical data points that the system _maintains in memory_ for the script’s variables and expressions.

By default, the Pine Script® runtime system attempts to automatically define an appropriate buffer for each variable and expression. Therefore, using the `max_bars_back` parameter or [max\_bars\_back()](../../reference manual/functions/max_bars_back.md) function is typically necessary only in the rare case where the system _cannot_ determine a sufficient buffer size. See the error page [The requested historical offset (X) is beyond the historical buffer’s limit (Y)](../5. Errors_And_Warnings/errors_re10143.md) to learn about the potential causes of this issue.

If you encounter a buffer error and must manually set the sizes of a script’s historical buffers by using the `max_bars_back` parameter or the [max\_bars\_back()](../../reference manual/functions/max_bars_back.md) function, choose the _smallest_ buffer sizes that accommodate your script’s historical references. Setting buffer sizes to store _more_ data than necessary causes an _excessive_ use of memory resources.

#### [Minimize historical buffer calculations](../5. Errors_And_Warnings/errors_re10139.md#minimize-historical-buffer-calculations)

As explained above, the Pine Script runtime system creates _historical buffers_ for all variables and expressions in a script. It determines the size of each buffer based on the _historical references_ that the script performs via the [\[\] history-referencing operator](../3. Language/language_operators.md#-history-referencing-operator) or the functions that reference history internally.
...
Specifying a `calc_bars_count` argument in the [indicator()](../../reference manual/functions/indicator.md) or [strategy()](../../reference manual/functions/strategy.md) declaration statement can help reduce memory issues, because it _restricts_ the number of _historical bars_ that the script can use for its calculations. Similarly, using the [max\_bars\_back()](../../reference manual/functions/max_bars_back.md) function to manually define the appropriate size for a buffer can help reduce buffer calculations. When using this function to specify the size of historical buffers, choose the **smallest** possible size that accommodates the script’s historical references, to avoid unnecessary memory use.

To learn more about historical buffer calculations and how to optimize them, see the [Minimizing historical buffer calculations](../4. Writing_Scripts/writing_profiling-and-optimization.md#minimizing-historical-buffer-calculations) section of the [Profiling and optimization](../4. Writing_Scripts/writing_profiling-and-optimization.md) page.

#### [Reduce drawing updates for tables](../5. Errors_And_Warnings/errors_re10139.md#reduce-drawing-updates-for-tables)

[Tables](../2. Visuals/visuals_tables.md) display only their _latest state_ as of the last available bar on the chart. The previous states of a table on historical bars are _not_ visible to script users. Therefore, updating and displaying tables across historical bars is typically _unnecessary_. To use the least memory in table drawings, prevent unneccessary updates by defining logic that creates the [table](../../reference manual/types/table.md) object once and then populates it on only the _last bar_.

To create a [table](../../reference manual/types/table.md) object on only one bar, assign the result of the [table.new()](../../reference manual/functions/table.new.md) call to a variable declared with the [var](../../reference manual/keywords/var.md) keyword. Then, populate the table on the last bar by placing calls to [table.cell()](../../reference manual/functions/table.cell.md) or the `table.cell_set_*()` functions inside a [conditional structure](../3. Language/language_conditional-structures.md) that uses [barstate.islast](../../reference manual/variables/barstate.islast.md) as its condition. See the [Tables](../2. Visuals/visuals_tables.md) page to learn more.

#### [Do not update drawings on historical bars](../5. Errors_And_Warnings/errors_re10139.md#do-not-update-drawings-on-historical-bars)

As with tables, all other [drawing objects](../3. Language/language_type-system.md#drawing-types), such as [lines](../2. Visuals/visuals_lines-and-boxes.md#lines) and [labels](../2. Visuals/visuals_text-and-shapes.md#labels), show only their _latest_ states after the script loads on the chart. Users cannot see the visuals update as the script executes across history; users see only the drawing updates that occur on _realtime bars_.

To help reduce a script’s runtime and memory use, eliminate updates to drawings on historical bars wherever possible. Refer to the [Reducing drawing updates](../4. Writing_Scripts/writing_profiling-and-optimization.md#reducing-drawing-updates) section of the [Profiling and optimization](../4. Writing_Scripts/writing_profiling-and-optimization.md) page for more information.

#### [Minimize the total drawings stored for a chart](../5. Errors_And_Warnings/errors_re10139.md#minimize-the-total-drawings-stored-for-a-chart)
...
For example, if a script draws a line between two points, and then needs to update the line’s coordinates later, newcomers to Pine might opt to delete the existing line and create a new one with the updated coordinates. However, such an approach is _inefficient_ and leads to unnecessary resource consumption. The more efficient approach is to use the built-in _setter functions_ for the drawing type, such as [line.set\_x2()](../../reference manual/functions/line.set_x2.md), to modify the coordinates of the original drawing without creating a new object.

In general, to help minimize the resource usage of drawings, optimize them by using any of the following approaches:

- Eliminate unnecessary redrawing by assigning a drawing object’s ID to a variable declared with the [var](../../reference manual/keywords/var.md) keyword and modifying that object with the available setter functions.
- Remove unnecessary drawings with the `*.delete()` functions (e.g., [line.delete()](../../reference manual/functions/line.delete.md) and [label.delete()](../../reference manual/functions/label.delete.md)) to release resources.
- Reduce a script’s drawing limits by specifying arguments for the `max_lines_count`, `max_labels_count`, `max_boxes_count`, or `max_polylines_count` parameters of the [indicator()](../../reference manual/functions/indicator.md) or [strategy()](../../reference manual/functions/strategy.md) declaration statement.

#### [Filter dates in strategies](../5. Errors_And_Warnings/errors_re10139.md#filter-dates-in-strategies)

The total number of trades or orders simulated by [strategies](../1. Concepts/concepts_strategies.md) can impact memory consumption. When running strategy scripts that generate frequent orders on large datasets, reduce the number of unnecessary historical orders and trades that persist in memory by limiting the _starting point_ of the strategy.

To limit the starting point of a strategy, a simple and effective approach is to use a [conditional structure](../3. Language/language_conditional-structures.md) that activates the strategy’s [order placement commands](../1. Concepts/concepts_strategies.md#order-placement-and-cancellation) only when the bar’s opening or closing time comes after a specified date.

See the [How do I filter trades by a date or time range?](../6. FAQ/faq_strategies.md#how-do-i-filter-trades-by-a-date-or-time-range) portion of our [Strategies FAQ page](../6. FAQ/faq_strategies.md) for an example of this technique.

[Previous 
**CW10003**](../5. Errors_And_Warnings/errors_cw10003.md) [Next 
**RE10143**](../5. Errors_And_Warnings/errors_re10143.md)