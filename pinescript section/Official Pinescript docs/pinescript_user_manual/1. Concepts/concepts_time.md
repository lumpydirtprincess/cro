![](../1. Concepts/concepts_time.md)

# [Time](../1. Concepts/concepts_time.md#time)

## [Introduction](../1. Concepts/concepts_time.md#introduction)

In Pine Script®, the following key aspects apply when working with date and time values:

- **UNIX timestamp**: The native format for time values in Pine, representing the absolute number of _milliseconds_ elapsed since midnight [UTC](https://en.wikipedia.org/wiki/Coordinated_Universal_Time) on 1970-01-01. Several built-ins return UNIX timestamps directly, which users can [format](../1. Concepts/concepts_time.md#formatting-dates-and-times) into readable dates and times. See the [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) section below for more information.
- **Exchange time zone**: The [time zone](../1. Concepts/concepts_time.md#time-zones) of the instrument’s exchange. All [calendar-based variables](../1. Concepts/concepts_time.md#calendar-based-variables) hold values expressed in the exchange time zone, and all built-in function overloads that have a `timezone` parameter use this time zone by default.
- **Chart time zone**: The time zone the chart and [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) message prefixes use to express time values. Users can set the chart time zone using the “Timezone” input in the “Symbol” tab of the chart’s settings. This setting only changes the _display_ of dates and times on the chart and the times that prefix logged messages. It does **not** affect the behavior of Pine scripts because they cannot access a chart’s time zone information.
- **`timezone` parameter**: A “string” parameter of time-related functions that specifies the time zone used in their calculations. For [calendar-based functions](../1. Concepts/concepts_time.md#calendar-based-functions), such as [dayofweek()](../../reference manual/functions/dayofweek.md), the `timezone` parameter determines the time zone of the returned value. For functions that return UNIX timestamps, such as [time()](../../reference manual/functions/time.md), the specified `timezone` defines the time zone of other applicable parameters, e.g., `session`. See the [Time zone strings](../1. Concepts/concepts_time.md#time-zone-strings) section to learn more.

## [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps)

[UNIX time](https://en.wikipedia.org/wiki/Unix_time) is a standardized date and time representation that measures the number of _non-leap seconds_ elapsed since January 1, 1970 at 00:00:00
[UTC](https://en.wikipedia.org/wiki/Coordinated_Universal_Time) (the _UNIX Epoch_), typically expressed in seconds or smaller time units. A UNIX time value in Pine Script is an “int” _timestamp_ representing the number of _milliseconds_ from the UNIX Epoch to a specific point in time.
...
For example, the UNIX timestamp `1723472500000` always represents the time 1,723,472,500,000 milliseconds (1,723,472,500 seconds) after the UNIX Epoch. This timestamp’s meaning does **not** change relative to any [time zone](../1. Concepts/concepts_time.md#time-zones).

To _format_ an “int” UNIX timestamp into a readable date/time “string” expressed in a specific time zone, use the [str.format\_time()](../../reference manual/functions/str.format_time.md) function. The function does not _modify_ UNIX timestamps. It simply _represents_ timestamps in a desired human-readable format.
...
![image](../images/Time-Unix-timestamps-1.CTbXturQ_ZqXuwb.webp)
...
- The value enclosed within square brackets in the logged message is an _automatic_ prefix representing the historical time of the [log.info()](../../reference manual/functions/log.info.md) call in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format, expressed in the [chart time zone](../1. Concepts/concepts_time.md#time-zones).
- The script [concatenates](../1. Concepts/concepts_strings.md#concatenation) three [literal strings](../1. Concepts/concepts_strings.md#literal-strings) to create one long `formatString` argument for the [log.info()](../../reference manual/functions/log.info.md) call.

See the [Formatting dates and times](../1. Concepts/concepts_time.md#formatting-dates-and-times) section to learn more about representing UNIX timestamps with formatted strings.

## [Time zones](../1. Concepts/concepts_time.md#time-zones)
...
The local time within a time zone depends on its defined _offset_ from [Coordinated Universal Time (UTC)](https://en.wikipedia.org/wiki/Coordinated_Universal_Time), which can range from UTC-12:00
(12 hours _behind_ UTC) to UTC+14:00
(14 hours _ahead_ of UTC). Some regions maintain a consistent offset from UTC, and others have an offset that changes over time due to [daylight saving time (DST)](https://en.wikipedia.org/wiki/Daylight_saving_time) and other factors.

Two primary time zones apply to data feeds and TradingView charts: the _exchange time zone_ and the _chart time zone_.

The exchange time zone represents the time zone of the current symbol’s _exchange_, which Pine scripts can access with the [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md) variable. [Calendar-based variables](../1. Concepts/concepts_time.md#calendar-based-variables), such as [month](../../reference manual/variables/month.md), [dayofweek](../../reference manual/variables/dayofweek.md), and [hour](../../reference manual/variables/hour.md), always hold values expressed in the exchange time zone, and all time function overloads that have a `timezone` parameter use this time zone by default.

The chart time zone is a _visual preference_ that defines how the chart and the time prefixes of [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) represent time values. To set the chart time zone, use the “Timezone” input in the “Symbol” tab of the chart’s settings or click on the current time shown below the chart. The specified time zone does **not** affect time calculations in Pine scripts because they cannot access this chart information. Although scripts cannot access a chart’s time zone, programmers can provide [inputs](../1. Concepts/concepts_inputs.md) that users can adjust to match the time zone.

For example, the script below uses [str.format\_time()](../../reference manual/functions/str.format_time.md) to represent the last historical bar’s opening and closing [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) ( [time](../../reference manual/variables/time.md) and [time\_close](../../reference manual/variables/time_close.md) values) as date-time strings expressed in the function’s default time zone, the exchange time zone, UTC-0, and a user-specified time zone. It uses a [table](../../reference manual/types/table.md) to display all four representations in the bottom-right corner of the chart for comparison:

![image](../images/Time-Time-zones-1.BAv9QYBX_ZR0qM5.webp)
...
- The [label](../../reference manual/types/label.md) on the chart signifies which bar’s times the displayed strings represent.
- The “Default” and “Exchange” rows in the table show identical results because [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md) is the [str.format\_time()](../../reference manual/functions/str.format_time.md) function’s default `timezone` argument.
- The exchange time zone on our example chart appears as `"America/New_York"`, the [IANA identifier](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) for the NASDAQ exchange’s time zone. It represents UTC-4 _or_ UTC-5, depending on the time of year. See the [next section](../1. Concepts/concepts_time.md#time-zone-strings) to learn more about time zone strings.

### [Time zone strings](../1. Concepts/concepts_time.md#time-zone-strings)

All built-in functions with a `timezone` parameter accept a “string” argument specifying the [time zone](../1. Concepts/concepts_time.md#time-zones) they use in their calculations. These functions can accept time zone strings in either of the following formats:
...
For the [str.format\_time()](../../reference manual/functions/str.format_time.md) function and the functions that calculate calendar-based values from a [UNIX timestamp](../1. Concepts/concepts_time.md#unix-timestamps), including [month()](../../reference manual/functions/month.md), [dayofweek()](../../reference manual/functions/dayofweek.md), and [hour()](../../reference manual/functions/hour.md), the “string” passed to the `timezone` parameter changes the returned value’s calculation to express the result in the specified time zone. See the [Formatting dates and times](../1. Concepts/concepts_time.md#formatting-dates-and-times) and [Calendar-based functions](../1. Concepts/concepts_time.md#calendar-based-functions) sections for more information.

The example below shows how time zone strings affect the returned values of calendar-based functions. This script uses three [hour()](../../reference manual/functions/hour.md) function calls to calculate “int” values representing the opening hour of each bar in the exchange time zone, UTC-0, and a user-specified UTC offset. It plots all three calculated hours in a separate pane for comparison:

![image](../images/Time-Time-zones-Time-zone-strings-1.CWMAwPYS_Z27Sd89.webp)
...
- The call to the [hour()](../../reference manual/functions/hour.md) function without a specified `timezone` argument returns the same value that the [hour](../../reference manual/variables/hour.md) _variable_ holds because both represent the bar’s opening hour in the exchange time zone ( [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md)).

For functions that return [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) directly, such as [time()](../../reference manual/functions/time.md) and [timestamp()](../../reference manual/functions/timestamp.md), the `timezone` parameter defines the time zone of the function’s calendar-based _parameters_, including `session`, `year`, `month`, `day`, `hour`, `minute`, and `second`. The parameter does _not_ determine the time zone of the returned value, as UNIX timestamps are _time zone-agnostic_. See the [Testing for sessions](../1. Concepts/concepts_time.md#testing-for-sessions) and [\`timestamp()\`](../1. Concepts/concepts_time.md#timestamp) sections to learn more.

The following script calls the [timestamp()](../../reference manual/functions/timestamp.md) function to calculate the UNIX timestamp of a specific date and time, and it draws a [label](../../reference manual/types/label.md) at the timestamp’s corresponding bar location. The user-selected `timezone` argument (`timezoneInput`) determines the time zone of the call’s calendar-based arguments. Consequently, the calculated timestamp varies with the `timezoneInput` value because identical local times in various time zones correspond to _different_ amounts of time elapsed since the [UNIX Epoch](../1. Concepts/concepts_time.md#unix-timestamps):

![image](../images/Time-Time-zones-Time-zone-strings-2.Bgcl5BRA_2f5IXK.webp)
...
- The [label.new()](../../reference manual/functions/label.new.md) call uses [xloc.bar\_time](../../reference manual/constants/xloc.bar_time.md) as its `xloc` argument, which is required to anchor the drawing to an absolute time value. Without this argument, the function treats the `unixTimestamp` as a relative bar index, leading to an incorrect location.
- The label’s `y` value is the [close](../../reference manual/variables/close.md) of the bar where the [time](../../reference manual/variables/time.md) value crosses the `unixTimestamp` value. If the timestamp represents a future time, the label displays the last historical bar’s price.
...
The following script demonstrates how UTC and IANA time zone strings can affect time calculations differently. It uses two calls to the [hour()](../../reference manual/functions/hour.md) function to calculate the hour from the current bar’s opening timestamp using `"UTC-4"` and `"America/New_York"` as `timezone` arguments. The script plots the results of both calls for comparison and colors the main pane’s background when the returned values do not match. Although these two [hour()](../../reference manual/functions/hour.md) calls may seem similar because UTC-4 is an observed UTC offset in New York, they _do not_ always return the same results, as shown below:

![image](../images/Time-Time-zones-Time-zone-strings-3.dJwwxe9V_Zch1PP.webp)
...
The plots in the chart above diverge periodically because New York observes daylight saving time, meaning its UTC offset _changes_ at specific points in a year. During DST, New York’s local time follows UTC-4. Otherwise, it follows UTC-5. Because the script’s first [hour()](../../reference manual/functions/hour.md) call uses `"UTC-4"` as its `timezone` argument, it returns the correct hour in New York _only_ during DST. In contrast, the call that uses the `"America/New_York"` time zone string adjusts its UTC offset automatically to return the correct hour in New York at _any_ time of the year.

## [Time variables](../1. Concepts/concepts_time.md#time-variables)

Pine Script has several built-in variables that provide scripts access to different forms of time information:

- The [time](../../reference manual/variables/time.md) and [time\_close](../../reference manual/variables/time_close.md) variables hold [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) representing the current bar’s opening and closing times, respectively.
- The [time\_tradingday](../../reference manual/variables/time_tradingday.md) variable holds a UNIX timestamp representing the starting time of the last UTC calendar day in a session.
- The [timenow](../../reference manual/variables/timenow.md) variable holds a UNIX timestamp representing the current time when the script executes.
- The [year](../../reference manual/variables/year.md), [month](../../reference manual/variables/month.md), [weekofyear](../../reference manual/variables/weekofyear.md), [dayofmonth](../../reference manual/variables/dayofmonth.md), [dayofweek](../../reference manual/variables/dayofweek.md), [hour](../../reference manual/variables/hour.md), [minute](../../reference manual/variables/minute.md), and [second](../../reference manual/variables/second.md) variables reference calendar values based on the current bar’s opening time, expressed in the [exchange time zone](../1. Concepts/concepts_time.md#time-zones).
- The [last\_bar\_time](../../reference manual/variables/last_bar_time.md) variable holds a UNIX timestamp representing the last available bar’s opening time.
- The [chart.left\_visible\_bar\_time](../../reference manual/variables/chart.left_visible_bar_time.md) and [chart.right\_visible\_bar\_time](../../reference manual/variables/chart.right_visible_bar_time.md) variables hold UNIX timestamps representing the opening times of the leftmost and rightmost visible chart bars.
- The [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md) variable holds a “string” value representing the [time zone](../1. Concepts/concepts_time.md#time-zones) of the current symbol’s exchange in [IANA database notation](../1. Concepts/concepts_time.md#time-zone-strings). All time-related function overloads with a `timezone` parameter use this variable as the default argument.

### [​`time`​ and ​`time_close`​ variables](../1. Concepts/concepts_time.md#time-and-time_close-variables)

The [time](../../reference manual/variables/time.md) variable holds the [UNIX timestamp](../1. Concepts/concepts_time.md#unix-timestamps) of the current bar’s _opening time_, and the [time\_close](../../reference manual/variables/time_close.md) variable holds the UNIX timestamp of the bar’s _closing time_.

These timestamps are unique, time zone-agnostic “int” values, which programmers can use to anchor [drawing objects](../3. Language/language_type-system.md#drawing-types) to specific bar times, calculate and inspect bar time differences, construct readable date/time strings with the [str.format\_time()](../../reference manual/functions/str.format_time.md) function, and more.

The script below displays bar opening and closing times in different ways. On each bar, it [formats](../1. Concepts/concepts_time.md#formatting-dates-and-times) the [time](../../reference manual/variables/time.md) and [time\_close](../../reference manual/variables/time_close.md) timestamps into strings containing the hour, minute, and second in the [exchange time zone](../1. Concepts/concepts_time.md#time-zones), and it draws [labels](../2. Visuals/visuals_text-and-shapes.md#labels) displaying the formatted strings at the [open](../../reference manual/variables/open.md) and [close](../../reference manual/variables/close.md) prices. Additionally, the script displays strings containing the unformatted UNIX timestamps of the last chart bar within a [table](../../reference manual/types/table.md) in the bottom-right corner:

![image](../images/Time-Time-variables-Time-and-time-close-1.SQRlfJYt_1DGEfn.webp)
...
- This script’s [label.new()](../../reference manual/functions/label.new.md) calls include [xloc.bar\_time](../../reference manual/constants/xloc.bar_time.md) as the `xloc` argument and [time](../../reference manual/variables/time.md) as the `x` argument to anchor the drawings to bar opening times.
- The formatted strings express time in the exchange time zone because we did not specify `timezone` arguments in the [str.format\_time()](../../reference manual/functions/str.format_time.md) calls. NYSE, our chart symbol’s exchange, is in the “America/New\_York” time zone (UTC-4/-5).
- Although our example chart uses an _hourly_ timeframe, the [table](../../reference manual/types/table.md) and the [labels](../2. Visuals/visuals_text-and-shapes.md#labels) at the end of the chart show that the last bar closes only _30 minutes_ (1,800,000 milliseconds) after opening. This behavior occurs because the chart aligns bars with [session](../1. Concepts/concepts_sessions.md) opening and closing times. A session’s final bar closes when the session ends, and a new bar opens when a new session starts. A typical session on our 60-minute chart with regular trading hours (RTH) spans from 09:30
to 16:00
(6.5 hours). The chart divides this interval into as many 60-minute bars as possible, starting from the session’s opening time, which leaves only 30 minutes for the final bar to cover.

It’s crucial to note that unlike the [time](../../reference manual/variables/time.md) variable, which has consistent behavior across chart types, [time\_close](../../reference manual/variables/time_close.md) behaves differently on _time-based_ and _non-time-based_ charts.

Time-based charts have bars that typically open and close at regular, _predictable_ times within a session. Thanks to this predictability, [time\_close](../../reference manual/variables/time_close.md) can accurately represent the _expected_ closing time of an open bar on a time-based chart, as shown on the last bar in the example above.

In contrast, the bars on [tick charts](https://www.tradingview.com/support/solutions/43000709225/) and _price-based_ charts (all [non-standard charts](../1. Concepts/concepts_non-standard-charts-data.md) excluding [Heikin Ashi](https://www.tradingview.com/support/solutions/43000619436/)) cover _irregular_ time intervals. Tick charts construct bars based on successive ticks in the data feed, and price-based charts construct bars based on significant price movements. The time it takes for new ticks or price changes to occur is _unpredictable_. As such, the [time\_close](../../reference manual/variables/time_close.md) value is [na](../../reference manual/variables/na.md) on the _open realtime bars_ of these charts.

The following script uses the [time](../../reference manual/variables/time.md) and [time\_close](../../reference manual/variables/time_close.md) variables with [str.tostring()](../../reference manual/functions/str.tostring.md) and [str.format\_time()](../../reference manual/functions/str.format_time.md) to create strings containing bar opening and closing [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) and [formatted](../1. Concepts/concepts_time.md#formatting-dates-and-times) date-time representations, which it displays in [labels](../2. Visuals/visuals_text-and-shapes.md#labels) at each bar’s [high](../../reference manual/variables/high.md) and [low](../../reference manual/variables/low.md) prices.

When applied to a [Renko](https://www.tradingview.com/support/solutions/43000502284/) chart, which forms new bars based on _price movements_, the labels show correct results on all historical bars. However, the last bar has a [time\_close](../../reference manual/variables/time_close.md) value of [na](../../reference manual/variables/na.md) because the future closing time is unpredictable. Consequently, the bar’s closing time label shows a timestamp of `"NaN"` and an _incorrect_ date and time:

![image](../images/Time-Time-variables-Time-and-time-close-2.DgbsHPgj_Z2oyTKY.webp)
...
- The script draws up to 50 [labels](../2. Visuals/visuals_text-and-shapes.md#labels) because we did not specify a `max_labels_count` argument in the [indicator()](../../reference manual/functions/indicator.md) declaration statement.
- The [str.format\_time()](../../reference manual/functions/str.format_time.md) function replaces [na](../../reference manual/variables/na.md) values with 0 in its calculations, which is why it returns an incorrect date-time “string” on the last bar. A timestamp of 0 corresponds to the _UNIX Epoch_ (00:00:00
UTC on January 1, 1970). However, the [str.format\_time()](../../reference manual/functions/str.format_time.md) call does not specify a `timezone` argument, so it expresses the epoch’s date and time in the _exchange time zone_, which was five hours behind UTC at that point in time.
- The [time\_close()](../../reference manual/functions/time_close.md) function, which returns the closing timestamp of a bar on a specified timeframe within a given session, also returns [na](../../reference manual/variables/na.md) on the open realtime bars of tick-based and price-based charts.

Scripts can retrieve a realtime bar’s closing time on tick charts and price-based charts once the bar is _confirmed_. The closing timestamp of an _elapsed realtime bar_ is committed to the realtime data feed as soon as the bar closes, so its [time\_close](../../reference manual/variables/time_close.md) value is no longer [na](../../reference manual/variables/na.md).
...
![image](../images/Time-Time-variables-Time-and-time-close-3.n-klv93m_ZY0mII.webp)
...
- A confirmed realtime bar is **not** the same as a historical bar. Pine’s [execution model](../3. Language/language_execution-model.md) uses separate data feeds for realtime and historical data. The closing time of a confirmed realtime bar is committed to the _realtime_ feed, until the script re-executes on the chart. Only then will this bar’s closing time load _historically_ along with all the other closed bars.
- The [barstate.islast](../../reference manual/variables/barstate.islast.md) value is `true` for all realtime bars in the dataset. Therefore, the elapsed realtime bar and the latest realtime bar both display a purple [label](../1. Concepts/concepts_text-and-shapes.md#labels) and highlighted [background](../1. Concepts/concepts_backgrounds.md). See the [Bar states](../1. Concepts/concepts_bar-states.md) page to learn more about the different `barstate.*` variables in Pine Script.
- The [time\_close()](../../reference manual/functions/time_close.md) function can similarly retrieve the previous bar’s closing time on price-based charts using a `bar_back = 1` argument.

### [​`time_tradingday`​](../1. Concepts/concepts_time.md#time_tradingday)

The [time\_tradingday](../../reference manual/variables/time_tradingday.md) variable holds a [UNIX timestamp](../1. Concepts/concepts_time.md#unix-timestamps) representing the starting time (00:00
UTC) of the last trading day in the current bar’s final session. It is helpful primarily for date and time calculations on _time-based_ charts for symbols with overnight sessions that start and end on _different_ calendar days.

On “1D” and lower timeframes, the [time\_tradingday](../../reference manual/variables/time_tradingday.md) timestamp corresponds to the beginning of the day when the current session _ends_, even for bars that open and close on the previous day. For example, the “Monday” session for “EURUSD” starts on Sunday at 17:00
and ends on Monday at 17:00
in the [exchange time zone](../1. Concepts/concepts_time.md#time-zones). The [time\_tradingday](../../reference manual/variables/time_tradingday.md) values of _all_ intraday bars within the session represent Monday at 00:00
UTC.

On timeframes higher than “1D”, which can cover _multiple_ sessions, [time\_tradingday](../../reference manual/variables/time_tradingday.md) holds the timestamp representing the beginning of the last calendar day of the bar’s _final_ trading session. For example, on a “EURUSD, 1W” chart, the timestamp represents the start of the last trading day in the week, which is typically Friday at 00:00
UTC.

The script below demonstrates how the [time\_tradingday](../../reference manual/variables/time_tradingday.md) and [time](../../reference manual/variables/time.md) variables differ on Forex symbols. On each bar, it draws [labels](../2. Visuals/visuals_text-and-shapes.md#labels) to display strings containing the variables’ [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) and [formatted dates and times](../1. Concepts/concepts_time.md#formatting-dates-and-times). It also uses the [dayofmonth()](../../reference manual/functions/dayofmonth.md) function to calculate the UTC calendar day from both timestamps, highlighting the background when the calculated days do not match.

When applied to the “FXCM:EURUSD” chart with the “3h” (“180”) timeframe, the script highlights the background of the _first bar_ in each session, as each session opens on the _previous_ calendar day. The [dayofmonth()](../../reference manual/functions/dayofmonth.md) call that uses [time](../../reference manual/variables/time.md) calculates the opening day on the session’s first bar, whereas the call that uses [time\_tradingday](../../reference manual/variables/time_tradingday.md) calculates the day when the session _ends_:

![image](../images/Time-Time-variables-Time-tradingday-1.oi6pPbd1_ZMMY0B.webp)
...
- The [str.format\_time()](../../reference manual/functions/str.format_time.md) and [dayofmonth()](../../reference manual/functions/dayofmonth.md) calls use `"UTC+0"` as the `timezone` argument, meaning the results represent calendar time values with no offset from UTC. In the screenshot, the first bar opens at 21:00
UTC, 17:00
in the exchange time zone (“America/New\_York”).
- The formatted strings show `"GMT"` as the acronym of the time zone, which is equivalent to `"UTC+0"` in this context.
- The [time\_tradingday](../../reference manual/variables/time_tradingday.md) value is the same for _all_ three-hour bars within each session, even for the initial bar that opens on the previous UTC calendar day. The assigned timestamp changes only when a new session starts.

### [​`timenow`​](../1. Concepts/concepts_time.md#timenow)

The [timenow](../../reference manual/variables/timenow.md) variable holds a [UNIX timestamp](../1. Concepts/concepts_time.md#unix-timestamps) representing the script’s _current time_. Unlike the values of other variables that hold UNIX timestamps, the values in the [timenow](../../reference manual/variables/timenow.md) series correspond to times when the script _executes_, not the times of specific bars or trading days.

A Pine script executes only _once_ per historical bar, and all historical executions occur when the script first _loads_ on the chart. As such, the [timenow](../../reference manual/variables/timenow.md) value is relatively consistent on historical bars, with only occasional millisecond changes across the series. In contrast, on realtime bars, a script executes once for _each new update_ in the data feed, which can happen several times per bar. With each new execution, the [timenow](../../reference manual/variables/timenow.md) value updates on the latest bar to represent the current time.
...
The script below inspects the value of [timenow](../../reference manual/variables/timenow.md) on the latest chart bars and uses it to analyze realtime bar updates. When the script first reaches the last chart bar, it declares three variables with the [varip](../../reference manual/keywords/varip.md) keyword to hold the latest [timenow](../../reference manual/variables/timenow.md) value, the total time elapsed between the bar’s updates, and the total number of updates. It uses these values to calculate the average number of milliseconds between updates, which it displays in a [label](../../reference manual/types/label.md) along with the current execution’s timestamp, a [formatted](../1. Concepts/concepts_time.md#formatting-dates-and-times) time and date in the [exchange time zone](../1. Concepts/concepts_time.md#time-zones), and the current number of bar updates:

![image](../images/Time-Time-variables-Timenow-1.OYWKIkvY_29NT7r.webp)
...
- Although we’ve set the chart [time zone](../1. Concepts/concepts_time.md#time-zones) to match the exchange time zone, the formatted time in the open bar’s label and the time shown below the chart _do not_ always align. The script records a new timestamp only when a _new execution_ occurs, whereas the time below the chart updates _continuously_.
- The [varip](../../reference manual/keywords/varip.md) keyword specifies that a variable does not revert to the last committed value in its series when new updates occur. This behavior allows the script to use variables to track changes in [timenow](../../reference manual/variables/timenow.md) on an open bar.
- Updates to [timenow](../../reference manual/variables/timenow.md) on open realtime bars do not affect the recorded timestamps on confirmed bars as the script executes. However, the historical series changes ( _repaints_) after reloading the chart because [timenow](../../reference manual/variables/timenow.md) references the script’s _current time_, not the times of specific bars.

### [Calendar-based variables](../1. Concepts/concepts_time.md#calendar-based-variables)

The [year](../../reference manual/variables/year.md), [month](../../reference manual/variables/month.md), [weekofyear](../../reference manual/variables/weekofyear.md), [dayofmonth](../../reference manual/variables/dayofmonth.md), [dayofweek](../../reference manual/variables/dayofweek.md), [hour](../../reference manual/variables/hour.md), [minute](../../reference manual/variables/minute.md), and [second](../../reference manual/variables/second.md) variables hold _calendar-based_ “int” values calculated from the current bar’s _opening time_, expressed in the [exchange time zone](../1. Concepts/concepts_time.md#time-zones). These variables reference the same values that [calendar-based functions](../1. Concepts/concepts_time.md#calendar-based-functions) return when they use the default `timezone` argument and [time](../../reference manual/variables/time.md) as the `time` argument. For instance, the [year](../../reference manual/variables/year.md) variable holds the same value that a `year(time)` call returns.

Programmers can use these calendar-based variables for several purposes, such as:

- Identifying a bar’s opening date and time.
- Passing the variables to the [timestamp()](../../reference manual/functions/timestamp.md) function to calculate [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps).
- Testing when date/time values or ranges occur in a data feed.

One of the most common use cases for these variables is checking for date or time ranges to control when a script displays visuals or executes calculations. This simple example inspects the [year](../../reference manual/variables/year.md) variable to determine when to plot a visible value. If the [year](../../reference manual/variables/year.md) is 2022 or higher, the script plots the bar’s [close](../../reference manual/variables/close.md). Otherwise, it plots [na](../../reference manual/variables/na.md):

![image](../images/Time-Time-variables-Calendar-based-variables-1.kYa8Z75h_16GMJY.webp)
...
For instance, suppose we want to detect when the first calendar day of each month occurs on the chart. Intuitively, one might consider simply checking when the [dayofmonth](../../reference manual/variables/dayofmonth.md) value equals 1. However, this condition only identifies when a bar _opens_ on a month’s first day. The bars on some charts can open and close in _different_ months. Additionally, a chart bar might not contain the first day of a month if the market is _closed_ on that day. Therefore, we must create extra conditions that work in these scenarios to identify the first day in _any_ month on the chart.

The script below uses the [dayofmonth](../../reference manual/variables/dayofmonth.md) and [month](../../reference manual/variables/month.md) variables, and the [month()](../../reference manual/functions/month.md) function, to create three conditions that detect the first day of the month in different ways. The first condition detects if the bar opens on the first day, the second checks if the bar opens in one month and closes in another, and the third checks if the chart skips the date entirely. The script draws [labels](../2. Visuals/visuals_text-and-shapes.md#labels) showing bar opening dates and highlights the background with different colors to visualize when each condition occurs:

![image](../images/Time-Time-variables-Calendar-based-variables-2.xQSXmhyL_Z1UJPmf.webp)
...
- The script calls the [month()](../../reference manual/functions/month.md) function with [time\_close](../../reference manual/variables/time_close.md) as the `time` argument to calculate each bar’s _closing_ month for the `containsFirst` condition.
- The `dayofweek.*` namespace contains variables that hold each possible [dayofweek](../../reference manual/variables/dayofweek.md) value, e.g., [dayofweek.sunday](../../reference manual/constants/dayofweek.sunday.md) has a constant value of 1 and [dayofweek.saturday](../../reference manual/constants/dayofweek.saturday.md) has a constant value of 7. The script compares [dayofweek](../../reference manual/variables/dayofweek.md) to these variables in a [switch](../../reference manual/keywords/switch.md) structure to determine the weekday name shown inside each [label](../../reference manual/types/label.md).
- To detect the _first opening time_ in a monthly timeframe, not strictly the first day in a calendar month, use `ta.change(time("1M")) > 0` or `timeframe.change("1M")` instead of conditions based on these variables. See the [Testing for changes in higher timeframes](../1. Concepts/concepts_time.md#testing-for-changes-in-higher-timeframes) section to learn more.

### [​`last_bar_time`​](../1. Concepts/concepts_time.md#last_bar_time)

The [last\_bar\_time](../../reference manual/variables/last_bar_time.md) variable holds a [UNIX timestamp](../1. Concepts/concepts_time.md#unix-timestamps) representing the _last_ available bar’s opening time. It is similar to [last\_bar\_index](../../reference manual/variables/last_bar_index.md), which references the latest bar index. On historical bars, [last\_bar\_time](../../reference manual/variables/last_bar_time.md) consistently references the [time](../../reference manual/variables/time.md) value of the last bar available when the script first _loads_ on the chart. The only time the variable’s value updates across script executions is when a new realtime bar opens.

The following script uses the [last\_bar\_time](../../reference manual/variables/last_bar_time.md) variable to get the opening timestamp of the last chart bar during its execution on the _first bar_. It displays the UNIX timestamp and a [formatted date and time](../1. Concepts/concepts_time.md#formatting-dates-and-times) using a single-cell [table](../../reference manual/types/table.md) created only on that bar. When the script executes on the last available bar, it creates a [label](../../reference manual/types/label.md) to show the bar’s [time](../../reference manual/variables/time.md) value and its formatted representation for visual comparison.

As the chart below shows, both drawings display _identical_ times, verifying that [last\_bar\_time](../../reference manual/variables/last_bar_time.md) correctly references the last bar’s [time](../../reference manual/variables/time.md) value on previous historical bars:

![image](../images/Time-Time-variables-Last-bar-time-1.COWu3Po-_ZkKs6o.webp)
...
- The script creates the [label](../../reference manual/types/label.md) only on the _first_ bar with the [barstate.islast](../../reference manual/variables/barstate.islast.md) state because that bar’s [time](../../reference manual/variables/time.md) value is what [last\_bar\_time](../../reference manual/variables/last_bar_time.md) equals on all historical bars. On subsequent bars, the [last\_bar\_time](../../reference manual/variables/last_bar_time.md) value _updates_ to represent the latest realtime bar’s opening time.
- Updates to [last\_bar\_time](../../reference manual/variables/last_bar_time.md) on realtime bars do not affect the values on historical bars as the script executes. However, the variable’s series _repaints_ when the script restarts because [last\_bar\_time](../../reference manual/variables/last_bar_time.md) always references the latest available bar’s opening time.
- This script expresses dates using the `"dd/MM/yy"` format, meaning the two-digit day appears before the two-digit month, and the month appears before the two-digit representation of the year. See [this section](../1. Concepts/concepts_time.md#formatting-dates-and-times) below for more information.

### [Visible bar times](../1. Concepts/concepts_time.md#visible-bar-times)

The [chart.left\_visible\_bar\_time](../../reference manual/variables/chart.left_visible_bar_time.md) and [chart.right\_visible\_bar\_time](../../reference manual/variables/chart.right_visible_bar_time.md) variables reference the opening [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) of the chart’s leftmost (first) and rightmost (last) _visible bars_ on every script execution. When a script uses these variables, it responds dynamically to visible chart changes, such as users scrolling across bars or zooming in/out. Each time the visible window changes, the script _re-executes_ automatically to update the variables’ values on all available bars.

The example below demonstrates how the [chart.left\_visible\_bar\_time](../../reference manual/variables/chart.left_visible_bar_time.md) and [chart.right\_visible\_bar\_time](../../reference manual/variables/chart.right_visible_bar_time.md) variables work across script executions. The script draws [labels](../2. Visuals/visuals_text-and-shapes.md#labels) anchored to the visible bars’ times to display the UNIX timestamps. In addition, it draws two single-cell [tables](../2. Visuals/visuals_tables.md) showing corresponding dates and times in the standard [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format. The script creates these drawings only when it executes on the first bar. As the script continues to execute on subsequent bars, it identifies each bar whose [time](../../reference manual/variables/time.md) value equals either visible bars’ timestamp and colors it on the chart:

![image](../images/Time-Time-variables-Visible-bar-times-1.Cm54dJVW_Z1LAsmQ.webp)
...
- The [chart.left\_visible\_bar\_time](../../reference manual/variables/chart.left_visible_bar_time.md) and [chart.right\_visible\_bar\_time](../../reference manual/variables/chart.right_visible_bar_time.md) values are consistent across all executions, which allows the script to identify the visible bars’ timestamps on the _first_ bar and check when the [time](../../reference manual/variables/time.md) value equals them. The script restarts on any chart window changes, updating the variables’ series to reference the new timestamps on every bar.
- The [str.format\_time()](../../reference manual/functions/str.format_time.md) function uses ISO 8601 format by default when the call does not include a `format` argument because it is the _international standard_ for expressing dates and times. See the [Formatting dates and times](../1. Concepts/concepts_time.md#formatting-dates-and-times) section to learn more about time string formats.

### [​`syminfo.timezone`​](../1. Concepts/concepts_time.md#syminfotimezone)

The [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md) variable holds a [time zone string](../1. Concepts/concepts_time.md#time-zone-strings) representing the current symbol’s _exchange_ time zone. The “string” value expresses the time zone as an _IANA identifier_ (e.g., `"America/New_York"`). The overloads of [time functions](../1. Concepts/concepts_time.md#time-functions) that include a `timezone` parameter use [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md) as the default argument.
...
- Displaying the “string” in [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) or [drawings](../3. Language/language_type-system.md#drawing-types) to inspect the exchange time zone’s IANA identifier.
...
The following script uses the [timenow](../../reference manual/variables/timenow.md) variable to retrieve the [UNIX timestamp](../1. Concepts/concepts_time.md#unix-timestamps) of its latest execution. It [formats](../1. Concepts/concepts_time.md#formatting-dates-and-times) the timestamp into date-time strings expressed in the main symbol’s exchange time zone and a requested symbol’s exchange time zone, which it displays along with the IANA identifiers in a [table](../../reference manual/types/table.md) on the last chart bar:

![image](../images/Time-Time-variables-Syminfo-timezone-1.BAiDPB0L_ZWthXd.webp)
...
- Pine scripts execute on realtime bars only when new updates occur in the data feed, and [timenow](../../reference manual/variables/timenow.md) updates only on script executions. As such, when no realtime updates are available, the [timenow](../../reference manual/variables/timenow.md) timestamp does not change. See [this section](../1. Concepts/concepts_time.md#timenow) above for more information.
- The default `symbolInput` value is `"NSE:BANKNIFTY"`. NSE is in the “Asia/Kolkata” [time zone](../1. Concepts/concepts_time.md#time-zones), which is 9.5 hours ahead of the main symbol’s exchange time zone (“America/New\_York”) at the time of the screenshot. Although the _local_ time representations differ, both refer to the same _absolute_ time that the [timenow](../../reference manual/variables/timenow.md) timestamp represents.
- Pine v6 scripts use dynamic `request.*()` calls by default, which allows the script to call [request.security()](../../reference manual/functions/request.security.md) dynamically inside the [if](../../reference manual/keywords/if.md) structure’s _local scope_. See the [Dynamic requests](../1. Concepts/concepts_other-timeframes-and-data.md#dynamic-requests) section of the [Other timeframes and data](../1. Concepts/concepts_other-timeframes-and-data.md) page to learn more.

## [Time functions](../1. Concepts/concepts_time.md#time-functions)

Pine Script features several built-in functions that scripts can use to retrieve, calculate, and express time values:

- The [time()](../../reference manual/functions/time.md) and [time\_close()](../../reference manual/functions/time_close.md) functions allow scripts to retrieve [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) for the opening and closing times of bars within a session on a specified timeframe, without requiring `request.*()` function calls.
- The [year()](../../reference manual/functions/year.md), [month()](../../reference manual/functions/month.md), [weekofyear()](../../reference manual/functions/weekofyear.md), [dayofmonth()](../../reference manual/functions/dayofmonth.md), [dayofweek()](../../reference manual/functions/dayofweek.md), [hour()](../../reference manual/functions/hour.md), [minute()](../../reference manual/functions/minute.md), and [second()](../../reference manual/functions/second.md) functions calculate calendar-based values, expressed in a specified [time zone](../1. Concepts/concepts_time.md#time-zones), from a UNIX timestamp.
- The [timestamp()](../../reference manual/functions/timestamp.md) function calculates a UNIX timestamp from a specified calendar date and time.
- The [str.format\_time()](../../reference manual/functions/str.format_time.md) function formats a UNIX timestamp into a human-readable date/time “string”, expressed in a specified time zone. The [Formatting dates and times](../1. Concepts/concepts_time.md#formatting-dates-and-times) section below provides detailed information about formatting timestamps with this function.
- The [input.time()](../../reference manual/functions/input.time.md) function returns a UNIX timestamp corresponding to the user-specified date and time, and the [input.session()](../../reference manual/functions/input.session.md) function returns a valid [time-based session string](../1. Concepts/concepts_sessions.md#time-based-sessions) corresponding to the user-specified start and end times. See the [Time input](../1. Concepts/concepts_inputs.md#time-input) and [Session input](../1. Concepts/concepts_inputs.md#session-input) sections of the [Inputs](../1. Concepts/concepts_inputs.md) page to learn more about these functions.

### [​`time()`​ and ​`time_close()`​ functions](../1. Concepts/concepts_time.md#time-and-time_close-functions)

The [time()](../../reference manual/functions/time.md) and [time\_close()](../../reference manual/functions/time_close.md) functions return [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) representing the opening and closing times of bars on a specified timeframe. Both functions can _filter_ their returned values based on a given [session](../1. Concepts/concepts_sessions.md) in a specific [time zone](../1. Concepts/concepts_time.md#time-zones). They each have the following signatures:
...
- The `timeframe` parameter accepts a [timeframe string](../1. Concepts/concepts_timeframes.md#timeframe-string-specifications) that defines the timeframe for the calculation. The function uses the script’s main timeframe if the argument is [timeframe.period](../../reference manual/variables/timeframe.period.md) or an empty string `""`.
- The optional `session` parameter accepts a [time-based session string](../1. Concepts/concepts_sessions.md#time-based-sessions) that defines the session’s start and end times (e.g., `"0930-1600"`) and the days for which it applies (e.g., `":23456"` means Monday - Friday). If the value does not specify the days, the session applies to _all_ trading days automatically. The function returns UNIX timestamps only for the bars _within_ the session. It returns [na](../../reference manual/variables/na.md) if a bar’s time is _outside_ the session. If the `session` argument is an empty string or not specified, the function uses the symbol’s session information.
- The optional `timezone` parameter accepts a valid [time zone string](../1. Concepts/concepts_time.md#time-zone-strings) that defines the time zone of the specified `session`. It does **not** change the meaning of returned UNIX timestamps, as they are _time zone-agnostic_. If the `timezone` argument is not specified, the function uses the exchange time zone ( [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md)).
- The optional `bars_back` parameter accepts an “int” value specifying the bar offset on the script’s main timeframe. If the value is positive, the function finds the bar that is N bars before the current bar on the main timeframe, then retrieves the timestamp of the corresponding bar on the timeframe specified by the `timeframe` argument. If the value is a negative number from -1 to -500, the function calculates the expected timestamp of the `timeframe` bar corresponding to N bars after the current bar on the main timeframe. The default is 0. See the [Calculating timestamps at bar offsets](../1. Concepts/concepts_time.md#calculating-timestamps-at-bar-offsets) section to learn more about this parameter.
- The optional `timeframe_bars_back` parameter accepts an “int” value specifying the additional bar offset on the timeframe specified by the `timeframe` argument. If the value is positive, the function retrieves the timestamp of the `timeframe` bar that is N `timeframe` bars before the one corresponding to the `bars_back` offset. If the value is a negative number from -1 to -500, the function calculates the expected timestamp of the `timeframe` bar that is N `timeframe` bars after the one corresponding to the `bars_back` offset. The default is 0. See the [Calculating timestamps at bar offsets](../1. Concepts/concepts_time.md#calculating-timestamps-at-bar-offsets) section to learn more about this parameter.

Similar to the [time](../../reference manual/variables/time.md) and [time\_close](../../reference manual/variables/time_close.md) variables, these functions behave differently on _time-based_ and _non-time-based_ charts.

Time-based charts have bars that open and close at _predictable_ times, whereas the bars on [tick charts](https://www.tradingview.com/support/solutions/43000709225/) and all [non-standard charts](../1. Concepts/concepts_non-standard-charts-data.md), excluding [Heikin Ashi](https://www.tradingview.com/support/solutions/43000619436/), open and close at irregular, _unpredictable_ times. Consequently, [time\_close()](../../reference manual/functions/time_close.md) cannot calculate the expected closing time of an open realtime bar on non-time-based charts, so it returns [na](../../reference manual/variables/na.md) on that bar. Similarly, the [time()](../../reference manual/functions/time.md) function with a negative bar offset cannot accurately calculate the expected opening time of a future realtime bar on these charts. See the _second example_ in [this section](../1. Concepts/concepts_time.md#time-and-time_close-variables) above. That example script exhibits the same behavior on a price-based chart if it uses a `time_close("")` call instead of the [time\_close](../../reference manual/variables/time_close.md) variable.

On non-time-based charts, a new bar’s closing time is available in the realtime data feed once the bar closes. Therefore, [time\_close()](../../reference manual/functions/time_close.md) can retrieve valid closing timestamps for _confirmed realtime bars_ without needing to restart the script to load them historically. See the _third example_ in [this section](../1. Concepts/concepts_time.md#time-and-time_close-variables) above. Replacing the `time_close[1]` variable in the example script with a `time_close("", 1)` call achieves the same result to retrieve the closing time of the elapsed realtime bar.

Typical use cases for the [time()](../../reference manual/functions/time.md) and [time\_close()](../../reference manual/functions/time_close.md) functions include:

- Testing for bars that open or close in [specific sessions](../1. Concepts/concepts_time.md#testing-for-sessions) defined by the `session` and `timezone` parameters.
- [Testing for changes](../1. Concepts/concepts_time.md#testing-for-changes-in-higher-timeframes) or measuring time differences on specified higher timeframes.
- [Calculating timestamps at bar offsets](../1. Concepts/concepts_time.md#calculating-timestamps-at-bar-offsets) on the script’s main timeframe, a specified timeframe, or both.

#### [Testing for sessions](../1. Concepts/concepts_time.md#testing-for-sessions)

The [time()](../../reference manual/functions/time.md) and [time\_close()](../../reference manual/functions/time_close.md) functions’ `session` and `timezone` parameters define the [sessions](../1. Concepts/concepts_sessions.md) for which they can return _non-na_ values. If a call to either function references a bar that opens/closes within the defined session in a given [time zone](../1. Concepts/concepts_time.md#time-zones), it returns a [UNIX timestamp](../1. Concepts/concepts_time.md#unix-timestamps) for that bar. Otherwise, it returns [na](../../reference manual/variables/na.md). Programmers can pass the returned values to the [na()](../../reference manual/functions/na.md) function to identify which bars open or close within specified intervals, which is helpful for session-based calculations and logic.

This simple script identifies when a bar on the chart’s timeframe opens at or after 11:00
and before 13:00
in the exchange time zone on any trading day. It calls [time()](../../reference manual/functions/time.md) with [timeframe.period](../../reference manual/variables/timeframe.period.md) as the `timeframe` argument and the `"1100-1300"` [session string](../1. Concepts/concepts_sessions.md#time-based-sessions) as the `session` argument, and then verifies whether the returned value is [na](../../reference manual/variables/na.md) with the [na()](../../reference manual/functions/na.md) function. When the value is **not** [na](../../reference manual/variables/na.md), the script highlights the chart’s background to indicate that the bar opened in the session:

![image](../images/Time-Time-functions-Time-and-time-close-Testing-for-sessions-1.CJeOvGxZ_Z1vRBpv.webp)
...
- The `session` argument in the [time()](../../reference manual/functions/time.md) call represents an interval in the _exchange_ time zone because [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md) is the default `timezone` argument.
- The session string expresses the start and end times in the `"HHmm-HHmm"` format, where `"HH"` is the two-digit _hour_ and `"mm"` is the two-digit _minute_. Session strings can also specify the _weekdays_ a session applies to. However, the [time()](../../reference manual/functions/time.md) call’s `session` argument (`"1100-1300"`) does not include this information, which is why it considers the session valid for _every_ day. See the [Sessions](../1. Concepts/concepts_sessions.md) page to learn more.

When using session strings in [time()](../../reference manual/functions/time.md) and [time\_close()](../../reference manual/functions/time_close.md) calls, it’s crucial to understand that such strings define start and end times in a _specific_ [time zone](../1. Concepts/concepts_time.md#time-zones). The local hour and minute in one region may not correspond to the same point in UNIX time as that same hour and minute in another region. Therefore, calls to these functions with different `timezone` arguments can return non-na timestamps at _different_ times, as the specified [time zone string](../1. Concepts/concepts_time.md#time-zone-strings) changes the meaning of the local times represented in the `session` argument.

This example demonstrates how the `timezone` parameter affects the `session` parameter in a [time()](../../reference manual/functions/time.md) function call. The script calculates an `opensInSession` condition that uses a [time()](../../reference manual/functions/time.md) call with arguments based on [inputs](../1. Concepts/concepts_inputs.md). The [session input](../1. Concepts/concepts_inputs.md#session-input) for the `session` argument includes four preset options: `"0000-0400"`, `"0930-1400"`, `"1300-1700"`, and `"1700-2100"`. The [string input](../1. Concepts/concepts_inputs.md#string-input) that defines the `timezone` argument includes four IANA time zone options representing different offsets from UTC: `"America/Vancouver"` (UTC-7/-8), `"America/New_York"` (UTC-4/-5), `"Asia/Dubai"` (UTC+4), and `"Austrailia/Sydney"` (UTC+10/+11).
...
![image](../images/Time-Time-functions-Time-and-time-close-Testing-for-sessions-2.DKRpvO46_ZiKVjh.webp)
...
- This script uses _IANA notation_ for all [time zone strings](../1. Concepts/concepts_time.md#time-zone-strings) because it is the recommended format. Using an IANA identifier allows the [time()](../../reference manual/functions/time.md) call to automatically adjust the session’s UTC offset based on a region’s local time policies, such as daylight saving time.

#### [Testing for changes in higher timeframes](../1. Concepts/concepts_time.md#testing-for-changes-in-higher-timeframes)

The `timeframe` parameter of the [time()](../../reference manual/functions/time.md) and [time\_close()](../../reference manual/functions/time_close.md) functions specifies the timeframe of the bars in the calculation, allowing scripts to retrieve opening/closing [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) from _higher timeframes_ than the current chart’s timeframe without requiring `request.*()` function calls.

Programmers can use the opening/closing timestamps from higher-timeframe (HTF) bars to detect timeframe changes. One common approach is to call [time()](../../reference manual/functions/time.md) or [time\_close()](../../reference manual/functions/time_close.md) with a consistent `timeframe` argument across all executions on a time-based chart and measure the one-bar change in the returned value with the [ta.change()](../../reference manual/functions/ta.change.md) function. The result is a _nonzero_ value only when an HTF bar opens. One can also check whether the data has a time gap at that point by comparing the [time()](../../reference manual/functions/time.md) value to the previous bar’s [time\_close()](../../reference manual/functions/time_close.md) value. A gap is present when the opening timestamp on the current bar is greater than the closing timestamp on the previous bar.
...
The script draws [labels](../2. Visuals/visuals_text-and-shapes.md#labels) containing [formatted](../1. Concepts/concepts_time.md#formatting-dates-and-times) “1M” opening times to indicate the chart bars that mark the start of monthly bars. If a monthly bar opens without a gap from the previous closing time, the script draws a blue label. If a monthly bar starts after a gap, it draws a red label. Additionally, if the “1M” opening time does not match the opening time of the chart bar, the script displays that bar’s formatted time in the label for comparison:

![image](../images/Time-Time-functions-Time-and-time-close-Testing-for-changes-in-higher-timeframes-1.EwKXV91R_1tjSNd.webp)
...
- Using [ta.change()](../../reference manual/functions/ta.change.md) on a [time()](../../reference manual/functions/time.md) or [time\_close()](../../reference manual/functions/time_close.md) call’s result is not the _only_ way to detect changes in a higher timeframe. The [timeframe.change()](../../reference manual/functions/timeframe.change.md) function is an equivalent, more convenient option for scripts that do not need to _use_ the UNIX timestamps from HTF bars in other calculations, as it returns a “bool” value directly without extra code.
...
#### [Calculating timestamps at bar offsets](../1. Concepts/concepts_time.md#calculating-timestamps-at-bar-offsets)

The `bars_back` and `timeframe_bars_back` parameters of the [time()](../../reference manual/functions/time.md) and [time\_close()](../../reference manual/functions/time_close.md) functions control _bar offsets_ in the calculations, enabling the functions to compute the opening/closing [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) for bars that are _before_ or _after_ the current bar, on a given timeframe, without requiring `request.*()` calls or history-referencing operations.
...
The following example shows how the `bars_back` and `timeframe_bars_back` parameters work together in timestamp calculations. The script below creates three drawings on the last historical bar, anchored to the results from different [time()](../../reference manual/functions/time.md) and [time\_close()](../../reference manual/functions/time_close.md) calls that return timestamps based on a specified chart bar offset (`chartOffsetInput`) and a higher-timeframe (HTF) offset (`tfOffsetInput`):

- The first drawing is a vertical [line](../2. Visuals/visuals_lines-and-boxes.md#lines) indicating the chart bar at the `bars_back` offset (`chartOffsetInput`), which the other drawings use in their timestamp calculations. The line uses the value from `time("", bars_back = chartOffsetInput)` as the `x1` and `x2` coordinates.
- The second is a blue [box](../2. Visuals/visuals_lines-and-boxes.md#boxes) showing the range of the higher-timeframe (HTF) bar that _contains_ the offset chart bar indicated by the line. It uses the value of `time(tfInput, bars_back = chartOffsetInput)` as the `left` coordinate, and the value of `time_close(tfInput, bars_back = chartOffsetInput)` as the `right` coordinate.
- The third is a purple box that shows the range of the HTF bar that is `tfOffsetInput` periods _away_ from the HTF bar represented by the blue box. The [time()](../../reference manual/functions/time.md) and [time\_close()](../../reference manual/functions/time_close.md) calls for this box use the same `timeframe` and `bars_back` arguments as those for the blue box, and they include `timeframe_bars_back = tfOffsetInput` to apply the additional HTF bar offset.

Both boxes also display [formatted](../1. Concepts/concepts_time.md#formatting-dates-and-times) opening and closing timestamps, expressed in the exchange time zone, to show the time ranges covered by their respective periods:
...
- The script tracks high, low, and opening time values for each HTF period in [arrays](../3. Language/language_arrays.md) to determine the price ranges of the boxes. The arrays contain one element for each successive [change](../1. Concepts/concepts_time.md#testing-for-changes-in-higher-timeframes) in the value of `time(tfInput)`. The script [searches](../3. Language/language_arrays.md#searching-arrays) through the `times` array and finds the indices of the earliest elements that match the calculated HTF opening times, then uses the `highs` and `lows` array elements at those indices to set the top and bottom edges of the boxes.
...
![image](../images/Time-Time-functions-Time-and-time-close-functions-Calculating-timestamps-at-bar-offsets-1.DcEMMg3B_YQKFl.webp)
...
![image](../images/Time-Time-functions-Time-and-time-close-functions-Calculating-timestamps-at-bar-offsets-2.CrnGcGog_Z2bCCHP.webp)
...
![image](../images/Time-Time-functions-Time-and-time-close-functions-Calculating-timestamps-at-bar-offsets-3.CwaV-eSs_2hcMS4.webp)

### [Calendar-based functions](../1. Concepts/concepts_time.md#calendar-based-functions)

The [year()](../../reference manual/functions/year.md), [month()](../../reference manual/functions/month.md), [weekofyear()](../../reference manual/functions/weekofyear.md), [dayofmonth()](../../reference manual/functions/dayofmonth.md), [dayofweek()](../../reference manual/functions/dayofweek.md), [hour()](../../reference manual/functions/hour.md), [minute()](../../reference manual/functions/minute.md), and [second()](../../reference manual/functions/second.md) functions calculate _calendar-based_ “int” values from a [UNIX timestamp](../1. Concepts/concepts_time.md#unix-timestamps). Unlike the [calendar-based variables](../1. Concepts/concepts_time.md#calendar-based-variables), which always hold exchange calendar values based on the current bar’s opening timestamp, these functions can return calendar values for any valid timestamp and express them in a chosen [time zone](../1. Concepts/concepts_time.md#time-zones).
...
- The `timezone` parameter accepts a [time zone string](../1. Concepts/concepts_time.md#time-zone-strings) specifying the returned value’s time zone. If the `timezone` argument is not specified, the function uses the exchange time zone ( [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md)).
...
For instance, the simple script below uses two calls to [dayofmonth()](../../reference manual/functions/dayofmonth.md) to calculate each bar’s opening day in the exchange time zone and the “Australia/Sydney” time zone. It plots the results of the two calls in a separate pane for comparison:

![image](../images/Time-Time-functions-Calendar-based-functions-1.7yz46qU5_Z1puKx5.webp)
...
- The first [dayofmonth()](../../reference manual/functions/dayofmonth.md) call calculates the bar’s opening day in the exchange time zone because it does not include a `timezone` argument. This call returns the same value that the [dayofmonth](../../reference manual/variables/dayofmonth.md) variable references.
- Our example symbol’s exchange time zone is “America/New\_York”, which follows UTC-5 during standard time and UTC-4 during daylight saving time (DST). The “Australia/Sydney” time zone follows UTC+10 during standard time and UTC+11 during DST. However, Sydney observes DST at _different_ times of the year than New York. As such, its time zone is 14, 15, or 16 hours ahead of the exchange time zone, depending on the time of year. The plots on our “1D” chart diverge when the difference is at least 15 hours because the bars open at 09:30
in exchange time, and 15 hours ahead is 00:30
on the _next_ calendar day.
...
For example, this script uses the [timestamp()](../../reference manual/functions/timestamp.md) function to calculate a UNIX timestamp from a date “string”, and it calculates the calendar day from that timestamp, in the exchange time zone, with the [dayofmonth()](../../reference manual/functions/dayofmonth.md) function. The script compares each bar’s opening day to the calculated day and highlights the background when the two are equal:

![image](../images/Time-Time-functions-Calendar-based-functions-2.CLs3hZaQ_2oqPB7.webp)
...
- The [timestamp()](../../reference manual/functions/timestamp.md) call treats its argument as a _UTC_ calendar date because its `dateString` argument does not specify time zone information. However, the [dayofmonth()](../../reference manual/functions/dayofmonth.md) call calculates the day in the _exchange time zone_. Our example symbol’s exchange time zone is “America/New\_York” (UTC-4/-5). Therefore, the returned value on this chart is 28 instead of 29.
- The script highlights _any_ bar on our chart that opens on the 28th day of _any_ month instead of only a specific bar because the [dayofmonth()](../../reference manual/functions/dayofmonth.md) function’s returned value does **not** represent a specific point in time on its own.
...
Similar to [calendar-based variables](../1. Concepts/concepts_time.md#calendar-based-variables), these functions are also helpful when testing for dates/times and detecting calendar changes on the chart. The example below uses the [year()](../../reference manual/functions/year.md), [month()](../../reference manual/functions/month.md), [weekofyear()](../../reference manual/functions/weekofyear.md), and [dayofweek()](../../reference manual/functions/dayofweek.md) functions on the [time\_close](../../reference manual/variables/time_close.md) timestamp to create conditions that test if the current bar is the first bar that closes in a new year, quarter, month, week, and day. The script [plots shapes](../1. Concepts/concepts_text-and-shapes.md#plotshape), draws [labels](../2. Visuals/visuals_text-and-shapes.md#labels), and uses [background colors](../1. Concepts/concepts_backgrounds.md) to visualize the conditions on the chart:

![image](../images/Time-Time-functions-Calendar-based-functions-3.BgN06lg1_8VRJr.webp)
...
- To detect when new bars start on a specific _timeframe_ rather than strictly calendar changes, check when the [ta.change()](../../reference manual/functions/ta.change.md) of a [time()](../../reference manual/functions/time.md) or [time\_close()](../../reference manual/functions/time_close.md) call’s returned value is nonzero, or use the [timeframe.change()](../../reference manual/functions/timeframe.change.md) function. See [this section](../1. Concepts/concepts_time.md#testing-for-changes-in-higher-timeframes) above for more information.

### [​`timestamp()`​](../1. Concepts/concepts_time.md#timestamp)

The [timestamp()](../../reference manual/functions/timestamp.md) function calculates a [UNIX timestamp](../1. Concepts/concepts_time.md#unix-timestamps) from a specified calendar date and time. It has the following three signatures:
...
The first two signatures listed include `year`, `month`, `day`, `hour`, `minute`, and `second` parameters that accept “int” values defining the calendar date and time. A [timestamp()](../../reference manual/functions/timestamp.md) call with either signature must include `year`, `month`, and `day` arguments. The other parameters are optional, each with a default value of 0. Both signatures can return either _“simple”_ or _“series”_ values, depending on the [qualified types](../3. Language/language_type-system.md#qualifiers) of the specified arguments.

The primary difference between the first two signatures is the `timezone` parameter, which accepts a [time zone string](../1. Concepts/concepts_time.md#time-zone-strings) that determines the [time zone](../1. Concepts/concepts_time.md#time-zones) of the _date and time_ specified by the other parameters. If a [timestamp()](../../reference manual/functions/timestamp.md) call with “int” calendar arguments does not include a `timezone` argument, it uses the exchange time zone ( [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md)) by default.

The third signature listed has only _one_ parameter, `dateString`, which accepts a “string” representing a valid calendar date (e.g., `"20 Aug 2024"`). The value can also include the time of day and time zone (e.g., `"20 Aug 2024 00:00:00 UTC+0"`). If the `dateString` argument does not specify the time of day, the [timestamp()](../../reference manual/functions/timestamp.md) call considers the time 00:00
(midnight).

Unlike the other two signatures, the default time zone for the third signature is **GMT+0**. It does **not** use the exchange time zone by default because it interprets time zone information from the `dateString` directly. Additionally, the third signature is the only one that returns a _“const int”_ value. As shown in the [Time input](../1. Concepts/concepts_inputs.md#time-input) section of the [Inputs](../1. Concepts/concepts_inputs.md) page, programmers can use this overload’s returned value as the `defval` argument in an [input.time()](../../reference manual/functions/input.time.md) function call.

When using the [timestamp()](../../reference manual/functions/timestamp.md) function, it’s crucial to understand how time zone information affects its calculations. The _absolute_ point in time represented by a specific calendar date _depends_ on its time zone, as an identical date and time in various time zones can refer to **different** amounts of time elapsed since the _UNIX Epoch_. Therefore, changing the time zone of the calendar date and time in a [timestamp()](../../reference manual/functions/timestamp.md) call _can change_ its returned [UNIX timestamp](../1. Concepts/concepts_time.md#unix-timestamps).

The following script compares the results of four different [timestamp()](../../reference manual/functions/timestamp.md) calls that evaluate the date 2021-01-01 in different time zones. The first [timestamp()](../../reference manual/functions/timestamp.md) call does not specify time zone information in its `dateString` argument, so it treats the value as a _UTC_ calendar date. The fourth call also evaluates the calendar date in UTC because it includes `"UTC0"` as the `timezone` argument. The second [timestamp()](../../reference manual/functions/timestamp.md) call uses the first signature listed above, meaning it uses the exchange time zone, and the third call uses the second signature with `"America/New_York"` as the `timezone` argument.

The script draws a [table](../../reference manual/types/table.md) with rows displaying each [timestamp()](../../reference manual/functions/timestamp.md) call, its assigned variable, the calculated UNIX timestamp, and a [formatted](../1. Concepts/concepts_time.md#formatting-dates-and-times) representation of the time. As we see on the “NASDAQ:MSFT” chart below, the first and fourth table rows show _different_ timestamps than the second and third, leading to different formatted strings in the last column:

![image](../images/Time-Time-functions-Timestamp-1.CFLoCdL8_Z7yfIK.webp)
...
- The [formatted](../1. Concepts/concepts_time.md#formatting-dates-and-times) date-time strings express results in the exchange time zone because the [str.format\_time()](../../reference manual/functions/str.format_time.md) function uses [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md) as the default `timezone` argument. The formatted values on our example chart show the offset string `"-0500"` because NASDAQ’s time zone (“America/New\_York”) follows UTC-5 during _standard time_.
- The formatted strings on the first and fourth rows show the date and time five hours _before_ January 1, 2021, because the [timestamp()](../../reference manual/functions/timestamp.md) calls evaluated the date in _UTC_ and the [str.format\_time()](../../reference manual/functions/str.format_time.md) calls used a time zone five hours _behind_ UTC.
- On our chart, the second and third rows have matching timestamps because both corresponding [timestamp()](../../reference manual/functions/timestamp.md) calls evaluated the date in the “America/New\_York” time zone. The two rows would show different results if we applied the script to a symbol with a different exchange time zone.

## [Formatting dates and times](../1. Concepts/concepts_time.md#formatting-dates-and-times)

Programmers can format [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps) into human-readable dates and times, expressed in specific [time zones](../1. Concepts/concepts_time.md#time-zones), using the [str.format\_time()](../../reference manual/functions/str.format_time.md) function. The function has the following signature:
...
- The `timezone` parameter determines the time zone of the formatted result. It accepts a [time zone string](../1. Concepts/concepts_time.md#time-zone-strings) in UTC or IANA notation. If the call does not specify a `timezone`, it uses the exchange time zone ( [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md)).

The general-purpose [str.format()](../../reference manual/functions/str.format.md) function can also format UNIX timestamps into readable dates and times. However, the function **cannot** express time information in _different_ time zones. It always expresses dates and times in **UTC+0**. In turn, using this function to format timestamps often results in _erroneous_ practices, such as mathematically modifying a timestamp to try and represent the time in another time zone. However, a UNIX timestamp is a unique, **time zone-agnostic** representation of a specific point in time. As such, modifying a UNIX timestamp changes the _absolute time_ it represents rather than expressing the same time in a different time zone.

The [str.format\_time()](../../reference manual/functions/str.format_time.md) function does not have this limitation, as it can calculate dates and times in _any_ time zone correctly without changing the meaning of a UNIX timestamp. In addition, unlike [str.format()](../../reference manual/functions/str.format.md), it is optimized specifically for processing time values. Therefore, we recommend that programmers use [str.format\_time()](../../reference manual/functions/str.format_time.md) instead of [str.format()](../../reference manual/functions/str.format.md) to format UNIX timestamps into readable dates and times.

A [str.format\_time()](../../reference manual/functions/str.format_time.md) call’s `format` argument determines the time information its returned value contains. The function treats characters and sequences in the argument as _formatting tokens_, which act as _placeholders_ for values in the returned date/time “string”. The following table outlines the most commonly used formatting tokens and explains what each represents:
...
| `"M"` | Month | Uppercase `"M"` for the month, not to be confused with _lowercase_`"m"` for the minute. <br>Use `"MM"` to include the two-digit month number with a leading zero for single-digit values (e.g., `"01"`), `"MMM"` to include the three-letter abbreviation of month (e.g., `"Jan"`), or `"MMMM"` for the full month name (e.g., `"January"`). |
| `"d"` | Day of the **month** | Lowercase `"d"`. <br>Includes the numeric day of the month (`"1"` to `"31"`). <br> Use `"dd"` for the two-digit day number with a leading zero for single-digit values. <br>It is _not_ a placeholder for the day number of the _week_ (1-7). Use [dayofweek()](../../reference manual/functions/dayofweek.md) to calculate that value. |
...
| `"E"` | Day of the **week** | Includes the abbreviation of the weekday _name_ (e.g., `"Mon"`). <br> Use `"EEEE"` for the weekday’s full name (e.g., `"Monday"`) |
...
| `"m"` | Minute | Lowercase `"m"` for the minute, not to be confused with _uppercase_`"M"` for the month. <br>Use `"mm"` for the two-digit minute with a leading zero for single-digit values. |
| `"s"` | Second | Lowercase `"s"` for the second, not to be confused with _uppercase_`"S"` for fractions of a second. <br>Use `"ss"` for the two-digit second with a leading zero for single-digit values. |
...
| `"z"` | Time zone ( **abbreviation or name**) | Lowercase `"z"`. <br> A single `"z"` includes the abbreviation of the time zone (e.g., `"EDT"`). <br> Use `"zzzz"` for the time zone’s name (e.g., `"Eastern Daylight Time"`). <br>It is not a placeholder for the _IANA identifier_. Use [syminfo.timezone](../../reference manual/variables/syminfo.timezone.md) to retrieve the exchange time zone’s IANA representation. |
...
The following example demonstrates how various formatting tokens affect the [str.format\_time()](../../reference manual/functions/str.format_time.md) function’s result. The script calls the function with different `format` arguments to create date/time strings from [time](../../reference manual/variables/time.md), [timenow](../../reference manual/variables/timenow.md), and [time\_close](../../reference manual/variables/time_close.md) timestamps. It displays each `format` value and the corresponding formatted result in a [table](../../reference manual/types/table.md) on the last bar:

![image](../images/Time-Formatting-dates-and-times-1.BsC0g9qz_2lteJl.webp)
...
## [Expressing time differences](../1. Concepts/concepts_time.md#expressing-time-differences)

Every [UNIX timestamp](../1. Concepts/concepts_time.md#unix-timestamps) represents a specific point in time as the absolute _time difference_ from a fixed historical point (epoch). The specific epoch all UNIX timestamps reference is _midnight UTC on January 1, 1970_. Programmers can [format](../1. Concepts/concepts_time.md#formatting-dates-and-times) UNIX timestamps into readable date-time strings with the [str.format\_time()](../../reference manual/functions/str.format_time.md) function because it uses the time difference from the UNIX Epoch in its date and time calculations.
...
Programmers may want to express the millisecond difference between two UNIX timestamps in _other time units_, such as seconds, days, etc. Some might assume they can use the difference as the `time` argument in a [str.format\_time()](../../reference manual/functions/str.format_time.md) call to achieve this result. However, the function always treats its `time` argument as the time elapsed from the _UNIX Epoch_ to derive a _calendar date/time_ representation in a specific [time zone](../1. Concepts/concepts_time.md#time-zones). It **does not** express time differences directly. Therefore, attempting to format timestamp _differences_ rather than timestamps with [str.format\_time()](../../reference manual/functions/str.format_time.md) leads to unintended results.

For example, the following script calculates the millisecond difference between the current execution time ( [timenow](../../reference manual/variables/timenow.md)) and the “1M” bar’s closing time (`time_close("1M")`) for a monthly countdown timer display. It attempts to express the time difference in another format using [str.format\_time()](../../reference manual/functions/str.format_time.md). It displays the function call’s result in a [table](../../reference manual/types/table.md), along with the original millisecond difference (`timeLeft`) and [formatted](../1. Concepts/concepts_time.md#formatting-dates-and-times) date-time representations of the timestamps.

As we see below, the table shows correct results for the formatted timestamps and the `timeLeft` value. However, the formatted time difference appears as `"1970-01-12T16:47:10-0500"`. Although the `timeLeft` value is supposed to represent a difference between timestamps rather than a specific point in time, the [str.format\_time()](../../reference manual/functions/str.format_time.md) function still treats the value as a **UNIX timestamp**. Consequently, it creates a “string” expressing the value as a _date and time_ in the UTC-5 time zone:

![image](../images/Time-Relative-time-differences-1.Bn5sSvwp_Z2g2NxI.webp)
...
The calculations required to express time differences depend on the chosen time units. The sections below explain how to express millisecond differences in [weekly and smaller units](../1. Concepts/concepts_time.md#weekly-and-smaller-units), and [monthly and larger units](../1. Concepts/concepts_time.md#monthly-and-larger-units).

### [Weekly and smaller units](../1. Concepts/concepts_time.md#weekly-and-smaller-units)
...
The script calls `formatTimeSpan()` to express the difference between two separate [time input](../1. Concepts/concepts_inputs.md#time-input) values in selected time units. It then displays the resulting “string” in a [table](../../reference manual/types/table.md) alongside [formatted](../1. Concepts/concepts_time.md#formatting-dates-and-times) representations of the start and end times:

![image](../images/Time-Relative-time-differences-Weekly-and-smaller-units-1.CkIHycnQ_ZLgkiQ.webp)
...
- The [user-defined function](../3. Language/language_user-defined-functions.md) uses [math.floor()](../../reference manual/functions/math.floor.md) to round each divided result down to the nearest “int” value to get the number of _complete_ units in the interval. After division, it uses the modulo assignment operator ( [%=](../../reference manual/operators/%=.md)) to get the _remainder_ and assign that value to the `timeDifference` variable. This process repeats for each selected unit.
...
![image](../images/Time-Relative-time-differences-Weekly-and-smaller-units-2.CwvuIXRj_1hdHCe.webp)

### [Monthly and larger units](../1. Concepts/concepts_time.md#monthly-and-larger-units)
...
Some programmers prefer to use the modular arithmetic outlined in the [previous section](../1. Concepts/concepts_time.md#weekly-and-smaller-units), with _approximate lengths_ for these irregular units, to calculate large-unit durations between [UNIX timestamps](../1. Concepts/concepts_time.md#unix-timestamps). With this process, programmers usually define the units in either of the following ways:
...
The function uses the process outlined in the [previous section](../1. Concepts/concepts_time.md#weekly-and-smaller-units) to calculate the daily and smaller units within the interval. For the monthly and yearly units, which have _irregular_ lengths, the function uses a [while](../3. Language/language_loops.md#while-loops) loop to iterate across calendar months. On each iteration, it increments monthly and yearly counters and subtracts the number of days in the added month from the day counter. After the loop ends, the function adjusts the year, month, and day counters to account for partial months elapsed between the timestamps. Finally, it uses the counters in a [str.format()](../../reference manual/functions/str.format.md) call to create a formatted “string” containing the calculated values.

The script calls this `formatTimeDifference()` function to calculate the years, months, days, hours, minutes, seconds, and milliseconds elapsed between two separate [time input](../1. Concepts/concepts_inputs.md#time-input) values and displays the result in a [label](../../reference manual/types/label.md):

![image](../images/Time-Relative-time-differences-Monthly-and-larger-units-1.BhYyGmym_Z1lA0Ft.webp)
