![](../2. Visuals/visuals_colors.md)

# [Colors](../2. Visuals/visuals_colors.md#colors)

## [Introduction](../2. Visuals/visuals_colors.md#introduction)
...
Pine Script has built-in colors such as [color.green](../../reference manual/constants/color.green.md), as well as functions like [color.rgb()](../../reference manual/functions/color.rgb.md) which allow you to dynamically generate any color in the RGBA color space.

### [Transparency](../2. Visuals/visuals_colors.md#transparency)
...
## [Constant colors](../2. Visuals/visuals_colors.md#constant-colors)

There are 17 built-in colors in Pine Script. This table lists their names, hexadecimal equivalent, and RGB values as arguments to [color.rgb()](../../reference manual/functions/color.rgb.md):
...
The following script shows three different ways to express [color.olive](../../reference manual/constants/color.olive.md) with 40% transparency. All three methods are functionally equivalent:

![image](../images/Colors-Constant-colors-1.CnlC-u-k_1tReXV.webp)
...
## [Conditional coloring](../2. Visuals/visuals_colors.md#conditional-coloring)
...
![image](../images/Colors-ConditionalColors-1.D1QaF3Y-_p2AUr.webp)
...
To undertand how this code works, one must first know that [ta.pivothigh()](../../reference manual/functions/ta.pivothigh.md) and [ta.pivotlow()](../../reference manual/functions/ta.pivotlow.md), used as they are here without an argument to the `source` parameter, will return a value when they find a [high](../../reference manual/variables/high.md)/ [low](../../reference manual/variables/low.md) pivot, otherwise they return [na](../../reference manual/variables/na.md).

When we test the value returned by the pivot function for [na](../../reference manual/variables/na.md) using the [nz()](../../reference manual/functions/nz.md) function, we allow the value returned to be assigned to the `pHi` or `pLo` variables only when it is not [na](../../reference manual/variables/na.md), otherwise the previous value of the variable is simply reassigned to it, which has no impact on its value. Keep in mind that previous values of `pHi` and `pLo` are preserved bar to bar because we use the [var](../../reference manual/keywords/var.md) keyword when initializing them, which causes the initialization to only occur on the first bar.

All that’s left to do next is, when we plot our lines, to insert a ternary conditional statement that will yield [na](../../reference manual/variables/na.md) for the color when the pivot value changes, or the color selected in the script’s inputs when the pivot level does not change.

## [Calculated colors](../2. Visuals/visuals_colors.md#calculated-colors)

Using functions like [color.new()](../../reference manual/functions/color.new.md), [color.rgb()](../../reference manual/functions/color.rgb.md) and [color.from\_gradient()](../../reference manual/functions/color.from_gradient.md), one can build colors on the fly, as the script executes bar to bar.

[color.new()](../../reference manual/functions/color.new.md) is most useful when you need to generate different transparency levels from a base color.

[color.rgb()](../../reference manual/functions/color.rgb.md) is useful when you need to build colors dynamically from red, green, blue, or tranparency components. While [color.rgb()](../../reference manual/functions/color.rgb.md) creates a color, its sister functions [color.r()](../../reference manual/functions/color.r.md), [color.g()](../../reference manual/functions/color.g.md), [color.b()](../../reference manual/functions/color.b.md) and [color.t()](../../reference manual/functions/color.t.md) can be used to extract the red, green, blue or transparency values from a color, which can in turn be used to generate a variant.

[color.from\_gradient()](../../reference manual/functions/color.from_gradient.md) is useful to create linear gradients between two base colors. It determines which intermediary color to use by evaluating a source value against minimum and maximum values.

### [color.new()](../2. Visuals/visuals_colors.md#colornew)

Let’s put [color.new()](../../reference manual/functions/color.new.md) to use to create different transparencies for volume columns using one of two bull/bear base colors:

![image](../images/Colors-CalculatingColors-1.BtEyXAO2_ed3fM.webp)
...
### [color.rgb()](../2. Visuals/visuals_colors.md#colorrgb)

In our next example we use [color.rgb()](../../reference manual/functions/color.rgb.md) to build colors from RGBA values. We use the result in a holiday season gift for our friends, so they can bring their TradingView charts to parties:

![image](../images/Colors-CalculatingColors-2.B7PJa61g_Z1UHxzu.webp)
...
seed)](../../reference manual/functions/math.random.md)
...
### [color.from\_gradient()](../2. Visuals/visuals_colors.md#colorfrom_gradient)

Our last examples of color calculations will use [color.from\_gradient()](../../reference manual/functions/color.from_gradient.md). Let’s first use it in its simplest form, to color a CCI signal in a version of the indicator that otherwise looks like the built-in:

![image](../images/Colors-CalculatingColors-3.BWe7BFsC_Z23u9GI.webp)
...
[ta.cci()](../../reference manual/functions/ta.cci.md)
instead of calculating it the long way.

The argument used for `value` in [color.from\_gradient()](../../reference manual/functions/color.from_gradient.md) does not necessarily have to be the value of the line we are calculating. Anything we want can be used, as long as arguments for `bottom_value` and `top_value` can be supplied. Here, we enhance our CCI indicator by coloring the band using the number of bars since the signal has been above/below the centerline:

![image](../images/Colors-CalculatingColors-4.-U0l6lwc_2g9Rrh.webp)
...
## [Mixing transparencies](../2. Visuals/visuals_colors.md#mixing-transparencies)
...
![image](../images/Colors-MixingTransparencies-1.DJ-yTBxm_Z2maq1y.webp)

And with the dark theme:

![image](../images/Colors-MixingTransparencies-2.BJOvmETq_6x9mv.webp)
...
## [Tips](../2. Visuals/visuals_colors.md#tips)

### [Maintaining automatic color selectors](../2. Visuals/visuals_colors.md#maintaining-automatic-color-selectors)

Under certain conditions, Pine Script can automatically display all of the colors used in a script’s plots in the “Settings/Style” menu. These plots are graphics created by all `plot*()` functions, [barcolor()](../../reference manual/functions/barcolor.md), and [bgcolor()](../../reference manual/functions/bgcolor.md). The user can change the colors using a color picker. This feature allows colors in scripts to be customized without any extra code.

For example, this simple script has a [plot()](../../reference manual/functions/plot.md) that is colored either teal or red, depending on the relationship between the bar’s [close](../../reference manual/variables/close.md) and [open](../../reference manual/variables/open.md). The script does not specify that these colors should be editable, nor does it create any color-related inputs. Nevertheless, Pine Script automatically displays the colors in the “Settings/Style” menu and allows the user to change them, along with the style of the plot:

![image](../images/Colors-MaintainAutomaticColorSelection-1.BHISUteP_ZDHaIR.webp)
...
Unfortunately, with these changes the “Style” tab does not display a color picker any longer. This is because we use the “series bool” condition `close > open` to decide the color, and then pass the result of this expression to a single [color.new()](../../reference manual/functions/color.new.md) call. The qualified type of the calculated color that it returns is “series color”.

![image](../images/Colors-MaintainAutomaticColorSelection-2.BO3w97fq_1y7w8G.webp)

![image](../images/Colors-MaintainAutomaticColorSelection-3.B0TUMw_p_1sY3g1.webp)

To avoid this, we can ensure that every calculated color created by [color.new()](../../reference manual/functions/color.new.md) is a “const color”. Below, we wrap [color.teal](../../reference manual/constants/color.teal.md) and [color.red](../../reference manual/constants/color.red.md) separately with [color.new()](../../reference manual/functions/color.new.md) — creating two constant calculated colors in the process — and then decide which one to assign to `plotColor` based on the condition. And while the `plotColor` variable is a “series color”, each [color.new()](../../reference manual/functions/color.new.md) call returns a constant color, so the script displays a color picker in the “Style” tab:
...
![image](../images/Colors-MaintainAutomaticColorSelection-4.B7QSrDzd_ZRzAFW.webp)

To calculate the colors at runtime, create custom color inputs for all of the colors that are to be editable. This approach requires more effort, but allows significantly more control over what the user can affect. Learn more about creating color inputs on the [Inputs page](../1. Concepts/concepts_inputs.md#color-input).

### [Designing usable colors schemes](../2. Visuals/visuals_colors.md#designing-usable-colors-schemes)
...
### [Plot crisp lines](../2. Visuals/visuals_colors.md#plot-crisp-lines)
...
![image](../images/Colors-PlotCrispLines-1.CJkrlPd__Z1lm8Mw.webp)
...
### [Customize gradients](../2. Visuals/visuals_colors.md#customize-gradients)
...
[Previous 
**Bar plotting**](../2. Visuals/visuals_bar-plotting.md) [Next 
