![](../2. Visuals/visuals_plots.md)

# [Plots](../2. Visuals/visuals_plots.md#plots)

## [Introduction](../2. Visuals/visuals_plots.md#introduction)
...
[Fills](../2. Visuals/visuals_fills.md).

This script showcases a few different uses of
[plot()](../../reference manual/functions/plot.md)
in an overlay script:

![image](../images/Plots-Introduction-01.Cylrqa-m_Iu01z.webp)
...
- The first [plot()](../../reference manual/functions/plot.md) call plots a blue line across the bar highs with a dashed line style. The line uses the default width of 1 pixel.
...
- The fourth call plots a gray circle at the bars’ [low](../../reference manual/variables/low.md).
- The last plot requires some preparation. We first define our bull/bear colors, calculate an [Arnaud Legoux Moving Average](https://www.tradingview.com/support/solutions/43000594683), then make our color calculations. We initialize our color variable on bar zero only, using [var](../../reference manual/keywords/var.md).
We initialize it to [color.silver](../../reference manual/constants/color.silver.md), so on the dataset’s first bars, until the line is higher/lower than its
value two bars ago, it is silver. Looking two bars back creates smoother color transitions than one bar back.

The next script shows other uses of [plot()](../../reference manual/functions/plot.md) in a separate pane:

![image](../images/Plots-Introduction-02.CEeiodqC_Z25q38U.webp)
...
- Because the first [plot()](../../reference manual/functions/plot.md) call plots _columns_, we do not use the `linewidth`
...
as a _histogram_, for which the `linewidth` parameter controls the
...
the bright `GREEN` or `PINK` colors when the bar is up/down _and_ the
...
`GREEN_LIGHT` or `PINK_LIGHT` colors when the bar is up/down _and_ the
...
A [plot()](../../reference manual/functions/plot.md)
call must always be in the script’s _global_ scope, i.e., at the _beginning_ of a non-indented line. Scripts cannot call the [plot()](../../reference manual/functions/plot.md) function from the indented (local) blocks of [user-defined functions](../3. Language/language_user-defined-functions.md) or conditional structures like [if](../../reference manual/keywords/if.md) blocks, [for](../../reference manual/keywords/for.md) loops, etc. Calls to
[plot()](../../reference manual/functions/plot.md) **can**, however, be designed to plot conditionally in two ways, which
we cover in the
[Plotting conditionally](../2. Visuals/visuals_plots.md#plotting-conditionally) section of this page.

If the [plot()](../../reference manual/functions/plot.md) call includes `force_overlay = true`, the result always displays on the main chart pane, even if the script is running in a separate pane. Otherwise, the script displays the plot in the same pane in which it runs. By contrast, scripts can only [color bars](../2. Visuals/visuals_bar-coloring.md) in the main chart pane, regardless of where they run.

## [​`plot()`​ parameters](../2. Visuals/visuals_plots.md#plot-parameters)
...
int/float” type. Note that because the [auto-casting](../3. Language/language_type-system.md#type-casting) rules in Pine
...
[plot.style\_columns](../../reference manual/constants/plot.style_columns.md)
is used.
...
- [plot.style\_line](../../reference manual/constants/plot.style_line.md)
(the default): It plots a continous line using the `linewidth`
argument in pixels for its width.
It does not plot any [na](../../reference manual/variables/na.md) values, but does draw a line through them by joining the most recent non- [na](../../reference manual/variables/na.md) value to the next non- [na](../../reference manual/variables/na.md) value.
comes in.
- [plot.style\_linebr](../../reference manual/constants/plot.style_linebr.md):
Allows the plotting of discontinuous lines by not plotting on [na](../../reference manual/variables/na.md) values, and not joining such gaps.
- [plot.style\_stepline](../../reference manual/constants/plot.style_stepline.md):
...
[na](../../reference manual/variables/na.md)
values.
- [plot.style\_area](../../reference manual/constants/plot.style_area.md):
...
values below it.
- [plot.style\_areabr](../../reference manual/constants/plot.style_areabr.md):
This is similar to
[plot.style\_area](../../reference manual/constants/plot.style_area.md)
...
the _y_ range of the script’s visual space. If only high values
situated far away from the `histbase` are plotted, for example,
those values will be used to calculate the _y_ scale of the
...
of `histbase` in the _y_ scale of the script’s visual space.
- [plot.style\_histogram](../../reference manual/constants/plot.style_histogram.md):
Plots columns similar to those of the “Volume” built-in
...
it. Always includes the value of `histbase` in the _y_ scale of
the script’s visual space.
- [plot.style\_circles](../../reference manual/constants/plot.style_circles.md)
and
[plot.style\_cross](../../reference manual/constants/plot.style_cross.md):
...
visible, counting _backward_ from the last bar. Bars beyond the specified amount show [na](../../reference manual/variables/na.md) values for this plot. It accepts an “input int” type argument, so it cannot be
...
[display.all](../../reference manual/constants/display.all.md). This parameter supports addition and subtraction of `display.*` options, e.g., `display.all - display.pane` hides the pane visuals but displays the plot’s numeric results everywhere else, while `display.status_line + display.data_window` displays results only in those two locations.

When this parameter is set to
[display.none](../../reference manual/constants/display.none.md), the script calculates the plot values, but does not display them in the script pane, status line, or Data Window, and the hidden plot does not affect the scale of the script’s visual space. This display option can be useful for plots intended for
...
Specifies the numeric format used to represent plot values in the status line, Data Window, and price scale. It accepts one of the following values: [format.price](../../reference manual/constants/format.price.md), [format.percent](../../reference manual/constants/format.percent.md), or [format.volume](../../reference manual/constants/format.volume.md).

By default, the plot uses the same format as the script’s [indicator()](../../reference manual/functions/indicator.md) or [strategy()](../../reference manual/functions/strategy.md) declaration statement. If the plot and declaration use different formats, then the plot’s `format` argument takes precedence.
...
By default, the plot uses the same precision as the script’s [indicator()](../../reference manual/functions/indicator.md) or [strategy()](../../reference manual/functions/strategy.md) declaration statement. If the plot and declaration use different precisions, then the plot’s `precision` argument takes precedence.

The `precision` parameter does not affect plot results formatted using the [format.volume](../../reference manual/constants/format.volume.md) argument, because the decimal precision rules of the volume formatting take precedence.
...
Controls the style of plotted lines, using one of the following arguments: [plot.linestyle\_solid](../../reference manual/constants/plot.linestyle_solid.md), [plot.linestyle\_dashed](../../reference manual/constants/plot.linestyle_dashed.md), or [plot.linestyle\_dotted](../../reference manual/constants/plot.linestyle_dotted.md).

This parameter only modifies lines, so the `style` parameter must use one of the following arguments: [plot.style\_line](../../reference manual/constants/plot.style_line.md) (the default), [plot.style\_linebr](../../reference manual/constants/plot.style_linebr.md), [plot.style\_stepline](../../reference manual/constants/plot.style_stepline.md), [plot.style\_stepline\_diamond](../../reference manual/constants/plot.style_stepline_diamond.md), or [plot.style\_area](../../reference manual/constants/plot.style_area.md) for it to take effect.

## [Plotting conditionally](../2. Visuals/visuals_plots.md#plotting-conditionally)

Scripts cannot call the [plot()](../../reference manual/functions/plot.md) function from conditional structures such as
[if](../../reference manual/keywords/if.md) blocks, but can control plots by varying their plotted values or their color.
When no plot is required, you can either plot
[na](../../reference manual/variables/na.md)
values, or plot values using
[na](../../reference manual/variables/na.md) color
or any color with 100 transparency (which also makes it invisible).

### [Value control](../2. Visuals/visuals_plots.md#value-control)
...
![image](../images/Plots-PlottingConditionally-01.DCGRnCbX_szhkT.webp)
...
[na](../../reference manual/variables/na.md)
values (or _gaps_), so the plot does not interrupt.
...
### [Color control](../2. Visuals/visuals_plots.md#color-control)

The [Conditional coloring](../2. Visuals/visuals_colors.md#conditional-coloring) section of the [Colors](../2. Visuals/visuals_colors.md) page discusses color control for plots. We’ll look here at a few examples.

The value of the `color` parameter in
[plot()](../../reference manual/functions/plot.md)
can be a constant, such as one of the built-in
[constant colors](../2. Visuals/visuals_colors.md#constant-colors) or a
[color literal](../3. Language/language_type-system.md#color). In Pine Script, the qualified type of such colors is
called **“const color”** (see the
[Type system](../3. Language/language_type-system.md) page). They
...
Finally, plot colors can also be _dynamic_ values, i.e., calculated
...
level transitions. Using [lines](../2. Visuals/visuals_lines-and-boxes.md) is one alternative, but you can also use
[plot()](../../reference manual/functions/plot.md)
like this:

![image](../images/Plots-PlottingConditionally-02.Dgz5RTVC_Z1D4462.webp)
...
[fixnan()](../../reference manual/functions/fixnan.md)
...
The [hline()](../../reference manual/functions/hline.md) function plots horizontal lines at fixed levels (see the page on
[Levels](../2. Visuals/visuals_levels.md)).
The [hline()](../../reference manual/functions/hline.md) function
is useful because it has some unique line styles that are not available with
[plot()](../../reference manual/functions/plot.md), and is often more performant than similar [plot()](../../reference manual/functions/plot.md) lines.
...
In cases where scripts need to use dynamically calculated prices or colors, the [plot()](../../reference manual/functions/plot.md) function can create similar horizontal levels.
...
![image](../images/Plots-Levels-01.jio4KIpw_ZQ7KPc.webp)
...
## [Offsets](../2. Visuals/visuals_plots.md#offsets)
...
For example, this script plots two [close](../../reference manual/variables/close.md) series. It displays the values in the _red_ series five bars to the left because its `offset` argument is negative, while displaying the values in the _green_ series five bars to the right because its `offset` is positive:

![image](../images/Plots-Offsets-01.CBzf5b6H_Z1APJvi.webp)
...
## [Plot count limit](../2. Visuals/visuals_plots.md#plot-count-limit)
...
_more than one_ plot in the total plot count.

For example, a [plot()](../../reference manual/functions/plot.md)
call counts as _one_ plot in the total plot count if it uses a “const
...
A [plot()](../../reference manual/functions/plot.md) call counts as _two_ plots in the total plot count if it uses a stronger [qualified type](../3. Language/language_type-system.md#qualifiers) for its `color` argument, such as any one of the following, because the resulting color is dynamic:
...
See the [Plot limits](../4. Writing_Scripts/writing_limitations.md#plot-limits) section of the [Limitations](../4. Writing_Scripts/writing_limitations.md) page for more information.

## [Scale](../2. Visuals/visuals_plots.md#scale)
...
displayed in a distinct _pane_ — or area — above or below the chart.
...
![image](../images/Plots-Scale-01.CE6dQl_T_1VGogS.webp)
...
Note that the _y_ axis of our script’s visual space is automatically
sized using the range of values plotted, i.e., the values of RSI. See
the page on [Colors](../2. Visuals/visuals_colors.md) for more
information on the
...
![image](../images/Plots-Scale-02.D8wP6yEJ_1ymoUP.webp)
...
### [Merging two indicators](../2. Visuals/visuals_plots.md#merging-two-indicators)
...
![image](../images/Plots-Scale-03.D9mEXIxt_Z1D5QSv.webp)
...
[Previous 
**Lines and boxes**](../2. Visuals/visuals_lines-and-boxes.md) [Next 
