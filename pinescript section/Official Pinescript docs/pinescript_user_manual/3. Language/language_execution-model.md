![](../3. Language/language_execution-model.md)

# [Execution model](../3. Language/language_execution-model.md#execution-model)

## [Introduction](../3. Language/language_execution-model.md#introduction)

Pine Script® relies on an event-driven, sequential execution model to control how a script’s compiled source code runs in charts, [alerts](../1. Concepts/concepts_alerts.md), [Deep Backtesting](https://www.tradingview.com/support/solutions/43000666199-what-is-deep-backtesting/) mode, and the [Pine Screener](https://www.tradingview.com/support/solutions/43000742436-tradingview-pine-screener-key-features-and-requirements/).

In contrast to the traditional execution model of most programming languages, Pine’s runtime system executes a script _repeatedly_ on the sequence of _historical bars_ and _realtime ticks_ in the dataset on which it runs, performing _separate_ calculations for _each bar_ as it progresses. After each execution on a closed bar, the necessary data from that execution becomes part of an internal [time series](../3. Language/language_execution-model.md#time-series), and the script can use that data in its calculations on subsequent bars.
...
### [Bar-by-bar execution](../3. Language/language_execution-model.md#bar-by-bar-execution)
...
Much of the power of Pine Script stems from its ability to process this time series data efficiently. When a user runs a script, its code does _not_ execute just once; it executes from start to end on _each bar_ in the symbol’s dataset individually, progressing from the first available bar to the most recent bar. Each separate script execution performs calculations or generates outputs (e.g., [plots](../1. Concepts/concepts_plots.md)) for a _specific bar_ using the data available on that bar.

A script can retrieve price, volume, and other essential data for each bar on which it executes by using the [built-in variables](../3. Language/language_built-ins.md#built-in-variables) that hold bar information, such as [open](../../reference manual/variables/open.md), [high](../../reference manual/variables/high.md), [low](../../reference manual/variables/low.md), [close](../../reference manual/variables/close.md), and [volume](../../reference manual/variables/volume.md). These variables automatically _update_ before each new execution to store the values for the _current bar_.

For example, the simple script below uses the [plot()](../../reference manual/functions/plot.md) function to display the series of [close](../../reference manual/variables/close.md) values (i.e., the closing price of each bar) on the chart:
...
1. The [close](../../reference manual/variables/close.md) variable automatically updates to hold the current bar’s latest price.
2. The [plot()](../../reference manual/functions/plot.md) function call plots the updated [close](../../reference manual/variables/close.md) value at the current bar’s position.
...
![image](../images/Execution-model-The-basics-Bar-by-bar-execution-1.D5nkQfFJ_Zan9Wp.webp)

Note that the above script evaluates the [plot()](../../reference manual/functions/plot.md) function call once for every bar on the chart, not just once in total. On each separate execution, the call defines the plotted point for the current bar: the chart’s first bar during the first execution, the second bar during the next, and so on.
...
Repeated code evaluation also applies to [variable declarations](../3. Language/language_variable-declarations.md). By default, a script does not declare a variable only once throughout its runtime; the script _re-declares_ that variable and assigns an initial value based on the current bar’s data during _each_ new evaluation of its scope.

Let’s look at a simple example. The following script declares an `x` variable of the “int” [type](../3. Language/language_type-system.md#types) with an initial value of 0. Then, it increases the variable’s value by 10 with the addition assignment operator ( [+=](../../reference manual/operators/+=.md)). The script calls [plot()](../../reference manual/functions/plot.md) to display the value of `x` on each bar in a separate pane:

![image](../images/Execution-model-The-basics-Bar-by-bar-execution-2.CYV_XhNc_Z22CJ8u.webp)
...
Programmers can change the behavior of a variable, enabling it to _persist_ and preserve updates to its value _across bars_, by including the [var](../../reference manual/keywords/var.md) keyword in its declaration, as described in the [Declaration modes](../3. Language/language_variable-declarations.md#declaration-modes) section of the [Variable declarations](../3. Language/language_variable-declarations.md) page.

Below, we modify the previous script by adding [var](../../reference manual/keywords/var.md) to the `x` declaration. Now, the script declares and initializes `x` only _once_ — on the _first bar_ — and that variable persists across _all_ bars that follow. The script now plots a line that _increases_ by 10 on each bar, because `x` preserves the result from each addition across the chart’s history. The value changes from 0 to 10 on the first bar, then to 20 on the second, and so on:

![image](../images/Execution-model-The-basics-Bar-by-bar-execution-3.2eRg8rX8_Z1OsSNk.webp)
...
### [Storing and using data from previous bars](../3. Language/language_execution-model.md#storing-and-using-data-from-previous-bars)
...
- Using the [\[\] history-referencing operator](../3. Language/language_operators.md#-history-referencing-operator). The number in the square brackets represents how many _bars back_ from the current bar the script looks to retrieve a past value. For instance, `close[1]` retrieves the [close](../../reference manual/variables/close.md) value from _one bar before_ the current bar, and `close[100]` retrieves the value from _100 bars back_.
- Calling the [built-in functions](../3. Language/language_built-ins.md#built-in-functions) that calculate on past values internally, such as `ta.*()` functions. For instance, `ta.change(close, 10)` calculates the difference between the current value of [close](../../reference manual/variables/close.md) and its value from 10 bars back.

The example below uses both of the above techniques to perform calculations based on data from previous bars. The script calculates a series of bar-by-bar price returns and plots the result as color-coded columns. It declares two global variables on each bar: `priceReturn` for the calculated returns, and `returnColor` for the plot’s color. The `priceReturn` value is the result of dividing the current one-bar change in closing prices (`ta.change(close, 1)`) by the previous bar’s closing price (`close[1]`). The `returnColor` value is [color.teal](../../reference manual/constants/color.teal.md) if the current value of `priceReturn` is higher than the value from the previous bar (`priceReturn[1]`), and [color.maroon](../../reference manual/constants/color.maroon.md) otherwise:

![image](../images/Execution-model-The-basics-Storing-and-using-data-from-previous-bars-1.CZbtEkeL_Z1yKAyd.webp)
...
- This script does _not_ plot a column on bar 0 (the _first_ bar). The `priceReturn` value is [na](../../reference manual/variables/na.md) on that bar, because there is _no previous bar_ available for the script to reference at that point.

### [Realtime bars](../3. Language/language_execution-model.md#realtime-bars)
...
Let’s look at rollback and recalculation in action. The following script uses [ta.stoch()](../../reference manual/functions/ta.stoch.md) to calculate the [Stochastic oscillator](https://www.tradingview.com/support/solutions/43000502332-stochastic-stoch/) based on the [close](../../reference manual/variables/close.md), [high](../../reference manual/variables/high.md), and [low](../../reference manual/variables/low.md) values over a specified number of bars, then plots the result in a separate pane. It also calls [bgcolor()](../../reference manual/functions/bgcolor.md) to highlight the background on each realtime bar — where [barstate.isrealtime](../../reference manual/variables/barstate.isrealtime.md) is `true` — for visual reference:
...
![image](../images/Execution-model-The-basics-Realtime-bars-1.AOvj4-Z1_xYxwo.webp)

Every time the bar updates, rollback _resets_ the script’s data for that bar, and the script _recalculates_ its result using the latest [high](../../reference manual/variables/high.md), [low](../../reference manual/variables/low.md), and [close](../../reference manual/variables/close.md) values. Here, halfway through the realtime bar’s period, the oscillator’s plot now shows a value of 16.71:

![image](../images/Execution-model-The-basics-Realtime-bars-2.C780uf5g_2fw87S.webp)
...
![image](../images/Execution-model-The-basics-Realtime-bars-3.rq8kAEPJ_ZnN0rj.webp)
...
- The [input.int()](../../reference manual/functions/input.int.md) function returns a value of the “input int” _qualified type_. Values qualified as “input” are established _before_ the first script execution, and they remain consistent throughout the script’s runtime. If the user changes the “Length” input to a new value, the script _restarts_ to perform new calculations across the dataset using that value. See the [Inputs](../1. Concepts/concepts_inputs.md) page and the [Qualifiers](../3. Language/language_type-system.md#qualifiers) section of the [Type system](../3. Language/language_type-system.md) page to learn more about script inputs and the “input” qualifier.
...
## [The details](../3. Language/language_execution-model.md#the-details)

The following sections provide in-depth details about Pine’s execution model, including the mechanics of executions on [historical bars](../3. Language/language_execution-model.md#executions-on-historical-bars) and [realtime bars](../3. Language/language_execution-model.md#executions-on-realtime-bars), which [events](../3. Language/language_execution-model.md#events-that-trigger-script-executions) trigger script executions, and how the runtime system maintains data across executions in a [time series](../3. Language/language_execution-model.md#time-series) format.

### [Executions on historical bars](../3. Language/language_execution-model.md#executions-on-historical-bars)

When a script loads on the chart or in another location after an [execution-triggering event](../3. Language/language_execution-model.md#events-that-trigger-script-executions), its compiled source code executes on _every_ accessible bar in the current dataset in order, starting with the first bar.

While the script loads, the runtime system performs the following steps for _each bar_ that it accesses:

1. It updates the built-in variables that hold bar information. For instance, the system sets the [open](../../reference manual/variables/open.md), [high](../../reference manual/variables/high.md), [low](../../reference manual/variables/low.md), and [close](../../reference manual/variables/close.md) variables to hold the OHLC price values of the bar _before_ each execution.
2. It executes the script’s compiled code from start to end using the data available as of the current bar.
3. After the execution ends, the system commits (saves) all necessary data for the current bar to the [time series](../3. Language/language_execution-model.md#time-series). The script can then access that data from [historical buffers](../3. Language/language_execution-model.md#historical-buffers) during its executions on subsequent bars by using the [history-referencing operator](../3. Language/language_operators.md#-history-referencing-operator) or the built-in functions that reference past bars internally.

These steps repeat for every successive bar up to the most recent bar. After the runtime system completes this process across the dataset, the script’s committed _outputs_ — such as [plots](../1. Concepts/concepts_plots.md), [drawings](../3. Language/language_type-system.md#drawing-types), [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs), and [Strategy Tester](../1. Concepts/concepts_strategies.md#strategy-tester) results — become available to the user.

All the closed bars on which the script executes while loading are _historical_, because they represent data points that were confirmed before the [event](../3. Language/language_execution-model.md#events-that-trigger-script-executions) that triggered the loading process. By default, all scripts execute **once** for each historical bar.

Let’s examine a simple indicator to understand how script executions work on historical bars.

The script below calculates the 20-bar moving average of [close](../../reference manual/variables/close.md) values and plots the result on the chart. The color of the plot depends on whether the average is above or below the value on the previous bar. The script also increments an `executionNum` variable to count code executions, then plots the result alongside [bar\_index](../../reference manual/variables/bar_index.md) for comparison. Additionally, it highlights the background of historical bars in orange for visual reference:
...
The [indicator()](../../reference manual/functions/indicator.md) call at the top of the code is a [declaration statement](../3. Language/language_script-structure.md#declaration-statement) that defines the script’s type and properties once, at _compile time_. This statement does not execute as the script runs on the dataset:

```pine
indicator("Executions on historical bars demo")
```

_Before_ each script execution on a bar, the runtime system updates the built-in [bar\_index](../../reference manual/variables/bar_index.md) and [close](../../reference manual/variables/close.md) variables required in the calculations. The [bar\_index](../../reference manual/variables/bar_index.md) value is the bar’s global _time series index_, where 0 represents the first bar, 1 represents the second, and so on. The [close](../../reference manual/variables/close.md) variable holds the bar’s _latest price_. For historical bars, its value is the _final price_ at the bar’s closing time.

Each time that the script executes, it declares and initializes a global `sma` variable of the “float” [type](../3. Language/language_type-system.md#types). This [variable declaration](../3. Language/language_variable-declarations.md) happens on _every_ execution because the code line does not specify a [declaration mode](../3. Language/language_variable-declarations.md#declaration-modes). The variable’s assigned value is the result of a [ta.sma()](../../reference manual/functions/ta.sma.md) function call. The call returns the average of the latest 20 [close](../../reference manual/variables/close.md) values as of the current bar, or [na](../../reference manual/variables/na.md) if fewer than 20 bars are available. After the execution ends, the system commits the new value of `sma` to the [time series](../3. Language/language_execution-model.md#time-series):
...
- The `//@variable` comment above the `sma` declaration is an [annotation](../3. Language/language_script-structure.md#compiler-annotations) that _documents_ the variable in the code. The Pine Editor displays the comment in a pop-up window when the user hovers the mouse pointer over the variable.

During each execution, the script also initializes a `plotColor` variable of the “color” type. The script uses a [ternary operation](../3. Language/language_operators.md#-ternary-operator) that compares the current `sma` value to `sma[1]` — the _last committed value_ for `sma` as of the _previous bar_ — to determine the `plotColor` variable’s assigned value. If the current `sma` value is higher than the last committed value, the `plotColor` value is [color.green](../../reference manual/constants/color.green.md). Otherwise, it is [color.red](../../reference manual/constants/color.red.md):
...
In contrast to the variables above, the script _does not_ initialize the `executionNum` variable on every execution. Instead, initialization happens only _once_ — on the _first_ bar — because the variable declaration is in the _global scope_ and uses the [varip](../../reference manual/keywords/varip.md) keyword. Once initialized, the variable _persists_ across all subsequent bars and the ticks within those bars. Only the reassignment or compound assignment [operators](../3. Language/language_operators.md) can change its value:
...
The code following the `executionNum` declaration uses the addition assignment operator ( [+=](../../reference manual/operators/+=.md)) to increase the variable’s value by one on each new execution. Starting from -1, the value increases to 0 on the first execution after initialization, then 1 on the second, and so on:
...
The script evaluates the [plot()](../../reference manual/functions/plot.md) and [bgcolor()](../../reference manual/functions/bgcolor.md) calls on every execution. Each [plot()](../../reference manual/functions/plot.md) call creates a new point on a line plot at the bar’s location on the time axis. The [bgcolor()](../../reference manual/functions/bgcolor.md) call creates a background color for the bar based on a ternary expression. The background is translucent orange if [barstate.ishistory](../../reference manual/variables/barstate.ishistory.md) is `true`. Otherwise, it is [na](../../reference manual/variables/na.md) (no color):
...
- The [plot()](../../reference manual/functions/plot.md) and [bgcolor()](../../reference manual/functions/bgcolor.md) calls that include `force_overlay = true` display their visuals on the main chart pane. The other [plot()](../../reference manual/functions/plot.md) calls
output visuals in a separate pane, because the [indicator()](../../reference manual/functions/indicator.md) call does not include `overlay = true`.
...
![image](../images/Execution-model-Executions-on-historical-bars-1.CbmTx9jr_Z202GQC.webp)

Note that:

- When the script first loads, _all_ bars, including the latest one, have an orange background because they initially represent _historical_ data. However, the latest bar on our chart is still open, meaning it is a _realtime bar_. After a new tick arrives from the realtime data feed, the bar’s values update, and the script executes _again_ on that bar. The orange background for the bar then _disappears_ because the system sets the value of [barstate.ishistory](../../reference manual/variables/barstate.ishistory.md) to `false`.
- The `executionNum` and [bar\_index](../../reference manual/variables/bar_index.md) values are identical on historical bars because the script executes _once per bar_ on that part of the dataset. However, they begin to differ on the realtime bar. On that bar, the script executes after _every new update_ to recalculate its results, and the `executionNum` value increases each time. See the [Executions on realtime bars](../3. Language/language_execution-model.md#executions-on-realtime-bars) section to learn more.
- An alternative, more robust method to track code executions is to use the [Pine Profiler](../4. Writing_Scripts/writing_profiling-and-optimization.md#pine-profiler). The profiler analyzes the total runtime and execution count of every significant part of the source code. To learn more about this feature, see the [Profiling and optimization](../4. Writing_Scripts/writing_profiling-and-optimization.md) page.

It’s important to note that, unlike indicators, [strategies](../1. Concepts/concepts_strategies.md) can execute _more than once_ per historical bar, depending on the specified [calculation behavior](../1. Concepts/concepts_strategies.md#altering-calculation-behavior). If the [strategy()](../../reference manual/functions/strategy.md) declaration statement includes `calc_on_order_fills = true`, or if the user selects the “After order is filled” checkbox in the “Settings/Properties” tab, the runtime system executes the script on _each available tick_ where the [broker emulator](../1. Concepts/concepts_strategies.md#broker-emulator) fills an order, or once per bar when there is no order to fill.

Let’s look at a simple example. The following strategy changes the direction of its simulated position on each execution. If there is an open short position or no position, the strategy places a [market order](../1. Concepts/concepts_strategies.md#market-orders) to close all short trades and enter a long trade. If a long position is open, the strategy places a market order to close it and open a short trade.

As with the previous example, this script increments an `executionNum` variable declared with [varip](../../reference manual/keywords/varip.md) to count new executions, plots the result alongside [bar\_index](../../reference manual/variables/bar_index.md) for comparison, and highlights the background of historical bars in orange with [bgcolor()](../../reference manual/functions/bgcolor.md):

![image](../images/Execution-model-Executions-on-historical-bars-2.DpP1JZmD_Z1F3fX1.webp)
...
- The [strategy.entry()](../../reference manual/functions/strategy.entry.md) command creates entry orders. By default, a long entry using this command reverses an open short position, and a short entry reverses an open long position. See the [Reversing positions](../1. Concepts/concepts_strategies.md#reversing-positions) section of the [Strategies](../1. Concepts/concepts_strategies.md) page to learn more.

The script above uses the default calculation behavior: it places a new order only at the close of each bar. The broker emulator fills the order at the next bar’s opening price, as the trade markers on the chart above indicate. The `executionNum` and [bar\_index](../../reference manual/variables/bar_index.md) plots show the same values because the script executes only once per bar.

If we include `calc_on_order_fills = true` in the [strategy()](../../reference manual/functions/strategy.md) declaration statement, the runtime system _re-executes_ the script on a bar after each new order fill to update the calculations. Our script’s logic generates a new order on _every_ execution, and the broker emulator considers historical bars to have _four ticks_ for filling orders by default (the open, high, low, and close). Therefore, with this change, the script executes **four times** per historical bar instead of only once. As shown below, the strategy now shows four trade markers on each historical bar, and the `executionNum` value is four times that of the [bar\_index](../../reference manual/variables/bar_index.md) variable:

![image](../images/Execution-model-Executions-on-historical-bars-3.C3-PPUqF_2mKdvK.webp)
...
- This script can execute _more than four_ times per bar if it uses [Bar Magnifier](../1. Concepts/concepts_strategies.md#bar-magnifier) mode, because this mode enables the broker emulator to fill orders on historical bars using intrabar prices from a _lower timeframe_.
...
- An alternative way to confirm the script’s increased execution count is to select and clear the “After order is filled” checkbox in the “Settings/Properties” tab while [profiling](../4. Writing_Scripts/writing_profiling-and-optimization.md#profiling-a-script) the code.

### [Executions on realtime bars](../3. Language/language_execution-model.md#executions-on-realtime-bars)
...
The runtime system erases the states of any variables that the script initializes during its executions before a bar’s close, excluding those declared using the [varip](../../reference manual/keywords/varip.md) keyword. When the script executes again after rollback, it _reinitializes_ the variables with new values or references based on the latest available data.

Likewise, the system does not preserve the _temporary_ states of built-in variables that hold values for the current bar. Before the new script execution, it sets the variables to use the bar’s most recent data. For instance, the system updates [close](../../reference manual/variables/close.md), [high](../../reference manual/variables/high.md), and [low](../../reference manual/variables/low.md) with the latest, highest, and lowest prices reported since the bar’s opening time.

**Reset changes to `var` variables**

Variables that use the [var](../../reference manual/keywords/var.md) keyword in their declaration remain initialized after the _first_ time that their scopes execute on a bar’s _closing tick_. Their assigned values or references _persist_ across subsequent bars, changing only after [reassignment](../3. Language/language_variable-declarations.md#variable-reassignment) or compound assignment operations.

Although these variables preserve data across successive bars, they **do not** preserve data across executions on the _ticks_ of an open bar. Rollback reverts all variables declared with [var](../../reference manual/keywords/var.md) before the current bar to the last committed states in the time series as of the previous bar.

For instance, if a variable declared with [var](../../reference manual/keywords/var.md) has a value of 20 on the open bar and 19 on the previous bar, the variable’s value reverts to 19 before the script executes on the next tick of the same bar. The temporary value of 20 does not persist.

**Replace plotted outputs**

The `plot*()`, [bgcolor()](../../reference manual/functions/bgcolor.md), [barcolor()](../../reference manual/functions/barcolor.md), and [fill()](../../reference manual/functions/fill.md) functions create visual outputs on _every bar_. These outputs are _temporary_ on the open realtime bar. When the script executes again after rollback, the new outputs for the bar from calls to these functions _replace_ the ones from the previous tick.

For example, when the expression `plot(close)` executes on the open bar, it displays the bar’s latest [close](../../reference manual/variables/close.md) value as of the current execution. However, the plotted result is **temporary** until the bar closes. After rollback, the [close](../../reference manual/variables/close.md) variable updates, then the script calls [plot()](../../reference manual/functions/plot.md) again on the next execution to replace the output from the previous tick and display the new value.

**Remove and revert objects**

[User-defined types (UDTs)](../3. Language/language_type-system.md#user-defined-types) and special types, such as [collections](../3. Language/language_type-system.md#collections) and [drawing types](../3. Language/language_type-system.md#drawing-types), are [reference types](../3. Language/language_type-system.md#reference-types). They define structures from which scripts create _objects_ — independent entities that store data elsewhere in memory. Variables of these types hold _references (IDs)_ that provide access to specific objects and their data; the variables do **not** store objects directly.

If a script creates objects on an open bar and does not assign their references to variables declared with the [varip](../../reference manual/keywords/varip.md) keyword, the rollback process _removes_ those objects. During the next execution on the open bar, the script creates _new objects_ if the updated logic allows it.

For example, if a script calls [label.new()](../../reference manual/functions/label.new.md) to create a [label](../../reference manual/types/label.md) object on the open bar, the system _deletes_ that object during rollback. On the next execution, the script evaluates [label.new()](../../reference manual/functions/label.new.md) again, creating a _new_ label that replaces the output. The label created on the previous tick no longer exists.

Similarly, for objects of built-in or user-defined types with references assigned to [var](../../reference manual/keywords/var.md) variables, the rollback process reverts any changes to those objects that occur on the open bar. The only exception is for [UDTs](../3. Language/language_type-system.md#user-defined-types) with _fields_ that include the [varip](../../reference manual/keywords/varip.md) keyword. See the [Objects](../3. Language/language_objects.md) page for more information.
...
- Variables or fields declared with the [varip](../../reference manual/keywords/varip.md) keyword **do not** revert to a previously committed state. They persist across _all_ script executions after initialization, even those on the ticks of an open realtime bar.
- Logged messages in the [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) pane do not disappear after rollback. The messages from any `log.*()` calls during executions on the ticks of realtime bars remain in the pane until the script reloads.
- The data from [strategy orders](../1. Concepts/concepts_strategies.md#orders-and-trades) placed or filled on the ticks within a bar is not subject to rollback. If a strategy script creates orders or the [broker emulator](../1. Concepts/concepts_strategies.md#broker-emulator) fills orders on an open bar, the data from those events persists.
- Rollback does not erase logs for [alerts](../1. Concepts/concepts_alerts.md) from the “Alerts” menu. All messages from a script alert remain visible until the user restarts the alert.
- Runtime errors from the system or the [runtime.error()](../../reference manual/functions/runtime.error.md) function completely _stop_ script executions. If an error occurs at any point while a script executes on an open bar, the system halts the script and does not revert the error after new updates from the data feed.

Let’s inspect the behavior of a simple indicator on realtime bars. The following script calculates an [RSI](https://www.tradingview.com/support/solutions/43000502338-relative-strength-index-rsi/) of [close](../../reference manual/variables/close.md) values using [ta.rsi()](../../reference manual/functions/ta.rsi.md) and displays the result with a [plot()](../../reference manual/functions/plot.md) call. To track the number of executions that occur _per bar_, the script increments an `executions` variable declared with [varip](../../reference manual/keywords/varip.md) and calculates its one-bar change using [ta.change()](../../reference manual/functions/ta.change.md). The script converts each bar’s execution count to a string with [str.tostring()](../../reference manual/functions/str.tostring.md), then displays the result in a color-coded [label](../../reference manual/types/label.md) at the bar’s [high](../../reference manual/variables/high.md). The label is purple if the bar is open. Otherwise, it is gray. The script also highlights the background of realtime bars in orange using [bgcolor()](../../reference manual/functions/bgcolor.md):
...
![image](../images/Execution-model-Executions-on-realtime-bars-1.C6ZIpkKC_GsYwv.webp)
...
When rollback occurs, the runtime system reverts the internal data of the [ta.rsi()](../../reference manual/functions/ta.rsi.md) call to its last committed state, erases the state of the `rsi` variable, and deletes the latest [label](../../reference manual/types/label.md) object. However, the system does not revert the `executions` variable because it uses the [varip](../../reference manual/keywords/varip.md) keyword.

After rollback, the system updates the built-in [close](../../reference manual/variables/close.md), [high](../../reference manual/variables/high.md), and `barstate.*` variables using the current bar’s latest data, and the new execution begins. The script evaluates the [ta.rsi()](../../reference manual/functions/ta.rsi.md) call using the new [close](../../reference manual/variables/close.md) price and reinitializes the `rsi` variable with the returned value. Then, it increases the `executions` value by one, evaluates [ta.change()](../../reference manual/functions/ta.change.md) again, and creates a new label at the bar’s current high price to show the updated result. Lastly, it evaluates the [plot()](../../reference manual/functions/plot.md) and [bgcolor()](../../reference manual/functions/bgcolor.md) calls to replace the bar’s plotted visuals. The last bar’s label remains purple because the bar is still open, but the background color is now _orange_ because [barstate.isrealtime](../../reference manual/variables/barstate.isrealtime.md) is `true`:

![image](../images/Execution-model-Executions-on-realtime-bars-2.DkBtTFWJ_Z27muce.webp)
...
![image](../images/Execution-model-Executions-on-realtime-bars-3.5O5RsfWP_ZLQqNx.webp)

The last time that rollback and another execution occur on this bar is after the _closing tick_, when the bar becomes an _elapsed_ realtime bar. After the final execution, the bar’s label is _gray_ because [barstate.isconfirmed](../../reference manual/variables/barstate.isconfirmed.md) is `true`. The runtime system then _commits_ necessary data from this execution to the [time series](../3. Language/language_execution-model.md#time-series) for calculations on future bars.

Then, a new realtime bar opens after another update from the data feed, and the execution pattern continues:

![image](../images/Execution-model-Executions-on-realtime-bars-4.C8UTIyzZ_1C51r1.webp)

Note that:

- Although the previous bar is now confirmed, it still has an orange background corresponding to a _realtime_ state because it closed **after** the script’s loading time. When the script later reloads after an [execution-triggering event](../3. Language/language_execution-model.md#events-that-trigger-script-executions), that bar becomes _historical_.

It’s important to note that [strategies](../1. Concepts/concepts_strategies.md) often execute differently than indicators on realtime bars. By default, they execute only **once** per bar at each _closing tick_ without undergoing rollback. However, users can modify a strategy’s [calculation behavior](../1. Concepts/concepts_strategies.md#altering-calculation-behavior) to allow rollback and re-execution on a bar before its closing tick.

If the [strategy()](../../reference manual/functions/strategy.md) statement includes `calc_on_every_tick = true`, or if the user selects the “On every tick” checkbox in the “Settings/Properties” tab, the script executes on a realtime bar after _each new update_ from the data feed, similar to an indicator.

Additionally, if the [strategy()](../../reference manual/functions/strategy.md) statement includes `calc_on_order_fills = true` or the user selects “After order is filled” in the “Settings/Properties” tab, the script executes on _each tick_ where the [broker emulator](../1. Concepts/concepts_strategies.md#broker-emulator) fills an order. With this behavior, the system can execute the script multiple times on the open bar, but only on the ticks where an _order fill_ occurs.

To summarize the general process for script executions on realtime bars:

- An [indicator](../../reference manual/functions/indicator.md) or [library](../../reference manual/functions/library.md) script executes on the _first available tick_ in an open realtime bar, then _once per update_ to recalculate the results for the bar using the latest data. A [strategy](../../reference manual/functions/strategy.md) script executes only on the bar’s _closing tick_ by default, but users can modify its calculation behavior to allow executions while the bar is open.
...
- After the script executes on an _elapsed_ realtime bar’s closing tick, the system _commits_ necessary data from that execution to the [time series](../3. Language/language_execution-model.md#time-series) for access on later bars. It does **not** commit the data from executions on the bar’s _unconfirmed_ values from previous ticks.

### [Events that trigger script executions](../3. Language/language_execution-model.md#events-that-trigger-script-executions)
...
Other events also trigger the loading process for a script on the chart. However, these events do not _always_ cause new script executions on past bars. The results from running a script with a unique combination of settings are often temporarily _cached_. If cached data exists for a selected combination of settings, the system loads the script using that data. See the [Caching](../3. Language/language_execution-model.md#caching) section for more information.
...
- The user selects new values for the [inputs](../1. Concepts/concepts_inputs.md) or [strategy properties](https://www.tradingview.com/support/solutions/43000628599-strategy-properties/) in the script’s “Settings” menu.
- The script uses the [chart.left\_visible\_bar\_time](../../reference manual/variables/chart.left_visible_bar_time.md) or [chart.right\_visible\_bar\_time](../../reference manual/variables/chart.right_visible_bar_time.md) variable, and the visible chart range changes.
- The script uses the [chart.fg\_color](../../reference manual/variables/chart.fg_color.md) or [chart.bg\_color](../../reference manual/variables/chart.bg_color.md) variable, and the user changes the chart’s background color.
...
- The user opens or closes the [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) pane.
- The user activates or deactivates the [Pine Profiler](../4. Writing_Scripts/writing_profiling-and-optimization.md#pine-profiler).

For scripts used in other locations, the following events trigger the loading process:

- The user creates a new script [alert](../1. Concepts/concepts_alerts.md) from the “Create Alert” dialog box.
- The user pauses and restarts an alert instance from the “Alerts” menu.
- The user clicks the “Generate report” button in the [Strategy Tester](../1. Concepts/concepts_strategies.md#strategy-tester) while [Deep Backtesting](https://www.tradingview.com/support/solutions/43000666199-what-is-deep-backtesting/) mode is enabled.
...
When a script completely reloads on the chart or in an alert after an applicable event, all the [elapsed realtime bars](../3. Language/language_execution-model.md#executions-on-realtime-bars) from the script’s previous run become [historical bars](../3. Language/language_execution-model.md#executions-on-historical-bars) in the new run, because they represent _confirmed_ data points that the script accesses from _a different data feed_ as it loads.
...
For example, the following script calculates the one-bar arithmetic return of the [close](../../reference manual/variables/close.md) series and displays the result as a line plot. On each realtime bar, the script updates three variables declared with [varip](../../reference manual/keywords/varip.md) to track the first, highest, and lowest return values calculated during executions across the bar’s ticks, then calls [plotcandle()](../../reference manual/functions/plotcandle.md) to plot a candle showing the values. Additionally, it uses [bgcolor()](../../reference manual/functions/bgcolor.md) to highlight the background of realtime bars in orange:
...
![image](../images/Execution-model-Events-that-trigger-script-executions-1.Bu_ElpOQ_ZQNKI6.webp)
...
![image](../images/Execution-model-Events-that-trigger-script-executions-2.CbIdk5sI_10biFO.webp)

Note that:

- The [barstate.isnew](../../reference manual/variables/barstate.isnew.md) variable has a value of `true` when a realtime bar opens, and `false` on all subsequent updates to the bar. If the script reloads midway through a realtime bar’s progression, only the background color appears on that bar. The script does not show a candle on the first realtime bar in that case, because its `o`, `h`, and `l` variables hold [na](../../reference manual/variables/na.md) until the first time that [barstate.isnew](../../reference manual/variables/barstate.isnew.md) is `true`.

#### [Caching](../3. Language/language_execution-model.md#caching)
...
- The values of [inputs](../1. Concepts/concepts_inputs.md) in the script’s “Settings/Inputs” tab.
...
- The values of the `chart.*` variables whose [qualifiers](../3. Language/language_type-system.md#qualifiers) are “input” ( [chart.left\_visible\_bar\_time](../../reference manual/variables/chart.left_visible_bar_time.md), [chart.right\_visible\_bar\_time](../../reference manual/variables/chart.right_visible_bar_time.md), [chart.fg\_color](../../reference manual/variables/chart.fg_color.md), and [chart.bg\_color](../../reference manual/variables/chart.bg_color.md)).
- The chart’s timeframe.
- The chart’s ticker identifier.
- Whether the [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) pane is open or closed.
- Whether the [Pine Profiler](../4. Writing_Scripts/writing_profiling-and-optimization.md#pine-profiler) is active or not.
...
This behavior enables users to change a script’s inputs, alter the chart, and toggle developer tools without losing information — including [bar states](../1. Concepts/concepts_bar-states.md) — from previous script runs using different settings. Additionally, caching helps reduce loading times and resource requirements when switching between settings or adding multiple instances of the same script to the chart.

To understand this behavior, let’s revisit the script from the [previous section](../3. Language/language_execution-model.md#events-that-trigger-script-executions). The script has different behaviors on historical and realtime bars. In the version below, we’ve added a `lengthInput` variable that holds the value from an [input.int()](../../reference manual/functions/input.int.md) call. The script uses this variable to define the length of the [ta.change()](../../reference manual/functions/ta.change.md) calculation and the offset of the [history-referencing operator](../3. Language/language_operators.md#-history-referencing-operator):
...
After we add the script to our 1m chart and let it run for a few minutes with a “Length” input value of 5, the script plots candles and highlights the background for the latest few bars, because [barstate.isrealtime](../../reference manual/variables/barstate.isrealtime.md) is `true` on those bars:

![image](../images/Execution-model-Events-that-trigger-script-executions-Caching-1.BY9-bg_Z_Z195PEh.webp)
...
![image](../images/Execution-model-Events-that-trigger-script-executions-Caching-2.DyVd2t5m_Z120RYY.webp)
...
![image](../images/Execution-model-Events-that-trigger-script-executions-Caching-3.ByASZdF6_1VDxju.webp)
...
![image](../images/Execution-model-Events-that-trigger-script-executions-Caching-4.CGwk5hPj_ZNpD5c.webp)
...
Pine Script’s internal time series structure follows a similar format. After executing a script on a closed bar’s confirmed values, the runtime system _commits (saves)_ the results of the script’s statements and expressions to internal time series for later use. Each bar with committed data has an assigned _index_ in the series, where 0 represents the first bar, 1 represents the second, and so on. Scripts can retrieve this index with the [bar\_index](../../reference manual/variables/bar_index.md) variable.

Scripts can access the data committed to the time series on past bars by using the [\[\] history-referencing operator](../3. Language/language_operators.md#-history-referencing-operator). The value between the operator’s square brackets specifies the position of the referenced bar in the time series as a _relative offset_ behind the current bar. For variables and expressions in the global scope, an offset value of 1 refers to the previous bar at `bar_index - 1` (one bar back), a value of 2 refers to the bar at `bar_index - 2` (two bars back), and so on. An offset of 0 always refers to the _current bar_.

For example, consider the [open](../../reference manual/variables/open.md) variable, which holds the opening price of the current bar on which the script executes. Before each script execution on a new bar, the runtime system commits the [open](../../reference manual/variables/open.md) value from the last execution on the previous bar. Then, it updates the variable to hold the current bar’s opening price. To access the committed [open](../../reference manual/variables/open.md) value for the previous bar, we can use the expression `open[1]`. To access the committed value from 10 bars back, we use `open[10]`.

The script below performs three history-referencing operations to retrieve the current bar’s [open](../../reference manual/variables/open.md) value, the value from one bar back, and the value from a user-specified number of bars back. Then, it plots the retrieved values on the chart for comparison:

![image](../images/Execution-model-Time-series-1.CzCPjlKh_1BHcVW.webp)
...
- The expression `open[0]` is equivalent to using [open](../../reference manual/variables/open.md) without the history-referencing operator, because an offset of 0 refers to the current bar.
- At the beginning of the chart’s dataset, the expressions `open[1]` and `open[offsetInput]` return [na](../../reference manual/variables/na.md) because they refer to previous bars that are unavailable.
...
- Internally, the system maintains a _limited amount_ of time series data for variables and expressions in fixed-length _historical buffers_. These buffers define the _maximum offsets_ allowed for history-referencing operations. See the next section, [Historical buffers](../3. Language/language_execution-model.md#historical-buffers), to learn more.

Another way that scripts use committed values from a time series is by calling the built-in functions that reference history internally, such as those in the `ta.*` namespace. For example, the expression `ta.highest(high, 20)` calculates the highest value from the [high](../../reference manual/variables/high.md) series over a 20-bar window. It compares the series’ current value to the committed values from the previous 19 bars to determine the result. The script below executes this call on each bar and plots the resulting series on the chart. Additionally, the script colors the background of the last 20 bars on the chart to highlight the bars used in the latest execution’s [ta.highest()](../../reference manual/functions/ta.highest.md) call:

![image](../images/Execution-model-Time-series-2.tycqzMI6_1x0PYg.webp)
...
- The first 19 bars of the chart have a plotted value of [na](../../reference manual/variables/na.md), because the [ta.highest()](../../reference manual/functions/ta.highest.md) function call requires the [high](../../reference manual/variables/high.md) values from the current bar and 19 previous bars to calculate the result.
...
- The [ta.highest()](../../reference manual/functions/ta.highest.md) function and other functions that access past values from a time series must execute in the **global scope** for consistent calculations. Time series storage for variables and expressions in local scopes works _differently_ than that for global values. See the [Time series in scopes](../3. Language/language_execution-model.md#time-series-in-scopes) section for more information.

#### [Historical buffers](../3. Language/language_execution-model.md#historical-buffers)

To promote efficiency and help ensure computing resources remain available for all users, the Pine Script runtime system uses fixed-length _historical buffers_ to maintain a _limited amount_ of time series data for all variables and expressions. These historical buffers define the _maximum_ number of committed data points that a script can access on any bar via the [history-referencing operator](../3. Language/language_operators.md#-history-referencing-operator) or the built-in functions that reference past bars internally.

For most series, the underlying historical buffer can contain data from up to **5000** past bars. The only exception is for some built-in series such as [open](../../reference manual/variables/open.md), [close](../../reference manual/variables/close.md), and [time](../../reference manual/variables/time.md), whose buffers can store data for _more_ than 5000 bars.
...
For example, the script below retrieves a past value from the [close](../../reference manual/variables/close.md) series using the [history-referencing operator](../3. Language/language_operators.md#-history-referencing-operator) with an offset of 100 bars back on historical bars and 150 bars back on realtime bars. Because the script references data from 100 bars back during all [historical executions](../3. Language/language_execution-model.md#executions-on-historical-bars), the system sets the [close](../../reference manual/variables/close.md) buffer’s size to include only 100 past values. Consequently, an error occurs when the script executes on the open realtime bar, because a historical offset of 150 is _beyond_ the buffer’s limit:
...
- Modify the script to reference the maximum required number of bars back with the [\[\]](../../reference manual/operators/[].md) operator during its execution on the _first bar_.
- Call the [max\_bars\_back()](../../reference manual/functions/max_bars_back.md) function to explicitly set the historical buffer size for a _specific_ series.
- Include a `max_bars_back` argument in the [indicator()](../../reference manual/functions/indicator.md) or [strategy()](../../reference manual/functions/strategy.md) declaration statement to set the initial size of _all_ historical buffers.

Below, we modified the script by including the expression `max_bars_back(close, 150)`, which sets the size of the [close](../../reference manual/variables/close.md) buffer to include 150 past values. With the appropriate buffer size manually defined, the script’s history-referencing operation no longer causes an error on realtime bars:

![image](../images/Execution-model-Time-series-Historical-buffers-1.BRNlhWd3_Z9knPp.webp)
...
#### [Time series in scopes](../3. Language/language_execution-model.md#time-series-in-scopes)
...
All variables and expressions in a script that are outside [user-defined functions](../3. Language/language_user-defined-functions.md) or [methods](../3. Language/language_methods.md#user-defined-methods), [conditional structures](../3. Language/language_conditional-structures.md), [loops](../3. Language/language_loops.md), and [user-defined type](../3. Language/language_type-system.md#user-defined-types) or [enum type](../3. Language/language_type-system.md#enum-types) declarations belong to the _global scope_. The script evaluates variables and expressions from this scope _once_ for _every execution_ across bars and ticks in the dataset.
...
![image](../images/Execution-model-Time-series-Time-series-in-scopes-2.DnGQwjrs_SJfqA.webp)
...
A similar behavior applies to all built-in functions that reference past values internally, such as those in the `ta` namespace. For example, the [ta.sma()](../../reference manual/functions/ta.sma.md) function uses the current value of a `source` series and `length - 1` past values from that series to calculate a [moving average](https://www.tradingview.com/support/solutions/43000696841-simple-moving-average/). If a script calls this function only on _some_ bars instead of on _every_ bar, the historical buffer for `source` does not contain values for consecutive past bars. Therefore, such a call can cause unintended results, because the call calculates the returned average using an inconsistent history of previous values.

The script below demonstrates how the results of the [ta.sma()](../../reference manual/functions/ta.sma.md) function can vary with the scope in which the function call occurs. The script declares three global variables to hold calculated SMA values: `controlSMA`, `localSMA`, and `globalSMA`. The script initializes `controlSMA` using the result of a [ta.sma()](../../reference manual/functions/ta.sma.md) function call, and it initializes the other two variables with [na](../../reference manual/variables/na.md). Within the [if](../../reference manual/keywords/if.md) structure, the script updates the value of `globalSMA` using `controlSMA`, and it updates `localSMA` using the result of another [ta.sma()](../../reference manual/functions/ta.sma.md) call with the same arguments as the first call.

As shown below, the `controlSMA` and `globalSMA` variables have the same value. Both hold the result of the _global_ [ta.sma()](../../reference manual/functions/ta.sma.md) call, which executes on _every bar_. The internal historical buffer for `source` in that call thus includes committed values for consecutive past bars. In contrast, the `localSMA` value differs, because the [ta.sma()](../../reference manual/functions/ta.sma.md) call for that variable does _not_ execute on every bar. The buffer for that call’s local `source` series contains only the values from bars with an _even_ [bar\_index](../../reference manual/variables/bar_index.md) value:

![image](../images/Execution-model-Time-series-Time-series-in-scopes-3.H3TNziJy_20udot.webp)
...
- A script evaluates its global scope once on _every execution_. After each script execution on a bar’s closing tick, the system commits the data for variables and expressions in the global scope and updates their [historical buffers](../3. Language/language_execution-model.md#historical-buffers). The resulting buffers thus include data for consecutive past bars, ensuring consistent results for operations and functions that rely on past data.
- A script evaluates local scopes _zero_, _one_, or _several_ times per execution. The runtime system **cannot** maintain consistent bar-by-bar historical buffers for scopes that a script does _not_ evaluate on every bar, or for scopes that the script evaluates _more than once_ on a bar’s closing tick. Therefore, using the [\[\]](../../reference manual/operators/[].md) operator on local variables and expressions, or not calling functions that access past data once on each closing tick, can cause **unintended results**.
