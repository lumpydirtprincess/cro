![](../3. Language/language_maps.md)

ADVANCED

# [Maps](../3. Language/language_maps.md#maps)

## [Introduction](../3. Language/language_maps.md#introduction)

Pine Script _maps_ are [collections](../3. Language/language_type-system.md#collections) that store data in _key-value pairs_. They enable scripts to collect multiple values or references in a single location and associate those elements with specific _unique values (keys)_.

In contrast to [arrays](../3. Language/language_arrays.md) and [matrices](../3. Language/language_matrices.md), maps are _unordered_ collections. Scripts do not access a map’s value elements by traversing an internal index. Instead, they quickly access values by using the _keys_ from the collection’s key-value pairs as _lookup indices_.

A map’s key elements must be of a [value type](../3. Language/language_type-system.md#value-types), including any _fundamental type_ or [enum type](../3. Language/language_type-system.md#enum-types). Maps _do not_ allow the IDs of any [reference type](../3. Language/language_type-system.md#reference-types) as keys, but they do allow those IDs as _value elements_. Note that, as with other collections, maps cannot directly store the IDs of other collections. However, their value elements can store references to objects of [user-defined types](../3. Language/language_type-system.md#user-defined-types) that have collection-type fields. See the [Maps of other collections](../3. Language/language_maps.md#maps-of-other-collections) section to learn more.
...
## [Declaring a map](../3. Language/language_maps.md#declaring-a-map)
...
Where `<keyType, valueType>` is the _type template_ that defines the types of _key elements_ and _value elements_ that the map can contain, and `<expression>` is an expression that returns either the _reference (ID)_ of a map or [na](../../reference manual/variables/na.md). See the [Collections](../3. Language/language_type-system.md#collections) section of the [Type system](../3. Language/language_type-system.md) page to learn about type templates.

When initializing a map variable using [na](../../reference manual/variables/na.md), programmers must prefix the variable declaration with the [map](../../reference manual/types/map.md) keyword followed by a type template to explicitly define the variable’s _type identifier_ (e.g., `map<int, float>` for a variable that can reference a map of “int” key elements and “float” value elements).

The following line of code declares a `myMap` variable with an initial reference of [na](../../reference manual/variables/na.md). The variable declaration includes the type identifier `map<string, float>`, which tells the compiler that the variable can accept the ID of a map containing “string” keys and corresponding “float” values:

```pine
map<string, float> myMap = na
```

If a map variable is not initialized with [na](../../reference manual/variables/na.md), specifying the type identifier is optional, because the compiler can automatically determine the variable’s accepted type from the assigned map ID.
...
### [Using ​`var`​ and ​`varip`​ keywords](../3. Language/language_maps.md#using-var-and-varip-keywords)
...
values on the first chart bar. The script plots a calculated `oscillator` series in a separate pane. It uses the “color” values put into the map on the _first_ bar to color the plot on _every_ bar:

![image](../images/Maps-Declaring-a-map-Using-var-and-varip-keywords-1.DMKyDQUN_ZmKnf9.webp)
...
## [Reading and writing](../3. Language/language_maps.md#reading-and-writing)

### [Putting and getting key-value pairs](../3. Language/language_maps.md#putting-and-getting-key-value-pairs)
...
[keys](../../reference manual/functions/map.keys.md),
...
[map.get()](../../reference manual/functions/map.get.md).
This function returns the value if the `id` map
[contains](../../reference manual/functions/map.contains.md)
...
![image](../images/Maps-Reading-and-writing-Putting-and-getting-key-value-pairs-1.CnULguMa_Z1DB6oI.webp)
...
[map.put()](../../reference manual/functions/map.put.md) method
calls, as each of these keys is unique and can only appear once
in the `data` map.
- Replacing the pairs in a map does not change the internal
_insertion order_ of its keys. We discuss this further in the
[next section](../3. Language/language_maps.md#inspecting-keys-and-values).
...
or a
[user-defined type](../3. Language/language_type-system.md#user-defined-types) into a map, it’s important to note the inserted pair’s
`value` refers to that same object without copying it. Modifying the
object referenced by a key-value pair directly modifies the _original_ object.
...
![image](../images/Maps-Reading-and-writing-Putting-and-getting-key-value-pairs-2.wNHX2-4R_1TK0jD.webp)
...
[Copying objects](../3. Language/language_objects.md#copying-objects) section of the [Objects](../3. Language/language_objects.md) page.

### [Inspecting keys and values](../3. Language/language_maps.md#inspecting-keys-and-values)

#### [​`map.keys()`​ and ​`map.values()`​](../3. Language/language_maps.md#mapkeys-and-mapvalues)

To retrieve all keys and values put into a map, use
[map.keys()](../../reference manual/functions/map.keys.md)
and
[map.values()](../../reference manual/functions/map.values.md).
...
Although maps are _unordered_ collections, Pine Script internally
maintains the _insertion order_ of a map’s key-value pairs. As a
result, the
[map.keys()](../../reference manual/functions/map.keys.md)
and
[map.values()](../../reference manual/functions/map.values.md)
functions always return [arrays](../3. Language/language_arrays.md) with their elements ordered based on the `id` map’s
insertion order.
...
![image](../images/Maps-Reading-and-writing-Inspecting-keys-and-values-1.BtySOORc_ZH2EjW.webp)
...
[removes](../3. Language/language_maps.md#removing-key-value-pairs) the key beforehand.

Below, we’ve added a line of code that calls
[map.put()](../../reference manual/functions/map.put.md) as a method to insert
...
new key-value pair into the map _after_ the inserting one with the “Third” key, the
pair’s key and value are still second in the `keys` and `values` arrays,
because the key is already present in the map _before_ the new call:

![image](../images/Maps-Reading-and-writing-Inspecting-keys-and-values-2.CRhK5y_B_8E9wm.webp)
...
#### [​`map.contains()`​](../3. Language/language_maps.md#mapcontains)

To check if a specific `key` exists within an `id` map, use
[map.contains()](../../reference manual/functions/map.contains.md).
...
![image](../images/Maps-Reading-and-writing-Inspecting-keys-and-values-3.BFc856qo_Z1eiW4O.webp)
...
### [Removing key-value pairs](../3. Language/language_maps.md#removing-key-value-pairs)

To remove a specific key-value pair from an `id` map, use
[map.remove()](../../reference manual/functions/map.remove.md).
...
[contained](../3. Language/language_maps.md#mapcontains) the specified `key`. Otherwise, it returns
[na](../../reference manual/variables/na.md).

To remove all key-value pairs from an `id` map at once, use
[map.clear()](../../reference manual/functions/map.clear.md).

The following script creates a new `m` map,
[puts](../3. Language/language_maps.md#putting-and-getting-key-value-pairs) key-value pairs into the map, uses
a [map.remove()](../../reference manual/functions/map.remove.md) method call within a loop to remove each valid `key` listed in the `removeKeys`
array, then calls
[map.clear()](../../reference manual/functions/map.clear.md) as a method to remove all remaining key-value pairs. The script uses a custom `debugLabel()`
method to display the
[map.size()](../../reference manual/functions/map.size.md),
[map.keys()](../../reference manual/functions/map.keys.md),
and
[map.values()](../../reference manual/functions/map.values.md) results for the map after each change:

![image](../images/Maps-Reading-and-writing-Removing-key-value-pairs-1.vUrgRJdK_Z1QOGos.webp)
...
### [Combining maps](../3. Language/language_maps.md#combining-maps)

Scripts can combine two maps via
[map.put\_all()](../../reference manual/functions/map.put_all.md).
This function puts _all_ key-value pairs from the `id2` map, in their
...
calls
[map.put\_all()](../../reference manual/functions/map.put_all.md) as a method
...
[keys](../../reference manual/functions/map.keys.md)
and
[values](../../reference manual/functions/map.values.md)
of `mapA` and `mapB`, then another label displaying the contents of
`mapA` after putting all key-value pairs from `mapB` into it:

![image](../images/Maps-Reading-and-writing-Combining-maps-1.pxWmo3rc_1WdGIS.webp)
...
## [Looping through a map](../3. Language/language_maps.md#looping-through-a-map)

There are several ways scripts can iteratively access the keys and
values in a map. For example, one could loop through the [map.keys()](../../reference manual/functions/map.keys.md)
array and use
[map.get()](../../reference manual/functions/map.get.md)
...
However, we recommend using a [\`for…in\` loop](../3. Language/language_loops.md#forin-loops) directly on a map, as it
...
![image](../images/Maps-Looping-through-a-map-1.CTALDkOv_Z1o194B.webp)
...
## [Copying a map](../3. Language/language_maps.md#copying-a-map)

### [Shallow copies](../3. Language/language_maps.md#shallow-copies)

Scripts can make a _shallow copy_ of an `id` map by using the
[map.copy()](../../reference manual/functions/map.copy.md)
...
![image](../images/Maps-Copying-a-map-Shallow-copies-1.e7-GYyqa_1ot5NQ.webp)
...
### [Deep copies](../3. Language/language_maps.md#deep-copies)

While a [shallow copy](../3. Language/language_maps.md#shallow-copies) will suffice when copying maps that have values of a
[fundamental type](../3. Language/language_type-system.md#types) or [enum type](../3. Language/language_type-system.md#enum-types), it’s crucial to understand that shallow copies of a map with elements of a _reference type_
( [line](../../reference manual/types/line.md),
[linefill](../../reference manual/types/linefill.md),
[box](../../reference manual/types/box.md),
[polyline](../../reference manual/types/polyline.md),
...
[table](../../reference manual/types/table.md),
[chart.point](../../reference manual/types/chart.point.md)
or a [UDT](../3. Language/language_type-system.md#user-defined-types)) point to the same objects as the original. Modifying the
...
instances referenced in other locations, one can make a _deep copy_ by
...
[label](../../reference manual/types/label.md)
value elements, then
[puts](../3. Language/language_maps.md#putting-and-getting-key-value-pairs)
...
![image](../images/Maps-Copying-a-map-Deep-copies-1.DxdX2Bvp_Jc0PE.webp)
...
[putting](../3. Language/language_maps.md#putting-and-getting-key-value-pairs) key-value pairs containing the copies into a new
map instance.

## [Scope and history](../3. Language/language_maps.md#scope-and-history)
...
![image](../images/Maps-Scope-and-history-1.D57XNJl__1RzJCr.webp)
...
## [Maps of other collections](../3. Language/language_maps.md#maps-of-other-collections)

Maps cannot directly store references to other maps,
[arrays](../3. Language/language_arrays.md), or
[matrices](../3. Language/language_matrices.md) as value elements, but
they can hold references to objects of a
[user-defined type](../3. Language/language_type-system.md#user-defined-types) whose fields can reference other collections.

For example, suppose we want to create a “2D” map that uses
[string](../../reference manual/types/string.md)
keys to access _nested maps_ that hold pairs of
[string](../../reference manual/types/string.md)
keys and
[float](../../reference manual/types/float.md)
values. Since maps cannot directly reference other collections, we will first
create a _wrapper type_ with a field to reference a `map<string, float>`
instance, like so:
...
![image](../images/Maps-Maps-of-other-collections-1.DtwJ2MOI_Z2jwz6j.webp)
