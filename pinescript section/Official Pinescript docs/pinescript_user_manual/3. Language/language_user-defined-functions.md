![](../3. Language/language_user-defined-functions.md)

# [User-defined functions](../3. Language/language_user-defined-functions.md#user-defined-functions)

## [Introduction](../3. Language/language_user-defined-functions.md#introduction)
...
- [Single-line](../3. Language/language_user-defined-functions.md#single-line-functions) format, where the function’s header and body occupy only _one_ line of code. This format is best suited for defining compact functions containing only minimal statements or expressions.
- [Multiline](../3. Language/language_user-defined-functions.md#multiline-functions) format, where the _first_ line defines the function’s header, and all _indented_ lines that follow define the function’s body. This format is optimal for functions that require [conditional structures](../3. Language/language_conditional-structures.md), [loops](../3. Language/language_loops.md), or multiple other statements.

Programmers can define functions for use only in a specific script, or create _library functions_ for use in other scripts. Refer to our [Style guide](../4. Writing_Scripts/writing_style-guide.md) for recommendations on where to include function definitions in a source code. To learn more about library functions and their unique requirements, see the [Libraries](../1. Concepts/concepts_libraries.md) page.

Regardless of format, location, or use, several common characteristics and [limitations](../3. Language/language_user-defined-functions.md#limitations) apply to every user-defined function, including the following:

- The function’s definition must be in the [global scope](../6. FAQ/faq_programming.md#what-does-scope-mean). Programmers cannot define functions inside the body of another function or the local blocks of any other structure.
...
- Each written call to the function must have consistent parameter [types](../3. Language/language_type-system.md#types), and therefore consistent _argument_ types, across all executions.
- Each call returns the result of evaluating the _final_ statement or separate expression defined in the function’s body, and that result inherits the _strongest_ [type qualifier](../3. Language/language_type-system.md#qualifiers) used in the call’s calculations. As with parameter types, the call’s returned types must be consistent across executions.
...
## [Structure and syntax](../3. Language/language_user-defined-functions.md#structure-and-syntax)
...
- [export](../../reference manual/keywords/export.md) is the optional keyword for exporting the function from a library, enabling its use in other scripts. See the [Libraries](../1. Concepts/concepts_libraries.md) page to learn more.
...
- `paramQualifier` and `paramType` are qualifier and type _keywords_, which together specify the parameter’s [qualified type](../3. Language/language_type-system.md). Using these keywords is optional in most cases. If the declaration does not include them, the compiler determines the parameter’s type information automatically. See the [Declaring parameter types](../3. Language/language_user-defined-functions.md#declaring-parameter-types) section to learn more.
...
- Because the function’s parameters do not include [type keywords](../3. Language/language_user-defined-functions.md#type-keywords), they inherit the _same_ qualified type as their argument in each separate call.

The following two sections explain the body structure of single-line and multiline functions.

### [Single-line functions](../3. Language/language_user-defined-functions.md#single-line-functions)

A single-line function’s body begins and ends on the _same_ line of code as the header. This format is convenient for defining compact functions that execute only simple statements and do not use [conditional structures](../3. Language/language_conditional-structures.md) or [loops](../3. Language/language_loops.md). The syntax to define a single-line function is as follows:
...
- `functionHeader` declares the function’s _name_ and _parameters_, as explained in the [previous section](../3. Language/language_user-defined-functions.md#structure-and-syntax).
...
- `returnExpression` is the _final_ expression, variable, or [tuple](../3. Language/language_type-system.md#tuples) in the function’s body. Each function call _returns_ the result of evaluating this code.

The following example defines an `add()` function in single-line format. The function includes two parameters named `val1` and `val2`. The body of the function contains a single [+](../../reference manual/operators/+.md) operation that _adds_ or [concatenates](../1. Concepts/concepts_strings.md#concatenation) the parameter values, depending on their [types](../3. Language/language_type-system.md#types). Each call to the function returns the result of that operation:

```pine
add(val1, val2) => val1 + val2
```

A script that includes this function definition can call `add()` with different arguments for `val1` and `val2`. The type of value returned by each call depends on these arguments. For example, the script below executes a few calls to `add()`, then passes their results to the `series`, `title`, and `linewidth` parameters in a call to [plot()](../../reference manual/functions/plot.md):

![image](../images/User-defined-functions-Structure-and-syntax-Single-line-functions-1.ZUSKhP6e_1lYyw4.webp)
...
- The `val1` and `val2` parameters automatically _inherit_ the qualified types of their arguments in each `add()` call, because the function’s header does not [declare their types](../3. Language/language_user-defined-functions.md#declaring-parameter-types) using type and qualifier keywords.
- Although the parameters accept arguments of any type, except for [void](../3. Language/language_type-system.md#void), an `add()` function call compiles successfully only if the arguments have types that are _compatible_ with the [+](../../reference manual/operators/+.md) operator, such as “int”, “float”, or “string”. As shown above, if the arguments are numbers (“int” or “float” values), the function performs _addition_. If they are strings, the function performs _concatenation_.
- The value returned by each `add()` call inherits the _strongest_ [type qualifier](../3. Language/language_type-system.md#qualifiers) used in the calculation. The first two calls return “series” results because both use at least one “series” value. In contrast, the other calls return “const” results, because all values in their calculations are _constants_.
...
For example, the following script contains a `zScore()` function defined in single-line format. The function computes the [z-score](https://en.wikipedia.org/wiki/Standard_score) of a `source` series over `length` bars. Its body declares two variables, `mean` and `sd`, to hold the average and standard deviation of the series. The final expression in the body uses these variables to calculate the function’s returned value. On each bar, the script calls the `zScore()` function using [close](../../reference manual/variables/close.md) as the `source` argument and 20 as the `length` argument, then plots the result:

![image](../images/User-defined-functions-Structure-and-syntax-Single-line-functions-2.BetlAr8S_Z1NQuye.webp)
...
- The `source` parameter requires an “int” or “float” value because its declaration includes the [float](../../reference manual/types/float.md) keyword. The `length` parameter requires an “int” value because its declaration uses the [int](../../reference manual/functions/int.md) keyword. See the [Declaring parameter types](../3. Language/language_user-defined-functions.md#declaring-parameter-types) section to learn more.
- The `//@variable` and `//@function` comments are [annotations](../3. Language/language_script-structure.md#compiler-annotations) that document identifiers in the code. The `//@function` annotation provides documentation for the `zScore()` function. Users can hover over the function’s name in the Pine Editor, or write a function call, to view the annotation’s formatted text in a pop-up window. See the [Documenting functions](../3. Language/language_user-defined-functions.md#documenting-functions) section for more information.

### [Multiline functions](../3. Language/language_user-defined-functions.md#multiline-functions)
...
- `functionHeader` declares the function’s _name_ and _parameters_. See the [Structure and syntax](../3. Language/language_user-defined-functions.md#structure-and-syntax) section above to learn the syntax for function headers.
...
- `returnExpression` is the _final_ statement, expression, variable, or tuple at the end of the body. Each call to the function returns the result of this code. If the end of the function’s body is a comma-separated list of statements or expressions, `returnExpression` is the code at the end of that list. If the final statement is a [conditional structure](../3. Language/language_conditional-structures.md) or [loop](../3. Language/language_loops.md), the function returns that structure’s result.
...
In the following example, we modified the second script from the [Single-line functions](../3. Language/language_user-defined-functions.md#single-line-functions) section to define an equivalent `zScore()` function using the multiline format. The function’s [variable declarations](../3. Language/language_variable-declarations.md) and return expression are now on _separate_ indented lines, and we’ve added comments inside the body to describe the function’s calculations:
...
- We added the [float](../../reference manual/types/float.md) keyword to the `mean` and `sd` declarations to declare their [types](../3. Language/language_type-system.md#types) in the code. The keyword is _optional_ in both variable declarations because the compiler can determine the correct types from the assigned values, but including it helps promote readability.

Programmers often define multiline functions to encapsulate complex or logical tasks involving [loops](../3. Language/language_loops.md) or [conditional structures](../3. Language/language_conditional-structures.md). If the final part of a function’s body contains one of these structures, a call to the function returns the result of evaluating that structure.

For example, the `smoothMedian()` function in the script below calculates the median of a `source` series over `length` bars, then smooths the result using a moving average specified by the `avgType` parameter. The function compares the `avgType` value in a [switch](../../reference manual/keywords/switch.md) statement to select the type of average that it returns. The script calls the function to calculate the median of [close](../../reference manual/variables/close.md) values over 10 bars, smoothed by an [EMA](https://www.tradingview.com/support/solutions/43000592270-exponential-moving-average/) with the same length, and then plots the result on the chart:

![image](../images/User-defined-functions-Structure-and-syntax-Multiline-functions-1.BxkB4CE1_HT9WV.webp)
...
## [Functions that return multiple results](../3. Language/language_user-defined-functions.md#functions-that-return-multiple-results)

Sometimes, a function must perform multiple calculations and return separate results for each one. Programmers can write functions that return _multiple_ values or references by using a [tuple](../3. Language/language_type-system.md#tuples) as the _final statement_ in the function’s body. A tuple is a comma-separated list of expressions enclosed in square brackets (`[]`). Every call to a function with a tuple at the end of its body returns the result of evaluating each listed expression in order.

For example, the `sumDiff()` function below calculates both the sum and difference of the values passed to its `val1` and `val2` parameters. Its body consists of a two-item tuple containing the addition and subtraction [operations](../3. Language/language_operators.md):
...
- The `val1` and `val2` parameters include the [float](../../reference manual/types/float.md) keyword in their declarations. Therefore, both parameters are of the type “float”, and they accept only “float” or “int” arguments. See the [Declaring parameter types](../3. Language/language_user-defined-functions.md#declaring-parameter-types) section for more information.

Because the end of the function’s body is a two-item tuple, each call to the function always returns two separate values. To assign the function’s results to variables, the script must use a [tuple declaration](../3. Language/language_variable-declarations.md#tuple-declarations) containing one _new_ variable for each returned item. It is _not_ possible to reassign existing tuples or to use previously declared variables within them.
...
![image](../images/User-defined-functions-Functions-that-return-multiple-results-1.CVZCb-PO_1728g7.webp)
...
In some cases, a script might not require _all_ the results from the tuple returned by a function call. Instead of writing unique identifiers for every variable in the tuple declaration, programmers can [use an underscore](../3. Language/language_variable-declarations.md#using-an-underscore-as-an-identifier) as the identifier for each variable that the script does not require. All variables with the name `_` are _not usable_ in the script’s calculations.

For example, if we change `sum` to `_` in the previous script’s tuple declaration, the first value returned by the `sumDiff()` call is _inaccessible_ to the script. Attempting to use the identifier elsewhere in the code causes a _compilation error_, as shown by the first [plot()](../../reference manual/functions/plot.md) call in the script version below:
...
## [Declaring parameter types](../3. Language/language_user-defined-functions.md#declaring-parameter-types)
...
The sections below explain how type and qualifier keywords affect the behavior of function parameters. For detailed information about Pine’s types and qualifiers, refer to the [Type system](../3. Language/language_type-system.md) page.

### [Type keywords](../3. Language/language_user-defined-functions.md#type-keywords)

Parameter declarations prefixed by _type keywords_ — such as [int](../../reference manual/functions/int.md), [float](../../reference manual/types/float.md), [string](../../reference manual/types/string.md), or [label](../../reference manual/types/label.md) — declare the [types](../3. Language/language_type-system.md#types) of data that the parameters represent in any function call. If a parameter declaration includes a type keyword, it accepts only arguments of that type, or arguments that Pine can automatically [cast](../3. Language/language_type-system.md#type-casting) to that type.

If a function parameter does _not_ have a type keyword in its declaration, its type is initially _undefined_. In each separate call to the function, the parameter automatically inherits the _same_ type as its specified argument. In other words, the parameter can take on _any type_, except for [void](../3. Language/language_type-system.md#void), depending on the function call.
...
![image](../images/User-defined-functions-Declaring-parameter-types-Type-keywords-1.zDkkXyfc_ZDD8OY.webp)
...
We can restrict the `source` parameter’s type, and thus the arguments it can accept, by including a type keyword in its declaration. For example, in the modified script below, we added the [int](../../reference manual/functions/int.md) keyword to the declaration to specify that the parameter’s type is “int”. With this change, the last three `pass()` calls now cause a compilation error, because the parameter no longer allows “string”, “color”, or “plot\_display” arguments:
...
- If a parameter declaration includes a type keyword _without_ a qualifier keyword, such as [const](../../reference manual/types/const.md) or [simple](../../reference manual/types/simple.md), the compiler automatically sets the qualifier to _“series”_ or _“simple”_, depending on the function’s calculations. In this example, the `source` parameter’s type is “series int”, because the function’s logic does not require a “simple” or weaker qualifier. See the [Qualifier keywords](../3. Language/language_user-defined-functions.md#qualifier-keywords) section to learn more.
...
- The parameter’s default argument is an [`na` value](../3. Language/language_user-defined-functions.md#language/type-system/#na-value).
- The parameter declaration includes a _qualifier keyword_.
- The function definition includes the [export](../../reference manual/keywords/export.md) keyword. Exported functions require declared types for _every_ parameter. See the [Libraries](../1. Concepts/concepts_libraries.md) page to learn more.
- The function definition includes the [method](../../reference manual/keywords/method.md) keyword, and the parameter is the _first_ one listed in the header. See the [User-defined methods](../3. Language/language_methods.md#user-defined-methods) section of the [Methods](../3. Language/language_methods.md) page for more information.

### [Qualifier keywords](../3. Language/language_user-defined-functions.md#qualifier-keywords)

A _qualifier keyword_ ( [const](../../reference manual/types/const.md), [simple](../../reference manual/types/simple.md), or [series](../../reference manual/types/series.md)) preceding a [type keyword](../3. Language/language_user-defined-functions.md#type-keywords) in a parameter declaration specifies the parameter’s [type qualifier](../3. Language/language_type-system.md#qualifiers). The keyword also indicates when the _argument_ for the parameter must be accessible, and whether that argument can _change_ across bars:

`const`

The parameter has the “const” qualifier. Its argument must be available at _compile time_, and that argument cannot change during any execution. Only the parameters of _non-exported_ functions can use the [const](../../reference manual/types/const.md) keyword.
...
The parameter has the “series” qualifier. It can accept an argument with _any_ type qualifier, because “series” is the _highest_ one in Pine’s [qualifier hierarchy](../3. Language/language_type-system.md#qualifiers). The argument for the parameter in each function call _can change_ on any execution.
...
The script below calls `pass()` using an “int” value with the “const” qualifier, then uses the returned value as the `length` argument in a call to [ta.ema()](../../reference manual/functions/ta.ema.md) and plots the result. This script compiles successfully because our `pass()` call returns the same qualified type as its argument (“const int”), and the `length` parameter of [ta.ema()](../../reference manual/functions/ta.ema.md) can accept a value of that type:

![image](../images/User-defined-functions-Declaring-parameter-types-Qualifier-keywords-1.UmWjQFYB_2kXXpm.webp)
...
If we add [int](../../reference manual/functions/int.md) to the `source` declaration, the parameter then requires an “int” value, but it **does not** directly inherit the _same_ type qualifier as its argument. Instead, the compiler first checks if it can assign _“series”_ to the parameter, then tries using _“simple”_ if “series” does not work.

Our `pass()` function does not use the `source` parameter in any local function calls that require a “simple int” value, so the compiler sets its qualifier to **“series”**. Consequently, the function’s returned type is always _“series int”_, even if the `source` argument is a “const” value. Adding this change to the previous script thus causes a _compilation error_, because the `length` parameter of [ta.ema()](../../reference manual/functions/ta.ema.md) cannot accept a “series” argument; only “simple” or weaker qualifiers are allowed:
...
We can restrict the type qualifier of our function’s `source` parameter by adding a _qualifier keyword_ to its declaration. In the script version below, we prefixed the declaration with the [simple](../../reference manual/types/simple.md) keyword. Now, the `source` parameter’s type is _“simple int”_ instead of “series int”. With this change, the script _does not_ cause an error, because the `pass()` function’s returned type is now compatible with the `length` parameter of [ta.ema()](../../reference manual/functions/ta.ema.md):

![image](../images/User-defined-functions-Declaring-parameter-types-Qualifier-keywords-2.3l-V8A-N_Z1fiiVi.webp)
...
For some types of calculations, using “series” values might not cause a compilation error, but if the values do not remain consistent across all bars, the calculations produce incorrect or unintended results. When wrapping such calculations in a function, declaring the relevant parameters with the [simple](../../reference manual/types/simple.md) keyword — or [const](../../reference manual/types/const.md) when appropriate — ensures that only _unchanging_ arguments are allowed in each call, preventing unintended behavior.

Let’s look at an example. The following `calcAvg()` function calculates a moving average of a `source` series over `length` bars. The function compares the value of its `avgType` parameter in a [switch](../../reference manual/keywords/switch.md) statement to select a built-in `ta.*()` call to use for the average calculation:
...
The compiler raises a _warning_ about this function’s structure inside the Pine Editor, because using `calcAvg()` with a _dynamic_`avgType` argument can cause _unintended results_. If the `ta.*()` call executed by the function changes on any bar, it affects the _history_ of values used in the average calculations. See the [Time series in scopes](../3. Language/language_execution-model.md#time-series-in-scopes) section of the [Execution model](../3. Language/language_execution-model.md) page for advanced details about this behavior.
...
![image](../images/User-defined-functions-Declaring-parameter-types-Qualifier-keywords-3.CmT8Y_eI_2110wI.webp)
...
To ensure that any `calcAvg()` call calculates consistent averages without modifying the function’s logic, we can prevent it from using _dynamic_`avgType` arguments by prefixing the parameter declaration with the [simple](../../reference manual/types/simple.md) keyword. With this change, the compiler does _not_ raise a warning about the function — as long as the script evaluates calls to the function on _every bar_ — because any `calcAvg()` call must always use the same `ta.*()` function. Now, if the script attempts to pass a “series string” value to `avgType`, a compilation error occurs:
...
## [Documenting functions](../3. Language/language_user-defined-functions.md#documenting-functions)

Pine Script features [annotations](../3. Language/language_script-structure.md#compiler-annotations) that programmers can use to document a function, its parameters, and its result directly in the source code. Annotations are _comments_ that issue special _instructions_ to the compiler or the Pine Editor. The editor can display the formatted text from function annotations in a pop-up window as users work with the function in their code. Additionally, the “Publish script” window uses the text from function annotations to generate default descriptions for [libraries](../1. Concepts/concepts_libraries.md).

To annotate a function, add comment lines containing valid annotation syntax directly above the function’s header. Each annotation comment must start with `@`, immediately followed by the keyword that indicates its purpose. Below is the syntax for all annotations that apply exclusively to function and [method](../3. Language/language_methods.md#user-defined-methods) definitions:
...
## [Function scopes](../3. Language/language_user-defined-functions.md#function-scopes)

All variables, expressions, and statements have a designated [scope](../6. FAQ/faq_programming.md#what-does-scope-mean), which refers to the part of the script where they are defined and accessible. Every script has one _global_ scope and zero or more _local_ scopes.
...
For example, the following script defines a function named `myFun()`. The function’s body contains a variable named `result`. The script attempts to use a [plot()](../../reference manual/functions/plot.md) call, in the global scope, to display the value of that variable. This script causes a compilation error, because the global scope _cannot_ access the function’s _local_ identifiers, and there is no global variable named `result`:
...
The only way for the script to use the function’s code is to execute a _function call_. Additionally, because the code in the function definition is _inaccessible_ to other scopes, the script can declare separate variables in other parts of the code with the _same_ identifiers as the function’s local variables and parameters. For example, the following script executes a call to the `myFun()` function and assigns its returned value to a new, _global_ variable _also_ named `result`. The [plot()](../../reference manual/functions/plot.md) call in this script does not cause an error, because only the global `result` variable is available to that call:

![image](../images/User-defined-functions-Function-scopes-1.WDscmOKC_Z1HYWMK.webp)
...
![image](../images/User-defined-functions-Function-scopes-2.CcNLbFZI_Y6NNV.webp)
...
### [Scope of a function call](../3. Language/language_user-defined-functions.md#scope-of-a-function-call)

A function’s definition acts as a _template_ for each function call. Every call _written_ in the source code establishes a _separate_ local scope using that definition. The parameters, variables, and expressions created for each function call are _unique_ to that call, and they leave _independent_ historical trails in the script’s [time series](../3. Language/language_execution-model.md#time-series). Because each written call has a separate scope, with an independent history, no two calls to the same function directly affect each other.

For example, the script below contains a user-defined function named `accumulate()`. The function’s structure declares a _persistent_ variable named `total` with an initial value of 0, then adds the value of its `source` parameter to that variable and returns the result. The script uses two separate `accumulate()` calls inside a [ternary operation](../3. Language/language_operators.md#-ternary-operator), and then plots the result returned by that expression. The operation triggers the first written call on every _third_ bar, and the second on all other bars. Both calls use a value of 1 as their `source` argument:
...
![image](../images/User-defined-functions-Function-scopes-Scope-of-a-function-call-1.BvL7oJKj_Z1uOD5g.webp)

It is crucial to emphasize that each function call _written_ in the source code has **one** unique scope created from the function’s definition. Repeated evaluations of the same written call **do not** create additional scopes with separate local variables and history. For example, a function call written in the body of a [loop](../3. Language/language_loops.md) performs calculations using the _same_ local series on _every_ iteration; it does _not_ calculate on different versions of those series for each separate iteration.

The following script demonstrates this behavior. The source code includes two written calls to our previous `accumulate()` function. The script evaluates the first call in the global scope, and the second inside the body of a [for](../../reference manual/keywords/for.md) loop that performs 10 iterations per execution. It then plots the results of both calls in a separate pane:
...
- It has the same number of required parameters as another overload, but at least _one_ required parameter has a different [qualified type](../3. Language/language_type-system.md) than the one declared at the _same position_ in the other overload’s header.

The following code block defines three overloads of a custom `negate()` function. Each overload has a different parameter type and performs a distinct task. The first overload uses the [-](../../reference manual/operators/-.md) operator to negate a “float” or “int” value; the second uses the [not](../../reference manual/keywords/not.md) operator to negate a “bool” value; and the third calculates the sRGB negative of a “color” value using the built-in `color.*()` functions:
...
- The `value` parameter of each `negate()` overload automatically inherits the _“series”_ qualifier, because its declaration includes a [type keyword](../3. Language/language_user-defined-functions.md#type-keywords) _without_ a [qualifier keyword](../3. Language/language_user-defined-functions.md#qualifier-keywords), and the overload does not use the parameter in a local function call that requires a “simple” or weaker qualifier.
- We included [annotations](../3. Language/language_script-structure.md#compiler-annotations) to [document](../3. Language/language_user-defined-functions.md#documenting-functions) each separate overload. As a user writes a `negate()` call, the Pine Editor shows the documentation for _one_ of the overloads in a pop-up window. While the window is open, the user can view the documentation for the _other_ overloads by using the Up and Down arrow keys.

With our function overloads defined, we can use `negate()` calls for different type-specific tasks. The script below uses a call to each overload. First, it uses the second overload to calculate the opposite of the condition `close > open`. It then uses the negated condition to determine the value of a plotted series. The plotted value is the result of `negate(close)` if the condition is `true`, and the value of [close](../../reference manual/variables/close.md) otherwise. The script colors the plot using the negative of the chart’s background color (`negate(chart.bg_color)`):

![image](../images/User-defined-functions-Function-overloading-1.BTe80Gur_ZyArF7.webp)
...
![image](../images/User-defined-functions-Function-overloading-2.CU45Rhl7_JVeGn.webp)
...
## [Limitations](../3. Language/language_user-defined-functions.md#limitations)

This section explains several key limitations common to all user-defined functions. These same limitations also apply to [user-defined methods](../3. Language/language_methods.md#user-defined-methods).

### [No global-only built-in function calls](../3. Language/language_user-defined-functions.md#no-global-only-built-in-function-calls)

The body of a user-defined function can contain calls to most other functions or [methods](../3. Language/language_methods.md), or previously defined [overloads](../3. Language/language_user-defined-functions.md#function-overloading) with the same function name. However, a function _cannot_ use calls to the following _built-in_ functions inside its scope:

- Declaration statements: [indicator()](../../reference manual/functions/indicator.md), [strategy()](../../reference manual/functions/strategy.md), and [library()](../../reference manual/functions/library.md).
- Plot-related functions: [plot()](../../reference manual/functions/plot.md), [hline()](../../reference manual/functions/hline.md), [fill()](../../reference manual/functions/fill.md), [plotshape()](../../reference manual/functions/plotshape.md), [plotchar()](../../reference manual/functions/plotchar.md), [plotarrow()](../../reference manual/functions/plotarrow.md), [plotbar()](../../reference manual/functions/plotbar.md), [plotcandle()](../../reference manual/functions/plotcandle.md), [barcolor()](../../reference manual/functions/barcolor.md), [bgcolor()](../../reference manual/functions/bgcolor.md), and [alertcondition()](../../reference manual/functions/alertcondition.md).

Global variables can be assigned values returned by functions in order to be used in calls to the above functions. However, the calls to these functions are allowed only in the script’s [global scope](../6. FAQ/faq_programming.md#what-does-scope-mean), outside the operands of [ternary operations](../3. Language/language_operators.md#-ternary-operator) or other conditional expressions.

### [Consistent types for each call](../3. Language/language_user-defined-functions.md#consistent-types-for-each-call)

Each written call to a user-defined function must have _consistent_ parameter types and return types during each evaluation of that call. The call _cannot_ accept varying argument types throughout the script’s runtime, nor can it change its return type. Similar limitations also apply to other structures and expressions, including [loops](../3. Language/language_loops.md), [conditional structures](../3. Language/language_conditional-structures.md), and ternary operations.

### [Cannot modify global variables or parameters](../3. Language/language_user-defined-functions.md#cannot-modify-global-variables-or-parameters)

The body of a user-defined function cannot use the _reassignment_ or _compound assignment_ [operators](../3. Language/language_operators.md) to modify variables declared in the _global scope_. Likewise, it cannot use these operators to reassign function _parameters_. The value or reference assigned to a global variable or the parameter of a function call _cannot change_ while the script evaluates the call. See the [Function scopes](../3. Language/language_user-defined-functions.md#function-scopes) section for an example.

### [No nested definitions](../3. Language/language_user-defined-functions.md#no-nested-definitions)

The body of a user-defined function _cannot_ contain the definition of another user-defined function. Likewise, [loops](../3. Language/language_loops.md) and [conditional structures](../3. Language/language_conditional-structures.md) cannot include function definitions in their local blocks. Function and [method](../3. Language/language_methods.md#user-defined-methods) definitions are allowed only in the script’s _global scope_.
...
### [No recursive functions](../3. Language/language_user-defined-functions.md#no-recursive-functions)

In contrast to functions in some programming languages, user-defined functions in Pine Script _cannot_ contain calls to _themselves_ within their bodies. Instead of using recursive function structures, programmers can replace those structures with equivalent [loop](../3. Language/language_loops.md) calculations.
...
Instead of attempting to use recursion, we can reformat the function’s structure to perform the necessary calculations using _iteration_. The example version below achieves our intended calculations using a [while](../../reference manual/keywords/while.md) loop and no recursive `gcd()` calls:
...
[Previous 
**Built-ins**](../3. Language/language_built-ins.md) [Next 
