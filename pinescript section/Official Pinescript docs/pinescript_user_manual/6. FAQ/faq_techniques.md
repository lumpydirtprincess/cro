![](../6. FAQ/faq_techniques.md)

# [Techniques](../6. FAQ/faq_techniques.md#techniques)

## [How can I prevent the “Bar index value of the ​`x`​ argument is too far from the current bar index. Try using ​`time`​ instead” and “Objects positioned using xloc.bar\_index cannot be drawn further than X bars into the future” errors?](../6. FAQ/faq_techniques.md#how-can-i-prevent-the-bar-index-value-of-the-x-argument-is-too-far-from-the-current-bar-index-try-using-time-instead-and-objects-positioned-using-xlocbar_index-cannot-be-drawn-further-than-x-bars-into-the-future-errors)

Both these errors occur when creating objects too distant from the current bar. An x point on a [line](../2. Visuals/visuals_lines-and-boxes.md#lines), [label](../2. Visuals/visuals_text-and-shapes.md#labels), or [box](../2. Visuals/visuals_lines-and-boxes.md#boxes) can not be more than 9999 bars in the past or more than 500 bars in the future relative to the bar on which the script draws it.

Scripts _can_ draw objects beyond these limits, however, using [xloc.bar\_time](../../reference manual/variables/xloc.bar_time.md) instead of the `xloc` parameter, and [time](../../reference manual/functions/time.md) as an alternative to [bar\_index](../../reference manual/variables/bar_index.md) for the `x` arguments.
...
## [How can I update the right side of all lines or boxes?](../6. FAQ/faq_techniques.md#how-can-i-update-the-right-side-of-all-lines-or-boxes)

Scripts can update the `x2` value of all lines or boxes by storing them in an array and using a [for…in](../../reference manual/operators/for...in.md) loop to iterate over each object. Update the `x2` value using the [line.set\_x2()](../../reference manual/functions/line.set_x2.md) or [box.set\_right()](../../reference manual/functions/box.set_right.md)
functions.
...
As an alternative to adding new drawings to a custom array, scripts can use the appropriate built-in variable that collects all instances of a drawing type. These arrays use the `<drawingNamespace>.all` naming scheme: for example, scritps can access all drawn labels by referring to [label.all](../../reference manual/variables/label.all.md), all polylines with [polyline.all](../../reference manual/variables/polyline.all.md), etc. Scripts can iterate over these arrays in the same way as with custom arrays.

This example implements gets the same result using the [line.all](../../reference manual/variables/line.all.md) built-in array instead:
...
## [How to avoid repainting when _not_ using the ​`request.security()`​ function?](../6. FAQ/faq_techniques.md#how-to-avoid-repainting-when-not-using-the-requestsecurity-function)

Scripts can give deceptive output if they [repaint](../1. Concepts/concepts_repainting.md) by behaving differently on historical and elapsed realtime bars. This type of repainting is most commonly caused by requesting data from another context using the [request.security()](../../reference manual/functions/request.security.md) function.

Scripts can also change their output during a realtime bar, as the [volume](../../reference manual/variables/volume.md), [close](../../reference manual/variables/close.md), [high](../../reference manual/variables/high.md), and [low](../../reference manual/variables/low.md) values change. This form of repainting is not normally deceptive or detrimental.
...
- Set alerts to fire on bar close. Read more about repainting alerts in the FAQ entry [Why is my alert firing at the wrong time?](../6. FAQ/faq_alerts.md#why-is-my-alert-firing-at-the-wrong-time)
- Use the [open](../../reference manual/variables/open.md) in calculations instead of the [close](../../reference manual/variables/close.md).
...
## [How can I trigger a condition n bars after it last occurred?](../6. FAQ/faq_techniques.md#how-can-i-trigger-a-condition-n-bars-after-it-last-occurred)

Using the [ta.barssince()](../../reference manual/functions/ta.barssince.md) function, scripts can implement a condition when a certain number of bars have elapsed since the last occurrence of that condition.

The following example script uses the `cond` condition to plot a blue star when the [close](../../reference manual/variables/close.md) value is greater than the [open](../../reference manual/variables/open.md) value for two consecutive bars. Then, the `trigger` variable is true only if the `cond` condition is already true _and_ the number of bars elapsed since `cond` was last true is greater than `lengthInput`. The script plots a red “O” on the chart, overlaying the blue star, each time these conditions are met. The Data Window displays the count since `cond` was last true.

![image](../images/Techniques-How-can-i-trigger-a-condition-only-when-a-number-of-bars-have-elapsed-since-the-last-condition-occured-1.Dt02f4F1_1WERNm.webp)
...
## [How can my script identify what chart type is active?](../6. FAQ/faq_techniques.md#how-can-my-script-identify-what-chart-type-is-active)

Various boolean [built-in](../3. Language/language_built-ins.md) variables within the `chart.*` namespace enable a script to detect the type of chart it is running on.
...
## [How can I plot the highest and lowest visible candle values?](../6. FAQ/faq_techniques.md#how-can-i-plot-the-highest-and-lowest-visible-candle-values)

To plot the highest [high](../../reference manual/variables/high.md) and lowest [low](../../reference manual/variables/low.md) within the range of visible bars, a script can use the [chart.left\_visible\_bar\_time](../../reference manual/variables/chart.left_visible_bar_time.md) and [chart.right\_visible\_bar\_time](../../reference manual/variables/chart.right_visible_bar_time.md) built-ins. These variables allow the script to identify the times of the earliest and latest visible bars on the chart and calculate the maximum or minimum values within that range.
...
The following example script uses functions from this library to create two horizontal [lines](../2. Visuals/visuals_lines-and-boxes.md#lines) on the chart, signifying the highest and lowest price points within the range of visible bars. The script draws [labels](../2. Visuals/visuals_text-and-shapes.md#labels) for these lines, displaying both the price and the corresponding timestamp for each high and low point. As the chart is manipulated through scrolling or zooming, these lines and labels dynamically update to reflect the highest and lowest values of the newly visible bars:

![image](../images/Techniques-How-can-i-plot-the-charts-visible-high-and-low-1.C8tRKhQr_1xO5Xm.webp)
...
- Values derived from visible chart variables can change throughout the script’s runtime. To accurately reflect the entire visible range, the script defers drawing the lines until the last bar (using [barstate.islast](../../reference manual/variables/barstate.islast.md)).
- Because the visible chart values are defined in the [global scope](../6. FAQ/faq_programming.md#what-does-scope-mean), _outside_ the local block defined by [barstate.islast](../../reference manual/variables/barstate.islast.md), the functions process the entire dataset before determining the final high and low values.
...
## [How to remember the last time a condition occurred?](../6. FAQ/faq_techniques.md#how-to-remember-the-last-time-a-condition-occurred)

Scripts can store the number of bars between the current bar and a bar on which a condition occurred in various ways:

- Using [ta.barssince()](../../reference manual/functions/ta.barssince.md). This built-in function is the simplest way to track the distance from the condition.
- Manually replicating the functionality of [ta.barssince()](../../reference manual/functions/ta.barssince.md) by initializing the distance to zero when the condition occurs, then incrementing it by one on each bar, resetting it if the condition occurs again.
- Saving the [bar\_index](../../reference manual/variables/bar_index.md) when the condition occurs, and calculating the difference from the current [bar\_index](../../reference manual/variables/bar_index.md).

Programmers can then use the number of bars with the [history-referencing operator \[\]](../../reference manual/operators/[].md) to retrieve the value of a variable, such as the [close](../../reference manual/variables/close.md), on that bar.
...
## [How can I plot the previous and current day’s open?](../6. FAQ/faq_techniques.md#how-can-i-plot-the-previous-and-current-days-open)
...
### [Using ​`timeframe.change()`​](../6. FAQ/faq_techniques.md#using-timeframechange)

The [timeframe.change()](../../reference manual/functions/timeframe.change.md) function identifies when a bar in a specified timeframe opens. When a new daily bar opens, the following example script first copies the existing daily opening value to the variable for the previous day, and then updates the opening price for the current day.
...
- For some markets and instrument types, the intraday data and the daily data is expected to differ. For example, the US exchanges like NASDAQ and NYSE include more trades in daily bars than in intraday ones, which results in different OHLC values between intraday and daily data, and in daily volume being far greater than intraday one. As a result, the first [open](../../reference manual/variables/open.md) of a trading session on an intraday chart can differ from the [open](../../reference manual/variables/open.md) of its respective 1D candle.

### [Using ​`request.security()`​](../6. FAQ/faq_techniques.md#using-requestsecurity)

To match the values on the chart with the values on higher timeframe charts, it’s necessary to access the higher timeframe data feeds. Scripts can achieve this by using the [request.security()](../../reference manual/functions/request.security.md) function.

The following example script requests two data feeds from a higher timeframe. To reduce the risk of [repainting](../1. Concepts/concepts_repainting.md), we use only confirmed values for historical bars. The script plots confirmed values retroactively on each preceding day when a new day begins. For the real-time bar of the higher timeframe, which represents the current day, we draw a separate set of lines. The realtime lines can change during the day. While this type of repainting is not apparent here when using the opening price, which does not change after the bar opens, it is more obvious for scripts that use the closing price, which takes the current price until the bar closes.
...
### [Using ​`timeframe`​](../6. FAQ/faq_techniques.md#using-timeframe)
...
If scripts include the `timeframe` parameter in the [indicator](../../reference manual/functions/indicator.md) declaration, the user can choose the timeframe in which the script runs. The script can set a default timeframe.
...
## [How can I count the occurrences of a condition in the last x bars?](../6. FAQ/faq_techniques.md#how-can-i-count-the-occurrences-of-a-condition-in-the-last-x-bars)
...
However, this method is inefficient, because it examines all bars in range _again_ on every bar, even though it already examined all but the last bar.

In general, using unnecessary, large, or nested [for](../../reference manual/keywords/for.md) loops can result in slower processing and longer chart loading times.

The simplest and most efficient method is to use the built-in [math.sum()](../../reference manual/functions/math.sum.md) function, and pass it a conditional series to count. This function maintains a running total of the count as each bar is processed, and can take a [simple](../3. Language/language_type-system.md#simple) or [series](../3. Language/language_type-system.md#series) length.

The following example script uses both of these calculation methods. It also uses a series length that adjusts for the first part of the chart, where the number of bars available is less than the length. This way, the functions do not return [na](../../reference manual/functions/na.md) values.

![image](../images/Techniques-How-can-i-count-the-occurences-of-a-condition-in-the-last-x-bars-1.DjfPEBjE_Z2hO669.webp)
...
![image](../images/Techniques-How-can-i-implement-an-on-off-switch-1.B-YIPmBl_ZEO1dj.webp)
...
## [How can I alternate conditions?](../6. FAQ/faq_techniques.md#how-can-i-alternate-conditions)
...
![image](../images/Techniques-How-can-i-allow-transitions-from-condition-ab-or-ba-but-not-aa-nor-bb-1.9yOp6mtx_1ideIU.webp)
...
- The script uses an [enum](../3. Language/language_enums.md#enums) variable with three possible values to store the type of the last pivot and to decide whether to confirm subsequent pivots.
...
- A pair of boolean variables can replicate this behavior, with careful handling. See the FAQ entry [“How can I accumulate a value for two exclusive states?”](../6. FAQ/faq_techniques.md#how-can-i-accumulate-a-value-for-two-exclusive-states) for an example of using two boolean values in this way.
- A string variable can also do the same thing. The advantage of an [enum](../3. Language/language_enums.md#enums) over a string is that all possible allowed values are known, thus avoiding the case where a condition tests for a value that is misspelled, outdated or otherwise not relevant. Such a test silently fails in every possible case, and the corresponding logic never runs. Such tests can therefore cause bugs that are difficult to find.

## [Can I merge two or more indicators into one?](../6. FAQ/faq_techniques.md#can-i-merge-two-or-more-indicators-into-one)
...
## [How can I rescale an indicator from one scale to another?](../6. FAQ/faq_techniques.md#how-can-i-rescale-an-indicator-from-one-scale-to-another)
...
The example script below uses a `rescale()` function to rescale RSI values, and a `normalize()` function to normalize [Commodity Channel Index (CCI)](https://www.tradingview.com/support/solutions/43000502001-commodity-channel-index-cci/) and [volume](../../reference manual/variables/volume.md) values. Although normalizing is an imperfect solution, it is more complete than using [ta.lowest()](../../reference manual/functions/ta.lowest.md) and [ta.highest()](../../reference manual/functions/ta.highest.md), because it uses the minimum and maximum values for the complete set of elapsed bars instead of a subset of fixed length.
...
![image](../images/Techniques-How-can-i-rescale-an-indicator-from-one-scale-to-another-1.C3veEcNW_Z56Pg1.webp)

## [How can I calculate my script’s run time?](../6. FAQ/faq_techniques.md#how-can-i-calculate-my-scripts-run-time)

Programmers can measure the time that a script takes to run and see detailed information about which parts of the code take longest in the Pine Profiler. See the section of the User Manual on [Profiling and optimization](../4. Writing_Scripts/writing_profiling-and-optimization.md) for more information.

## [How can I save a value when an event occurs?](../6. FAQ/faq_techniques.md#how-can-i-save-a-value-when-an-event-occurs)

To save a value when an event occurs, use a _persistent variable_. Scripts declare persistent variables by using the [var](../3. Language/language_variable-declarations.md#var) keyword. Such variables are initialized only once, at [bar\_index](../../reference manual/variables/bar_index.md) zero, instead of on each bar, and maintain the same value after that unless changed.

In the following example script, the [var](../3. Language/language_variable-declarations.md#var) keyword allows the `priceAtCross` variable to maintain its value between bars until a crossover event occurs, when the script updates the variable with the current close price. The [:=](../3. Language/language_operators.md#-reassignment-operator) reassignment operator ensures that the global variable `priceAtCross` is modified. Using the [=](../3. Language/language_operators.md#-assignment-operator) assignment operator instead would create a new local variable that is inaccessible outside the [if](../3. Language/language_conditional-structures.md#if-structure) block. The new local variable would have the same name as the global variable, which is called _shadowing_. The compiler warns about shadow variables.
...
## [How can I count touches of a specific level?](../6. FAQ/faq_techniques.md#how-can-i-count-touches-of-a-specific-level)
...
The following example script records a value of 1 in a series whenever a touch occurs, and uses the [math.sum()](../../reference manual/functions/math.sum.md) function to count these instances within the last `touchesLengthInput` bars. This script displays the median and touches on the chart using the `force_overlay` parameter of the `plot*()` functions, and displays the count in a separate pane.

![image](../images/Techniques-How-can-i-count-touches-of-a-specific-level-1.zhHd3SG6_Z2rjWCT.webp)
...
## [How can I know if something is happening for the first time since the beginning of the day?](../6. FAQ/faq_techniques.md#how-can-i-know-if-something-is-happening-for-the-first-time-since-the-beginning-of-the-day)

One way is to use the [ta.barssince()](../../reference manual/functions/ta.barssince.md) function to check if the number of bars since the last occurrence of a condition, plus one, is greater than the number of bars since the beginning of the new day.
...
![image](../images/Techniques-How-can-know-if-something-is-happening-for-the-first-time-since-the-beginning-of-the-day-1.CvNLhI4e_Z1InH46.webp)
...
## [How can I optimize Pine Script code?](../6. FAQ/faq_techniques.md#how-can-i-optimize-pine-script-code)

Optimizing Pine Script code can make scripts run faster and use less memory. For large or complex scripts, optimization can avoid scripts reaching the [computational limits](../4. Writing_Scripts/writing_limitations.md).

The [Pine Profiler](../4. Writing_Scripts/writing_profiling-and-optimization.md) analyzes all significant code in a script and displays how long each line or block takes to run. Before optimizing code, run the Pine Profiler to identify which parts of the code to optimize first. The Pine Profiler section of the User Guide contains an extensive discussion of [how to optimize code](../4. Writing_Scripts/writing_profiling-and-optimization.md#optimization). In addition, consider the following tips:

- Use strategy scripts only to model trades. Otherwise, use indicator scripts, which are faster.
- Become familiar with the Pine [execution model](../3. Language/language_execution-model.md) and [time series](../3. Language/language_execution-model.md#time-series) to structure code effectively.
- Declare variables with the [var](../3. Language/language_variable-declarations.md#var) keyword when initialization involves time-consuming operations like complex functions, arrays, objects, or string manipulations.
...
- Using built-in functions is usually faster than writing custom functions that do the same thing. Sometimes, alternative logic can be more efficient than using standard functions. For example, use a persistent variable when an event occurs, to avoid using [ta.valuewhen()](../../reference manual/functions/ta.valuewhen.md), as described in the FAQ entry [How can I save a value when an event occurs?](../6. FAQ/faq_techniques.md#how-can-i-save-a-value-when-an-event-occurs). Or save the [bar\_index](../../reference manual/variables/bar_index.md) when a condition occurs to avoid using [ta.barssince()](../../reference manual/functions/ta.barssince.md), as described in the FAQ entry [How to remember the last time a condition occurred?](../6. FAQ/faq_techniques.md#how-to-remember-the-last-time-a-condition-occurred).

## [How can I access a stock’s financial information?](../6. FAQ/faq_techniques.md#how-can-i-access-a-stocks-financial-information)

In Pine, the [request.financial()](../../reference manual/functions/request.financial.md) function can directly request financial data.

On the chart, users can open financial indicators in the “Financials” section of the “Indicators, Metrics & Strategies” window.

## [How can I find the maximum value in a set of events?](../6. FAQ/faq_techniques.md#how-can-i-find-the-maximum-value-in-a-set-of-events)

Finding the maximum value of a variable that has a meaningful value _on every bar_, such as the [high](../../reference manual/variables/high.md) or [low](../../reference manual/variables/low.md) in price, is simple, using the [ta.highest()](../../reference manual/functions/ta.highest.md) function.

However, if the values do not occur on every bar, we must instead store each value when it occurs and then find the maximum. The most flexible way to do this is by using an [array](../3. Language/language_arrays.md).

The following example script stores pivot highs in a fixed-length array. The array is managed as a [queue](../3. Language/language_arrays.md#using-an-array-as-a-queue): the script adds new pivots to the end, and removes the oldest element from the array. To identify the highest value among the stored pivots, we use the [array.max()](../../reference manual/functions/array.max.md) function and plot this maximum value on the chart. Additionally, we place markers on the chart to indicate when the pivots are detected, and the bars where the pivots occurred. By definition, these points are not the same, because a pivot is only confirmed after a certain number of bars have elapsed.
...
## [How can I display plot values in the chart’s scale?](../6. FAQ/faq_techniques.md#how-can-i-display-plot-values-in-the-charts-scale)
...
![image](../images/Techniques-How-can-i-display-plot-values-in-the-charts-scale-1.DoK1stsV_1hol50.webp)

## [How can I reset a sum on a condition?](../6. FAQ/faq_techniques.md#how-can-i-reset-a-sum-on-a-condition)

To sum a series of values, initialize a persistent variable by using the [var](../3. Language/language_variable-declarations.md#var) keyword to the track the sum. Then use a logical test to reset the values when a condition occurs.

In the following example script, we initialize a persistent variable called `cumulativeVolume` to track the sum of the [volume](../../reference manual/variables/volume.md). Then we reset it to zero on a Moving Average Convergence/Divergence (MACD) cross up or down.

We plot the cumulative [volume](../../reference manual/variables/volume.md) on the chart, as well as arrows to show the MACD crosses.

![image](../images/Techniques-How-can-i-reset-a-sum-on-a-condition-1.-sS8zw0l_2pbPm7.webp)
...
- In the [ta.macd()](../../reference manual/functions/ta.macd.md) function call, we only require two of the three values returned in the [tuple](../3. Language/language_type-system.md#tuples). To avoid unnecessary variable declarations, we assign the third tuple value to an underscore. Here, [the underscore acts like a dummy variable](../3. Language/language_variable-declarations.md#using-an-underscore-_-as-an-identifier).

## [How can I accumulate a value for two exclusive states?](../6. FAQ/faq_techniques.md#how-can-i-accumulate-a-value-for-two-exclusive-states)

Consider a simple indicator defined by two exclusive states: _buy_ and _sell_. The indicator cannot be in both _buy_ and _sell_ states simultaneously. In the _buy_ state, the script accumulates the [volume](../../reference manual/variables/volume.md) of shares being traded. In the _sell_ state, the accumulation of volume begins again from zero.

There are different ways to code this kind of logic. See the FAQ entry [“How can I alternate conditions”](../6. FAQ/faq_techniques.md#how-can-i-alternate-conditions) for an example of using an [enum](../3. Language/language_enums.md#enums) to manage two exclusive states. The following example script uses two boolean variables to do the same thing.
...
## [How can I organize my script’s inputs in the Settings/Inputs tab?](../6. FAQ/faq_techniques.md#how-can-i-organize-my-scripts-inputs-in-the-settingsinputs-tab)
...
- **Grouping inputs:** Create a section header for a group of inputs by using the `group` parameter in the [input()](../../reference manual/functions/input.md) functions. Use constants for group names to simplify any potential name changes.
...
- Provide adequate minimum and maximum values for numeric values, selecting the proper [float](../../reference manual/types/float.md) or [int](../../reference manual/types/int.md) type.
- Customize step values based on the specific needs of each input.
- Because checkboxes cannot be indented, use the [input()](../../reference manual/functions/input.md) function’s `options` parameter to create dropdown selections so that the sections appear more organized compared to using checkboxes.
...
## [Can I plot values from a local scope?](../6. FAQ/faq_techniques.md#can-i-plot-values-from-a-local-scope)

A script can use `plot*()` functions and other [plot visuals](../2. Visuals/visuals_overview.md#plot-visuals) only in the [global scope](../6. FAQ/faq_programming.md#what-does-scope-mean) — they cannot be included in the local scopes of [conditional structures](../3. Language/language_conditional-structures.md), [loops](../3. Language/language_loops.md), or [user-defined functions](../3. Language/language_user-defined-functions.md) and [methods](../3. Language/language_methods.md#user-defined-methods). Therefore, plots can only use variables and literals that are declared globally.

However, programmers can [extract data from local scopes](../4. Writing_Scripts/writing_debugging.md#extracting-data-from-local-scopes) to the global scope to make the data accessible to `plot*()` functions. Assign the local scope values to globally declared variables, using [return expressions](../4. Writing_Scripts/writing_debugging.md#extraction-using-return-expressions) or [reference types](../4. Writing_Scripts/writing_debugging.md#extraction-using-reference-types), then use these global variables in `plot*()` calls to visualize the local data.

Alternatively, use [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) or [drawings](../4. Writing_Scripts/writing_debugging.md#pine-drawings) to display values from within local scopes directly.

[Previous 
**Strings and formatting**](../6. FAQ/faq_strings-and-formatting.md) [Next 
**Times, dates, and sessions**](../6. FAQ/faq_times-dates-and-sessions.md)