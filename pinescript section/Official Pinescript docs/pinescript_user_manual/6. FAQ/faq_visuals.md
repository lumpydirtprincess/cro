![](../6. FAQ/faq_visuals.md)

# [Visuals](../6. FAQ/faq_visuals.md#visuals)

## [Why can’t I use a plot in an ​`if`​ or ​`for`​ statement?](../6. FAQ/faq_visuals.md#why-cant-i-use-a-plot-in-an-if-or-for-statement)

In Pine Script®, scripts cannot place [plot()](../../reference manual/functions/plot.md) calls directly within [if](../../reference manual/keywords/if.md) or [for](../../reference manual/keywords/for.md) statements — or in any other [local scopes](../6. FAQ/faq_programming.md#what-does-scope-mean). The compiler needs to know about all plots during script compilation.

However, scripts _can_ plot values conditionally, by changing the series or color of the plot.

Our example script plots two [ALMA](https://www.tradingview.com/support/solutions/43000594683/) moving averages only when the shorter average is below the longer one. It fills between the two averages on every bar, but the fill color is [na](../../reference manual/variables/na.md) unless the shorter average is above or equal to the longer one.

![image](../images/Visuals-Why-cant-i-use-a-plot-in-an-if-or-for-statement-1.3rjsIGWF_19Q5U0.webp)
...
- The script updates the Boolean variable that describes whether the moving averages are in bullish order — and thereby controls the plots and fill — in a _local scope_. However, the script declares it in the _global scope_ so that the [fill()](../../reference manual/functions/fill.md) and [plot()](../../reference manual/functions/plot.md) calls can use it.
- The moving averages are each plotted twice. The plots that display in the Data Window must be assigned to variables so that the [fill()](../../reference manual/functions/fill.md) function call can reference them. The [fill()](../../reference manual/functions/fill.md) function cannot use the later plots, which display on the chart, because they have [na](../../reference manual/variables/na.md) values when the fill color is not [na](../../reference manual/variables/na.md).
- We use `plot.style_linebr` as the argument for the `style` parameter of the [plot()](../../reference manual/functions/plot.md) function so that the plot does not span bars with an [na](../../reference manual/variables/na.md) plot value.

## [Can I plot diagonals between two points on the chart?](../6. FAQ/faq_visuals.md#can-i-plot-diagonals-between-two-points-on-the-chart)

Scripts can plot diagonal lines between two points on a chart by using plots or line objects.

### [Using plots](../6. FAQ/faq_visuals.md#using-plots)

The [plot()](../../reference manual/functions/plot.md) function connects consecutive data points with straight lines.

The default value of the `style` argument for the [plot()](../../reference manual/functions/plot.md) function is `plot.style_line`. This style of plot connects the plotted points on either side of bars that have [na](../../reference manual/variables/na.md) series values to each other with a line. If the points are at different heights, the lines are diagonal.

The functions [ta.pivotlow()](../../reference manual/functions/ta.pivotlow.md) and [ta.pivothigh()](../../reference manual/functions/ta.pivothigh.md) return [na](../../reference manual/variables/na.md) for all bars except those with identified pivots. The following example script draws diagonal lines joining pivot highs and lows using [plot()](../../reference manual/functions/plot.md).

![image](../images/Visuals-Can-i-plot-diagonals-between-two-points-on-the-chart-1.Dcpal7Wj_Z1hKgJm.webp)
...
- The [ta.pivotlow()](../../reference manual/functions/ta.pivotlow.md) and [ta.pivothigh()](../../reference manual/functions/ta.pivothigh.md) functions confirm pivot highs and lows only after a specified number of bars, so the script must offset the plotted lines by the same number of bars. Plot offsets cannot change during script execution.

### [Using lines](../6. FAQ/faq_visuals.md#using-lines)

[Line objects](../1. Concepts/concepts_lines-and-boxes.md#lines) provide more flexibility than plots. The [plot()](../../reference manual/functions/plot.md) function can plot a line, symbol, or area only at the bar on which the script is executing (or at a _fixed_ offset from it). In contrast, line objects can be created on _any_ bar. Similarly, whereas plots are fixed once the bar closes, line properties can be updated at any time.

The following example script demonstrates these advantages. Like the example script in the previous section, [Using plots](../6. FAQ/faq_visuals.md#using-plots), this script draws lines between pivot highs and lows. In this case, however, the pivot highs and lows are confirmed an _unpredictable_ number of bars afterwards, so plots are completely unsuitable for drawing lines between them. Line drawings are suitable because scripts can offset lines an arbitrary and dynamic number of bars into the past. The script below also retroactively _changes_ the color of drawn lines, which are initially white, depending on whether the _next_ line of the same type slopes up or down. Such updates are not possible with plots.

![image](../images/Visuals-Can-i-plot-diagonals-between-two-points-on-the-chart-2.ZmicdTOe_Z1udlu8.webp)
...
- We use an [enum](../3. Language/language_enums.md#enums) to store the state of the pivot confirmation process. This, together with a set of explicit rules for changing the state, forms a simple [state machine](https://en.wikipedia.org/wiki/Finite-state_machine). Such constructs can be easier to debug than maintaining and resetting multiple persistent global variables that depend on each other.
- Scripts can create only a certain number of lines. The limit is set by the `max_lines_count` parameter of the [indicator()](../../reference manual/functions/indicator.md) or [strategy()](../../reference manual/functions/strategy.md) declaration. The default is 50, and the maximum is 500 per script.

## [How can I plot a line with gaps?](../6. FAQ/faq_visuals.md#how-can-i-plot-a-line-with-gaps)

Scripts can plot lines with gaps on specific bars by setting the argument of _either_ the `series` parameter or `color` parameter of the [plot()](../../reference manual/functions/plot.md) function to [na](../../reference manual/variables/na.md) for some bars.

Note that this is different to plotting dashed lines using `linestyle = plot.linestyle_dashed` in the [plot()](../../reference manual/functions/plot.md) call, which plots a line with gaps at regular intervals that do not correspond to particular bars.

Using [na](../../reference manual/variables/na.md) values for the color or series enables scripts to omit a plot on an _arbitrary_ selection of bars. These two methods have different effects:

- An [na](../../reference manual/variables/na.md) _color_ value omits the plotted line from the most recent bar with a non-na value to the first bar with an [na](../../reference manual/variables/na.md) value. The size of the gap is equal to the number of consecutive bars with [na](../../reference manual/variables/na.md) values.
- An [na](../../reference manual/variables/na.md) _series_ value omits the plot point for each bar with an [na](../../reference manual/variables/na.md) value, effectively **increasing the size of the gap by one bar**.

The following example script shows the effect of these two methods. It plots two straight lines above the chart bars, one using each method. At a configurable interval, the `series` or `color` argument is [na](../../reference manual/variables/na.md), and the background color changes for that bar.

![image](../images/Visuals-How-can-i-plot-a-line-with-gaps-1.D6RMbvkI_ZtXxDl.webp)
...
- The style for the plot with [na](../../reference manual/variables/na.md)`series` values must be `plot.style_linebr` in order to show gaps. The default value of `plot.style_line` fills in the gaps.

## [How do I plot a line using start/stop criteria?](../6. FAQ/faq_visuals.md#how-do-i-plot-a-line-using-startstop-criteria)
...
3. Choose a plotting style. Use [na](../../reference manual/variables/na.md) for either the `series` or `color` parameter of the [plot()](../../reference manual/functions/plot.md) function. See the section [How can I plot a line with gaps?](../6. FAQ/faq_visuals.md#how-can-i-plot-a-line-with-gaps) above for examples.
4. Optionally, use debug. For example, plot logical states using the [plotchar()](../../reference manual/functions/plotchar.md) function.

In the following example script, the start condition is the detection of a new pivot. The start condition turns on the `doPlot` Boolean flag that controls when to plot, and captures the value to plot in the `savedValue` “float” variable. The stop condition is price closing above the pivot level. The script uses a bar timer as an extra stop condition. Either stop condition turns off the flag and _resets_ the plot value to [na](../../reference manual/variables/na.md). The script plots debug characters for all logical conditions.

![image](../images/Visuals-How-do-i-plot-a-line-using-start-stop-criteria-1.D0se5nwy_ZWMSQY.webp)
...
## [How can I plot a support or trend line?](../6. FAQ/faq_visuals.md#how-can-i-plot-a-support-or-trend-line)
...
### [Plotting support and resistance](../6. FAQ/faq_visuals.md#plotting-support-and-resistance)

The following example script tracks levels of support and resistance until price action breaks them. The script uses the `ta.pivot*()` built-in functions to detect pivot highs and lows, then draws horizontal lines from these points. This script visualizes lows as support (green lines) and highs as resistance (red lines) for simplicity. If the [close](../../reference manual/variables/close.md) of a bar crosses a line, the script stops extending that line:

![image](../images/Visuals-How-do-i-plot-a-support-or-a-trend-line-1.zCbz0Lc8_1PqXml.webp)
...
### [Plotting trend lines](../6. FAQ/faq_visuals.md#plotting-trend-lines)
...
![image](../images/Visuals-How-do-i-plot-a-support-or-a-trend-line-2.CDeAY9mr_Z1o1Qox.webp)
...
- For simplicity, we do not deactivate lines when price crosses them. For an example of how to do this, see the example script from the previous section, [Plotting support and resistance](../6. FAQ/faq_visuals.md#plotting-support-and-resistance).
...
## [How can I use colors in my indicator plots?](../6. FAQ/faq_visuals.md#how-can-i-use-colors-in-my-indicator-plots)
...
Without needing to specify hexadecimal or RGB (Red Green Blue) values, programmers can use predefined [color constants](../1. Concepts/concepts_colors.md#constant-colors), such as [color.red](../../reference manual/variables/color.red.md). In the Pine Editor, clicking the automatically generated color swatch next to a color constant opens up a color picker; choosing a different color from the picker updates the code that defines the color.

**Custom colors**

For a more personalized appearance, programmers can create custom colors, either by specifying a hexadecimal code, or by using the [color.rgb()](../../reference manual/functions/color.rgb.md) function to specify the RGB values. For the hexadecimal and RGB equivalents of the built-in color constants, see the table in the [Constant colors](../1. Concepts/concepts_colors.md#constant-colors) section.

**Transparency settings**

The [color.new()](../1. Concepts/concepts_colors.md#colornew) function can create colors with a specific transparency. The `transp` parameter can even take series values, meaning that a single color declaration can provide a color with dynamic transparency.

**Conditional coloring**

Scripts can assign different colors to variables based on logical conditions. Programmers can use [conditional colors](../1. Concepts/concepts_colors.md#conditional-coloring) to color plots, fills, shapes, drawings, or the chart background differently on different bars.

**Gradient transitions**

The [color.from\_gradient()](../../reference manual/functions/color.from_gradient.md) function creates color gradients that can highlight shifts in data values while ensuring a smooth transition between colors. For detailed guidance and innovative examples on implementing gradients, consult the [Color Gradient Framework](https://www.tradingview.com/script/hqH4YIFa-Color-Gradient-Framework-PineCoders/) by PineCoders.

**Fills**

Scripts can [create shaded areas](../1. Concepts/concepts_fills.md) between lines, plots, or hlines, as well as within boxes and polylines. Fills can be especially useful for highlighting ranges, zones, or contrasts.
...
![image](../images/Visuals-How-can-i-use-colors-in-my-indicator-plots-1.Cp4v-X8j_ZTB0lc.webp)

![image](../images/Visuals-How-can-i-use-colors-in-my-indicator-plots-2.BUNwe5Dz_Z2aQkLP.webp)
...
- The script uses the [plotcandle()](../../reference manual/functions/plotcandle.md) function to create bordered column bars painted in bullish or bearish hues based on the trend’s direction, serving as a visually distinct histogram that depicts the distance between the TSI and signal line.
...
## [How do I make my indicator plot in the main chart pane?](../6. FAQ/faq_visuals.md#how-do-i-make-my-indicator-plot-in-the-main-chart-pane)

By default, new scripts display in a separate pane. To make a script display in the main chart pane instead, use `overlay = true` in the [strategy()](../../reference manual/functions/strategy.md) or [indicator()](../../reference manual/functions/indicator.md) declaration statement:
...
## [How can I plot vertical lines on a chart?](../6. FAQ/faq_visuals.md#how-can-i-plot-vertical-lines-on-a-chart)
...
### [By drawing lines](../6. FAQ/faq_visuals.md#by-drawing-lines)
...
### [By plotting histograms](../6. FAQ/faq_visuals.md#by-plotting-histograms)
...
With this method, unlike using line drawings, the user must right-click the price scale and select “Scale price chart only” to avoid distorting the chart vertically. Scripts can plot histograms, like all types of plot, only on the bar where the script is executing or at a _fixed_ offset.
...
### [By coloring the background](../6. FAQ/faq_visuals.md#by-coloring-the-background)
...
## [How can I toggle hline() levels on and off?](../6. FAQ/faq_visuals.md#how-can-i-toggle-hline-levels-on-and-off)

Scripts can toggle the display of horizontal levels plotted using [hline()](../../reference manual/functions/hline.md) in several ways. Conditionally setting the argument of the `price` or `color` parameter to [na](../../reference manual/variables/na.md) shows or hides the level in a similar way that setting the `series` or `color` to [na](../../reference manual/variables/na.md) does for plotted lines, as described in the entry [How do I plot a line using start/stop criteria?](../6. FAQ/faq_visuals.md#how-do-i-plot-a-line-using-startstop-criteria) above.

Additionally, the `display` parameter of the [hline()](../../reference manual/functions/hline.md) function can take an input value as its argument. This parameter controls where the line displays.
...
## [How can I draw lines or labels into the future?](../6. FAQ/faq_visuals.md#how-can-i-draw-lines-or-labels-into-the-future)

Individual _plotted_ lines and shapes cannot be drawn into the future — only the entire series can be offset. By contrast, scripts can extend any _drawn_ lines or boxes, or position drawn labels, at an arbitrary distance beyond the last data point. There are two ways to achieve this: using [bar\_index](../../reference manual/variables/bar_index.md) and using [xloc.bar\_time](../../reference manual/variables/xloc.bar_time.md).

### [Using bar\_index](../6. FAQ/faq_visuals.md#using-bar_index)

The [bar\_index](../../reference manual/variables/bar_index.md) built-in variable represents the sequential number of the current bar, starting from zero for the first bar in the chart history and incrementing by 1 for each subsequent bar. Drawing objects with their `xloc` parameter set to [xloc.bar\_index](../../reference manual/variables/xloc.bar_index.md) can use a `bar_index` as their x coordinates. If the `xloc` parameter is not specified, it defaults to [xloc.bar\_index](../../reference manual/variables/xloc.bar_index.md).

To project a certain number of bars into the future or past, simply add or subtract that number from the current [bar\_index](../../reference manual/variables/bar_index.md). For instance, `bar_index + 20` positions the object 20 bars into the future.
...
![image](../images/Visuals-How-can-i-draw-lines-or-labels-into-the-future-1.jzxOMGSo_PelPy.webp)
...
- On the last bar of the dataset, we adjust the `x2` point of both lines and the `x` point of the labels to extend into the future by the user-defined offset amount. In this section, for demonstration purposes, we call the same setting functions as [methods](../3. Language/language_methods.md#methods) by using dot notation syntax.

### [Using time](../6. FAQ/faq_visuals.md#using-time)

Drawing objects can also be positioned based on UNIX time values, by using [xloc.bar\_time](../../reference manual/variables/xloc.bar_time.md) for an object’s `xloc` parameter. The time values can be timestamps, or a bar’s open time, or any other calculated time.
...
## [How can I keep only the last n number of drawings?](../6. FAQ/faq_visuals.md#how-can-i-keep-only-the-last-n-number-of-drawings)
...
### [Using a ​`*.all`​ array](../6. FAQ/faq_visuals.md#using-a-all-array)
...
The `*.all` arrays are _read-only_. Scripts cannot change the arrays directly by pushing or shifting elements, but they can _update_ or _delete_ elements in the arrays. The following example script keeps a maximum of 10 labels on the chart. It gets the first label’s ID via the [array.first()](../../reference manual/functions/array.first.md) function and then deletes that label with the [label.delete](../../reference manual/functions/label.delete.md) function.
...
- Only built-in drawing types have `*.all` arrays available, so if a script uses [objects of user-defined types](../3. Language/language_objects.md) as containers for several drawings, using an array as a queue instead is the only option.
- When the script deletes a label, the label is _also_ automatically removed from the [label.all](../../reference manual/variables/label.all.md) array. By contrast, if a script adds drawings to an array manually, it must both remove the object from the array _and_ delete it. In this case, [array.shift()](../../reference manual/functions/array.shift.md) is usually the best choice, because it removes the first value from an array and returns it for further manipulation.

### [Using an array as a queue](../6. FAQ/faq_visuals.md#using-an-array-as-a-queue)

A more flexible method to keep only the last _n_ number of drawings is to [use an array as a queue](../3. Language/language_arrays.md#using-an-array-as-a-queue). Each time the script adds a new element to the array, it removes the oldest element. This is possible because each element in an array has a unique _index_, and array indexes _always_ begin at zero. If we remove element `0` from an array with several elements, the element that was at index `1` is now at index `0`, the element at index `2` moves down to index `1`, and so on.
...
## [Is it possible to draw geometric shapes?](../6. FAQ/faq_visuals.md#is-it-possible-to-draw-geometric-shapes)

Geometric shapes can illustrate patterns, mark zones of interest, or create other visual aids for technical analysis. Scripts can use two main methods for drawing shapes: [polylines](../1. Concepts/concepts_lines-and-boxes.md#polylines) and [lines](../1. Concepts/concepts_lines-and-boxes.md#lines).

### [Drawing with polylines](../6. FAQ/faq_visuals.md#drawing-with-polylines)
...
1. Create an array containing objects of type [chart.point](../../reference manual/types/chart.point.md) that define the vertices of your desired shape.
2. Pass this array to the [polyline.new()](../../reference manual/functions/polyline.new.md) function. This function draws sequential lines between the points in the order they appear in the array.
...
### [Drawing with lines](../6. FAQ/faq_visuals.md#drawing-with-lines)

Lines are simpler than polylines. A [line](../1. Concepts/concepts_lines-and-boxes.md#lines) in Pine Script is straight connection between two points. Here’s how to use lines for drawing shapes:
...
![image](../images/Visuals-Is-it-possible-to-draw-geometric-shapes-2.DEB53-5u_ZAOwU8.webp)
...
## [How can I color the chart’s background on a condition detected on the last bar?](../6. FAQ/faq_visuals.md#how-can-i-color-the-charts-background-on-a-condition-detected-on-the-last-bar)

The usual way to color the chart background is by calling the [bgcolor()](../../reference manual/functions/bgcolor.md) function, which colors the chart background _for the bar on which it is called_. The background for a particular bar cannot be changed on later bars, and bars cannot be colored retroactively, using this method.

To color the entire chart background based on a condition detected on the last bar, one workaround is to use a table sized to the visible area of the chart, and color the background of the cell. See this example in the page on [tables](../1. Concepts/concepts_tables.md#coloring-the-charts-background).
