![](../6. FAQ/faq_data-structures.md)

# [Data structures](../6. FAQ/faq_data-structures.md#data-structures)

## [What data structures can I use in Pine Script®?](../6. FAQ/faq_data-structures.md#what-data-structures-can-i-use-in-pine-script)
...
### [Tuples](../6. FAQ/faq_data-structures.md#tuples)

A [tuple](../3. Language/language_type-system.md#tuples) in Pine Script is a list of values that is returned by a [function](../3. Language/language_user-defined-functions.md), [method](../3. Language/language_methods.md), or local block. Unlike in other languages, tuples in Pine serve no other function.
...
Using a tuple to request several values from the same symbol and timeframe using a [request.security()](../../reference manual/functions/request.security.md) call is more efficient than making several calls. For instance, consider a script that contains separate [request.security()](../../reference manual/functions/request.security.md) calls for the
[open](../../reference manual/variables/open.md), [high](../../reference manual/variables/high.md),
[low](../../reference manual/variables/low.md), and [close](../../reference manual/variables/close.md) prices:
...
Using a tuple can consolidate these calls into a single [request.security()](../../reference manual/functions/request.security.md) function call, reducing performance overhead:

```pine
[o, h, l, c] = request.security(syminfo.tickerid, "D", [open, high, low, close])
```

See the [Tuples](../3. Language/language_type-system.md#tuples) section in the User Manual for more information.

### [Arrays](../6. FAQ/faq_data-structures.md#arrays)

[Arrays](../3. Language/language_arrays.md) store multiple values of the same [type](../3. Language/language_type-system.md) in a single variable. Each _element_ in an array can be efficiently accessed by its _index_—an integer corresponding to its position within the array.
...
Arrays can be created with either the [array.new<type>()](../../reference manual/functions/array.new<type>.md)
or [array.from()](../../reference manual/functions/array.from.md) function. In this simple example, we store the last five closing prices in an array and display it in a table:
...
See the [Arrays](../3. Language/language_arrays.md) section in the User Manual for more information.

### [Matrices](../6. FAQ/faq_data-structures.md#matrices)

A [matrix](../3. Language/language_matrices.md) is a two-dimensional array, made of rows and columns, like a spreadsheet. Matrices, like arrays, store values of the same built-in or user-defined [type](../3. Language/language_type-system.md#types).
...
This script demonstrates a simple example of matrix addition. It creates a 3x3 matrix, calculates its [transpose](../3. Language/language_matrices.md#transposing), then calculates the
[matrix.sum()](../../reference manual/functions/matrix.sum.md) of the two matrices. This example displays [strings](../1. Concepts/concepts_strings.md) representing the original matrix, its transpose, and the resulting sum matrix in a [table](../1. Concepts/concepts_tables.md) on the chart:

![image](../images/Data-structures-What-are-the-primary-data-structues-available-in-the-pine-script-1.CN2IZH7V_y3sJJ.webp)
...
See the [Matrices](../3. Language/language_matrices.md) section in the User Manual for more information.

### [Objects](../6. FAQ/faq_data-structures.md#objects)

Pine Script [objects](../3. Language/language_objects.md) are containers that group together multiple fields into one logical unit.

Objects are _instances_ of [user-defined types](../3. Language/language_type-system.md#user-defined-types) (UDTs). UDTs are similar to _structs_ in traditional programming languages. They define the rules for what an object can contain. Scripts first create a UDT by using the [type](../../reference manual/keywords/type.md) keyword and then create one or more objects of that type by using the UDT’s built-in `new()` method.
...
See the User Manual page on [Objects](../3. Language/language_objects.md) to learn more about working with UDTs.

### [Maps](../6. FAQ/faq_data-structures.md#maps)

[Maps](../3. Language/language_maps.md#maps) in Pine Script are similar to _dictionaries_ in other programming languages, such as dictionaries in Python, objects in JavaScript, or HashMaps in Java.
...
![image](../images/Data-structures-What-are-the-primary-data-structues-available-in-the-pine-script-2.F207zRIS_tzrNs.webp)
...
Here, we use [request.earnings()](../../reference manual/functions/request.earnings.md) with the barmerge parameter set to [barmerge.gaps\_on](../../reference manual/constants/barmerge.gaps_on.md) to return the earnings value on bars where earnings data is available, and return [na](../../reference manual/variables/na.md) otherwise.
We add non-na values to the map, associating the dates that earnings occurred with the earnings numbers. Finally, on the last historical bar, the script [loops through the map](../3. Language/language_maps.md#looping-through-a-map), logging each key-value pair to display the map’s contents.

To learn more about working with maps, refer to the [Maps](../3. Language/language_maps.md) section in the User Manual.

## [What’s the difference between a series and an array?](../6. FAQ/faq_data-structures.md#whats-the-difference-between-a-series-and-an-array)

In Pine Script, [“series”](../3. Language/language_type-system.md#series) variables are calculated on each bar. Historical values cannot change. Series values can change during the open realtime bar, but when the bar closes, the value for that bar becomes fixed and immutable.
These fixed values are automatically indexed for each bar. Scripts can access values from previous bars by using the [\[\] history-referencing operator](../3. Language/language_operators.md#-history-referencing-operator) to go back one or more bars.

Where “series” variables are strictly time-indexed, and the historical values are created automatically, [arrays](../3. Language/language_arrays.md) are created, filled, and manipulated arbitrarily by a script’s logic. Programmers can change the size of arrays dynamically by using functions that [insert or remove elements](../3. Language/language_arrays.md#inserting-and-removing-array-elements). Any element in an array can also be altered using the [array.set()](../../reference manual/functions/array.set.md) function.

The concept of [time series](../3. Language/language_execution-model.md#time-series) is a fundamental aspect of Pine Script. Its series-based [execution model](../3. Language/language_execution-model.md) processes scripts bar-by-bar. This built-in behavior mimics looping, allowing a series to track values, accumulate totals, or perform calculations across a sequence of data on each bar.
...
## [How do I create and use arrays in Pine Script?](../6. FAQ/faq_data-structures.md#how-do-i-create-and-use-arrays-in-pine-script)

Pine Script [arrays](../3. Language/language_arrays.md) are one-dimensional collections that can hold multiple values of a single type.
...
or
[array.copy()](../../reference manual/functions/array.copy.md).
Arrays can be declared with the [var](../3. Language/language_variable-declarations.md#var) keyword to have their values persist from bar to bar, or without it, so that the values initialize again on each bar.
For more on the differences between declaring arrays with or without [var](../3. Language/language_variable-declarations.md#var), see [this section](../6. FAQ/faq_data-structures.md#whats-the-difference-between-an-array-declared-with-or-without-var) of this FAQ.
...
inserts a new element at the specified index and increases the index of existing elements at or after the insertion index by one. It accepts both positive and [negative indices](../3. Language/language_arrays.md#negative-indexing), which reference an element’s position starting from the beginning of the array or from the end, respectively.
...
removes the element at the specified index and returns that element’s value. It accepts both positive and [negative indices](../3. Language/language_arrays.md#negative-indexing), which reference an element’s position starting from the beginning of the array or from the end, respectively.
...
[queues](../3. Language/language_arrays.md#using-an-array-as-a-queue) or [stacks](../3. Language/language_arrays.md#using-an-array-as-a-stack), which are useful for custom datasets or sliding window calculations.
Read more about implementing a stack or queue in [this FAQ entry](../6. FAQ/faq_data-structures.md#what-are-queues-and-stacks).

**Calculations on arrays**

Because arrays are not [time series](../3. Language/language_execution-model.md#time-series) data structures, performing operations across an array’s elements requires special functions designed for arrays.
...
See the [Calculation on arrays](../3. Language/language_arrays.md#calculations-on-arrays) section of the User Manual for more information.

**Script example**

This script example demonstrates a practical application of arrays by tracking the opening prices of the last five sessions. The script declares a float array to hold the prices using the [var](../3. Language/language_variable-declarations.md#var) keyword, allowing it to retain its values from bar to bar.

At the start of each session, we update the array by adding the new opening price and removing the oldest one. This process, resembling a [queue](../3. Language/language_arrays.md#using-an-array-as-a-queue), keeps the array’s size constant while maintaining a moving window of the session opens for the last five days.
...
![image](../images/Data-structures-How-do-i-create-and-use-arrays-in-pine-script-1.SP394-6D_Z1DQxBa.webp)
...
For more information about arrays, see the [Arrays](../3. Language/language_arrays.md#arrays) page in the User Manual.

## [What’s the difference between an array declared with or without ​`var`​?](../6. FAQ/faq_data-structures.md#whats-the-difference-between-an-array-declared-with-or-without-var)

Using the [var](../../reference manual/operators/var.md) keyword, a script can declare an [array](../3. Language/language_arrays.md) variable in a script that is initialized only once, during the first iteration on the first chart bar.

**Persistent arrays**

When an array is declared with [var](../../reference manual/operators/var.md), it is initialized only once, at the first execution of the script. This allows the array to retain its contents and potentially grow in size across bars, making it ideal for cumulative data collection or tracking values over time.

**Non-persistent arrays**

Arrays declared without [var](../../reference manual/operators/var.md) are reinitialized on every new bar, effectively resetting their content. This behavior suits scenarios where calculations are specific to the current bar, and historical data retention is unnecessary.

**Example script**

Here, we initialize two arrays. Array `a` is declared without using the [var](../../reference manual/operators/var.md) keyword, while array `b`
is declared with [var](../../reference manual/operators/var.md), allowing us to observe and compare their behavior. Throughout the runtime, we incrementally add an element to
each array on each bar. We use a [table](../2. Visuals/visuals_tables.md) to present and compare both the sizes of these arrays and the number of chart bars, effectively illustrating the impact of
...
[declaration modes](../3. Language/language_variable-declarations.md#declaration-modes) and their use in
[array declarations](../3. Language/language_arrays.md#using-var-and-varip-keywords) in the User Manual.

## [What are queues and stacks?](../6. FAQ/faq_data-structures.md#what-are-queues-and-stacks)

Scripts can use [arrays](../3. Language/language_arrays.md) to create [queues](../3. Language/language_arrays.md#using-an-array-as-a-queue) and [stacks](../3. Language/language_arrays.md#using-an-array-as-a-stack).

**Stacks**

A stack uses the “last in, first out” (LIFO) principle, where the most recently added item is the first to be taken away.
Think of this like a stack of plates, where you can only place a new plate on top or remove the top plate. To use an array as a stack, add elements to the end of the array using [array.push()](../../reference manual/functions/array.push.md) and remove elements from the end of the array using [array.pop()](../../reference manual/functions/array.pop.md).

**Queues**

A queue uses the “first in, first out” (FIFO) principle, where the first item to be added is the first to be removed. This kind of queue in code is like a queue in real life, such as in a coffee shop, where no matter how many people join the end of the queue, the first person still gets served first. To use an array as a queue, add elements to the end of the array using [array.push()](../../reference manual/functions/array.push.md) and remove them from the beginning using [array.shift()](../../reference manual/functions/array.shift.md).
...
This script uses arrays as stacks to manage pivot points. It draws [lines](../2. Visuals/visuals_lines-and-boxes.md#lines) from the pivot points and extends the lines with each new bar until price intersects them.
...
![image](../images/Data-structures-What-does-queue-stack-mean-1.DR_kqOoE_Z1u8fFP.webp)
...
![image](../images/Data-structures-What-does-queue-stack-mean-2.B1hrWGZc_11uYgb.webp)
...
For more information on manipulating arrays, see the [Arrays](../3. Language/language_arrays.md) section in the User Manual.

## [How can I perform operations on all elements in an array?](../6. FAQ/faq_data-structures.md#how-can-i-perform-operations-on-all-elements-in-an-array)
...
The easiest way to retrieve each element in an array is by using a [for…in](../../reference manual/keywords/for...in.md) structure. This type of loop retrieves each element in turn, without the need for specifying the number of iterations.
...
If the script’s logic requires the position of the element in the array, use the two-argument form: `for [index, element] in array`. This form returns both the current element and its index in a [tuple](../3. Language/language_type-system.md#tuples).

**Example: retrieving array elements**

This first example script uses an array as a [queue](../3. Language/language_arrays.md#using-an-array-as-a-queue) to store [lines](../2. Visuals/visuals_lines-and-boxes.md#lines) representing the latest four pivot highs and lows. The [for…in](../../reference manual/keywords/for...in.md) loop performs two tasks:

- It adjusts the `x2` endpoint of each line to the current [bar\_index](../../reference manual/variables/bar_index.md).
- It changes the colors of the lines to blue for support or orange for resistance, based on their position relative to the [close](../../reference manual/variables/close.md)
price.

Note that neither of these operations requires knowing the index of the array element.

![image](../images/Data-structures-How-can-i-perform-operations-on-all-elements-in-an-array-1.Cv5bYrg2_1p3aMC.webp)
...
In our second script, we use the two-argument variant of the [for…in](../../reference manual/keywords/for...in.md) loop to access elements and their indices in an array.
...
## [What’s the most efficient way to search an array?](../6. FAQ/faq_data-structures.md#whats-the-most-efficient-way-to-search-an-array)

The obvious way to search for an element in an array is to use a [loop](../3. Language/language_loops.md) to check each element in turn.
However, there are more efficient ways to search, which can be useful in different situations.
Some of the following functions return only the index of a value. Programmers can then use [array.get()](../../reference manual/functions/array.get.md) if the script needs the actual value.

### [Checking if a value is present in an array](../6. FAQ/faq_data-structures.md#checking-if-a-value-is-present-in-an-array)

If all the script needs to do is to check whether a certain value is present in an array or not, use the [array.includes()](../../reference manual/functions/array.includes.md) function.
If the element is found, the function returns [true](../../reference manual/constants/true.md); otherwise, it returns [false](../../reference manual/constants/false.md).
This method does not return the index of the element.

The following example script checks if the value `3` is present in the `values` array, and displays either “found” or “not found” in a [label](../2. Visuals/visuals_text-and-shapes.md#labels).
...
### [Finding the position of an element](../6. FAQ/faq_data-structures.md#finding-the-position-of-an-element)

If the script requires the _position_ of an element, programmers can use the [array.indexof()](../../reference manual/functions/array.indexof.md) function.
This function returns the index of the _first_ occurrence of a value within an array. If the value is not found, the function returns `-1`.
...
### [Binary search](../6. FAQ/faq_data-structures.md#binary-search)

If the script requires the position of the element in a sorted array, the function [array.binary\_search()](../../reference manual/functions/array.binary_search.md) returns the index of a value more efficiently than [array.indexof()](../../reference manual/functions/array.indexof.md). The performance improvement is significant for large arrays. If the value is not found, the function returns `-1`.
...
If a script does not need the exact value, the functions [array.binary\_search\_leftmost()](../../reference manual/functions/array.binary_search_leftmost.md) and [array.binary\_search\_rightmost()](../../reference manual/functions/array.binary_search_rightmost.md) provide an effective way to locate the nearest index to a given value in sorted arrays.
...
## [How can I debug arrays?](../6. FAQ/faq_data-structures.md#how-can-i-debug-arrays)
...
For information about commonly encountered array-related errors, refer to the array [Error Handling](../3. Language/language_arrays.md#error-handling) section in the User Manual.

### [Plotting](../6. FAQ/faq_data-structures.md#plotting)

Using the [plot()](../../reference manual/functions/plot.md) function to inspect the contents of an array can be helpful because this function can show numerical values on the script’s status line, the price
scale, and the Data Window. It is also easy to review historical values.

Limitations of this approach include:

- Arrays must be of type [“float”](../3. Language/language_type-system.md#float) or [“int”](../3. Language/language_type-system.md#int).
- The number of plots used for debugging counts towards the [plot limit for a script](../2. Visuals/visuals_plots.md#plot-count-limit).
...
Here we populate an array with the [open](../../reference manual/variables/open.md), [high](../../reference manual/variables/high.md), [low](../../reference manual/variables/low.md) and [close](../../reference manual/variables/close.md) (OHLC) prices on each bar. The script retrieves all the elements of the array and plots them on the chart.
...
### [Using labels](../6. FAQ/faq_data-structures.md#using-labels)

Using [labels](../2. Visuals/visuals_text-and-shapes.md#labels) to display array values on certain bars is particularly useful for non-continuous data points or to view all elements of an array simultaneously.
Scripts can create labels within any local scope, including [functions](../3. Language/language_user-defined-functions.md) and [methods](../3. Language/language_methods.md#user-defined-methods). Scripts can also position drawings at any available chart location, irrespective of the current [bar\_index](../../reference manual/variables/bar_index.md).
...
- Scripts can display only up to a [maximum number of labels](../4. Writing_Scripts/writing_limitations.md#line-box-polyline-and-label-limits).
...
For more information, see the [Labels](../4. Writing_Scripts/writing_debugging.md#labels) section of the [Debugging](../4. Writing_Scripts/writing_debugging.md) page in the User Manual.

### [Using label tooltips](../6. FAQ/faq_data-structures.md#using-label-tooltips)
...
In this case, displaying the array contents in a label tooltip can be visually clearer. This method has the same advantages and limitations as [using labels](../6. FAQ/faq_data-structures.md#using-labels) in the section above.
...
### [Using tables](../6. FAQ/faq_data-structures.md#using-tables)

Using [tables](../2. Visuals/visuals_tables.md) for debugging offers a more organized and scalable alternative to labels. Tables can display multiple [“series”](../3. Language/language_type-system.md#series) strings in a clear format that remains unaffected by the chart’s scale or the index of the bars.
...
### [Using Pine Logs](../6. FAQ/faq_data-structures.md#using-pine-logs)

[Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) are messages that display in the Pine Logs pane, along with a timestamp when the logging function was called.
Scripts can create log messages at specific points during the execution of a script. Programmers can use the `log.*()` functions to create Pine Logs from almost anywhere in a script — including inside the _local scopes_ of [user-defined functions](../3. Language/language_user-defined-functions.md), [conditional structures](../3. Language/language_conditional-structures.md), and [loops](../3. Language/language_loops.md).
...
The script below updates a previous example script from the section on [queues and stacks](../6. FAQ/faq_data-structures.md#what-are-queues-and-stacks) to add logging. It uses arrays as [stacks](../3. Language/language_arrays.md#using-an-array-as-a-stack) to track lines drawn from pivot points. When a pivot occurs, the script adds a new line to the stack and continues to extend the lines on each bar until an intersection with price occurs. If an intersection is found, the script removes (pops) the intersected line from the stack, meaning it will no longer be extended with new bars.
...
![image](../images/Data-structures-How-do-i-debug-arrays-1.BCL_wjfL_ZOXuvn.webp)
...
## [Can I use matrices or multidimensional arrays in Pine Script?](../6. FAQ/faq_data-structures.md#can-i-use-matrices-or-multidimensional-arrays-in-pine-script)

Pine Script does not directly support multidimensional arrays; however, it provides [matrices](../3. Language/language_matrices.md) and [user-defined types](../3. Language/language_type-system.md#user-defined-types) (UDTs).
Programmers can use these data structures to create and manipulate complex datasets.

**Matrices**

Pine Script [matrices](../3. Language/language_matrices.md) are like two-dimensional arrays. They organize data in a rectangular grid, facilitating operations like transformations, linear algebra, and other complex calculations. They are particularly useful for quantitative modeling, such as portfolio optimization, correlation matrix analysis, and more.
Just as in [arrays](../3. Language/language_arrays.md), all elements in a matrix must be of the same [type](../3. Language/language_type-system.md#types), which can be a built-in or a [user-defined](../3. Language/language_type-system.md#user-defined-types) type. Pine Script provides a range of functions for [manipulating](../3. Language/language_matrices.md#manipulating-a-matrix) and performing [calculations](../3. Language/language_matrices.md#matrix-calculations) on matrices, including addition, subtraction, multiplication, and more.

**Using UDTs for multidimensional structures**

Programmers can achieve similar functionality to multidimensional arrays through defining [user-defined types](../3. Language/language_type-system.md#user-defined-types) (UDTs). For example, a script can define a UDT that includes an array as one of its fields. UDTs themselves can be contained in arrays. In this way, scripts can effectively have arrays of arrays.

For more information, see the sections on [Matrices](../3. Language/language_matrices.md), [Maps](../3. Language/language_maps.md), and [Objects](../3. Language/language_objects.md) in the User Manual.

## [How can I debug objects?](../6. FAQ/faq_data-structures.md#how-can-i-debug-objects)

To debug [objects](../3. Language/language_objects.md), create custom functions that break down an object into its constituent fields and convert these fields into strings.
See the [Debugging](../4. Writing_Scripts/writing_debugging.md) section of the User Manual for information about methods to display debug information.
In particular, [Pine Logs](../4. Writing_Scripts/writing_debugging.md#pine-logs) can display extensive and detailed debug information. See the FAQ section about debugging arrays [using Pine Logs](../6. FAQ/faq_data-structures.md#using-pine-logs) for an explanation of using logs for debugging.

In our example script, we create a [user-defined type](../3. Language/language_type-system.md#user-defined-types) (UDT) named `openLine`, which includes fields such as `price`, `openTime`, and a line object called `level`.
...
![image](../images/Data-structures-How-can-i-debug-objects-1.C0Wd6CjK_ggbeG.webp)
...
[Previous 
**Alerts**](../6. FAQ/faq_alerts.md) [Next 
**Functions**](../6. FAQ/faq_functions.md)