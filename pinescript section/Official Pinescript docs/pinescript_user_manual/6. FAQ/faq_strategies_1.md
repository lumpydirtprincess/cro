![](../6. FAQ/faq_strategies.md)

# [Strategies](../6. FAQ/faq_strategies.md#strategies)

Using Pine Script® strategy scripts, users can test _simulated_ trades on historical and realtime data, to backtest and forward test trading systems. Strategies are similar to indicators, but with added capabilities such as placing, modifying, and canceling simulated orders and analyzing their results. Scripts that use the [strategy()](../../reference manual/functions/strategy.md) function as their [declaration statement](../3. Language/language_script-structure.md#declaration-statement) gain access to the `strategy.*` namespace, which contains functions and variables for simulating orders and retrieving strategy information.

When a user applies a strategy that uses [order placement commands](../1. Concepts/concepts_strategies.md#order-placement-and-cancellation) to the chart, the strategy uses the [broker emulator](../1. Concepts/concepts_strategies.md#broker-emulator) to calculate simulated trades, and displays the results in the [Strategy Tester](../1. Concepts/concepts_strategies.md#strategy-tester) tab.

Strategies support [various types of orders](../1. Concepts/concepts_strategies.md#order-types) including market, limit, stop, and stop-limit orders, allowing programmers to simulate different trading scenarios. Strategy order commands can send alerts when [order fill events](../1. Concepts/concepts_alerts.md#order-fill-events) occur. An _order fill event_ is triggered by the broker emulator when it executes a simulated order in realtime.

For a thorough exploration of strategy features, capabilities, and usage, refer to the [Strategies](../1. Concepts/concepts_strategies.md) section in the User Manual.

## [Strategy basics](../6. FAQ/faq_strategies.md#strategy-basics)

### [How can I turn my indicator into a strategy?](../6. FAQ/faq_strategies.md#how-can-i-turn-my-indicator-into-a-strategy)

To convert an indicator to a strategy, begin by replacing the [indicator()](../../reference manual/functions/indicator.md) declaration with the [strategy()](../../reference manual/functions/strategy.md) declaration. This designates the script as a strategy.

Add [order placement commands](../1. Concepts/concepts_strategies.md#order-placement-and-cancellation) for simulating orders. Use logical conditions from the initial indicator to trigger the commands in the converted strategy.
...
![image](../images/Strategies-Strategy-basics-How-can-i-turn-my-indicator-into-a-strategy-1.gVd2FK1g_Z28vptr.webp)

The initial indicator colors the plot line and the bars on the chart in a lime color when the RSI is greater than 50 and fuchsia when less than 50. We use [plotshape()](../../reference manual/functions/plotshape.md) to plot triangles at the top and bottom of the oscillator on bars where the RSI crosses over or under the 50 level.
...
In the converted strategy version, we maintain the same RSI crossover and crossunder conditions used in the indicator script. These conditions, which previously only drew the [plotshape()](../../reference manual/functions/plotshape.md) triangles, now also trigger entry orders for long and short positions using the [strategy.entry()](../../reference manual/functions/strategy.entry.md) function. A long entry is called when the RSI crosses over 50, and a short entry is initiated when it crosses under 50. A long entry cancels a short trade, and vice-versa.
...
### [How do I set a basic stop-loss order?](../6. FAQ/faq_strategies.md#how-do-i-set-a-basic-stop-loss-order)

Stop losses are a risk management method that traders use to limit potential losses. The [strategy.exit()](../../reference manual/functions/strategy.exit.md) function sets an order to exit a trade once it hits a specified price, thus preventing the loss from exceeding a predetermined amount.

To implement a basic stop loss in Pine Script, use the [strategy.exit()](../../reference manual/functions/strategy.exit.md) function with either the `stop` or the `loss` parameter. The `stop` parameter specifies the _price_ for the stop loss order, while the `loss` parameter sets the stop loss a certain number of _ticks_ away from the entry order’s price. Similarly, to set a take-profit level, use either the `limit` parameter, specifying the exact price for taking profit, or the `profit` parameter, defining the profit
size in ticks from the entry price.

If a [strategy.exit()](../../reference manual/functions/strategy.exit.md) call includes both the `stop` _and_`loss` parameters, or both the `limit` _and_`profit` parameters, the function uses the price level that is expected to trigger an exit first.
...
![image](../images/Strategies-Strategy-basics-How-do-i-set-a-stop-loss-1.BKHhl7z6_1ISKGq.webp)
...
- In this example, we include `from_entry` arguments in the [strategy.exit()](../../reference manual/functions/strategy.exit.md) calls so that each exit order closes only open trades with the corresponding entry ID. Without this argument, the exit intended for long positions would apply to both long _and_ short positions, and the exit intended for short positions would likewise attempt to close any open position.

For more information, see the entry in the User Manual on [strategy.exit()](../1. Concepts/concepts_strategies.md#strategyexit).

### [How do I set an advanced stop-loss order?](../6. FAQ/faq_strategies.md#how-do-i-set-an-advanced-stop-loss-order)
...
A bracket order is a pair of orders that close the position if price moves far enough in either direction. Scripts can combine a stop-loss and take-profit order within a single [strategy.exit()](../../reference manual/functions/strategy.exit.md) function call. See the FAQ entry about [bracket orders](../6. FAQ/faq_strategies.md#how-can-i-configure-a-bracket-order-with-a-specific-risk-to-reward-rr-ratio) for more details.

**Trailing stop losses**

A trailing stop loss is a stop loss that moves with price, but in the profitable direction only. To create a trailing stop, either adjust the stop price with each new bar, or use the built-in trailing stop parameters in the [strategy.exit()](../../reference manual/functions/strategy.exit.md) function. Refer to the FAQ on implementing a
[trailing stop loss](../6. FAQ/faq_strategies.md#how-do-i-place-a-trailing-stop-loss) for information and examples.

**Scaled exits**

Scaled exits use multiple exit orders at varied price levels. When using tiered exit strategies, which progressively scale out of a position, ensure that the total quantity of all exit orders does not surpass the size of the initial entry position. Consult the FAQ on [multiple exits](../6. FAQ/faq_strategies.md#how-can-i-use-multiple-take-profit-levels-to-close-a-position) for more information.

**Moving a stop loss to breakeven**

Adjusting a stop loss to the breakeven point once a specific condition is met can help in risk management. Details can be found in the FAQ on [moving stop losses to breakeven](../6. FAQ/faq_strategies.md#how-do-i-move-my-stop-loss-order-to-breakeven).

**Adjusting position size based on stop loss**

Modify the position size relative to the stop loss to maintain a constant risk percentage of total equity. For more insights, see the FAQ on [position sizing](../6. FAQ/faq_strategies.md#how-can-i-risk-a-fixed-percentage-of-my-equity-per-trade).

### [How can I save the entry price in a strategy?](../6. FAQ/faq_strategies.md#how-can-i-save-the-entry-price-in-a-strategy)

Scripts can access the entry price for a _specific trade_, or the average entry price for a _position_.

**Average entry price**

The [strategy.position\_avg\_price](../../reference manual/variables/strategy.position_avg_price.md) variable automatically updates to the average entry price of the current position. If the position consist of only one trade, the average price of the position is equal to the entry price of that single trade.
If a strategy closes a market position that consists of multiple trades, trades are closed in the order they were opened, by default. Since the average price of the open position changes according to which positions are still open, be aware of the order in which trades are closed, and if necessary, configure it using the `close_entries_rule` parameter of the [strategy()](../../reference manual/functions/strategy.md) declaration function.

**Specific entry price**

The [strategy.opentrades.entry\_price()](../../reference manual/functions/strategy.opentrades.entry_price.md) function returns the entry price for a given trade ID. To find the entry price for the most recent open trade, and remembering that the trade indexes start at zero, use `float entryPrice = strategy.opentrades.entry_price(strategy.opentrades - 1)`.

### [How do I filter trades by a date or time range?](../6. FAQ/faq_strategies.md#how-do-i-filter-trades-by-a-date-or-time-range)
...
![image](../images/Strategies-Strategy-basics-How-do-i-implement-date-time-range-filtering-in-strategies-1.D00x6Oy9_ZgrI0R.webp)
...
- We use the [time()](../../reference manual/functions/time.md) function to calculate whether bars are outside the user-defined session times. For additional details on integrating session data in Pine Script, refer to the [Sessions](../1. Concepts/concepts_sessions.md) section in the User Manual.
...
## [Order execution and management](../6. FAQ/faq_strategies.md#order-execution-and-management)

### [Why are my orders executed on the bar following my triggers?](../6. FAQ/faq_strategies.md#why-are-my-orders-executed-on-the-bar-following-my-triggers)

Each historical bar in a chart is composed of a single set of [open](../../reference manual/variables/open.md), [high](../../reference manual/variables/high.md), [low](../../reference manual/variables/low.md) and [close](../../reference manual/variables/close.md) (OHLC) data. Pine scripts execute on this data once per historical bar, at the **close** of the bar.
...
Users can [alter a strategy’s calculation behavior](../1. Concepts/concepts_strategies.md#altering-calculation-behavior) by configuring strategies to process orders at the close of the signal bar instead, by selecting the “Fill orders/On bar close” setting in the “Settings/Properties” tab. Programmers can do the same by setting the [process\_orders\_on\_close](../1. Concepts/concepts_strategies.md#process_orders_on_close) parameter to `true` in the [strategy()](../../reference manual/functions/strategy.md) declaration statement:
...
An alternative method is to specify the `immediately` parameter as `true` in a [strategy.close()](../../reference manual/functions/strategy.close.md) or [strategy.close\_all](../../reference manual/functions/strategy.close_all.md) function call. This setting causes the broker emulator to close a position on the same tick that the strategy creates the close order — meaning, when bar closes instead of the beginning of the next one. The [process\_orders\_on\_close](../1. Concepts/concepts_strategies.md#process_orders_on_close) parameter affects all closing orders in the strategy, whereas the `immediately` parameter affects only the close order in which it is used.
...
### [How can I use multiple take-profit levels to close a position?](../6. FAQ/faq_strategies.md#how-can-i-use-multiple-take-profit-levels-to-close-a-position)
...
- Multiple [strategy.exit](../../reference manual/functions/strategy.exit.md) calls. This method is most suitable when each take-profit level has a corresponding stop loss.
- An [OCA reduce group](../1. Concepts/concepts_strategies.md#strategyocareduce). This method is ideal for a different number of take-profit levels and stop losses.

#### [Multiple ​`strategy.exit()`​ functions](../6. FAQ/faq_strategies.md#multiple-strategyexit-functions)

Each [strategy.exit()](../../reference manual/functions/strategy.exit.md) call can set a bracket order for a specific take-profit and stop-loss level.
However, if a strategy uses multiple [strategy.exit()](../../reference manual/functions/strategy.exit.md) functions with the **same** stop level, each function call triggers a _separate_ order (and therefore multiple order alerts). If order alerts are configured to trigger real trades, ensure that the trade system handles multiple alerts at the same stop level appropriately.

The following example script uses two separate [strategy.exit()](../../reference manual/functions/strategy.exit.md) functions, each with its own stop-loss and take-profit levels. The quantity for the first bracket order is set to 50% of the total position size. This combination of orders creates a scaled exit with distinct stop levels.

![image](../images/Strategies-Order-execution-and-management-How-can-i-set-up-multiple-take-profit-levels-to-gradually-close-out-a-position-1.B0Hj7Z0J_Z1CmABM.webp)
...
- We use persistent global variables for the take-profit and stop-loss levels so that we can plot them. Otherwise, declaring the variables in the first [if](../../reference manual/keywords/if.md) block would be simpler.

#### [Using ​`strategy.oca.reduce`​](../6. FAQ/faq_strategies.md#using-strategyocareduce)

Creating exit orders as a group, using the [strategy.oca.reduce](../1. Concepts/concepts_strategies.md#strategyocareduce) type, ensures that when one exit order from the group is filled, the quantity of the remaining orders is reduced accordingly. This method is ideal in scripts that have an unequal number of take-profit levels to stops.

When using a group of orders whose OCA type is [strategy.oca.reduce](../1. Concepts/concepts_strategies.md#strategyocareduce), we recommend ensuring that the total size of all exit orders, after any reductions, matches the size of the initial entry orders. Matching the sizes guarantees that the strategy closes the position entirely without leaving any part open or inadvertently opening a new position in the opposite direction.
...
![image](../images/Strategies-Order-execution-and-management-How-can-i-set-up-multiple-take-profit-levels-to-gradually-close-out-a-position-2.aS_yB9mZ_ZbBmYy.webp)
...
### [How can I execute a trade partway through a bar?](../6. FAQ/faq_strategies.md#how-can-i-execute-a-trade-partway-through-a-bar)

On [historical bars](../3. Language/language_execution-model.md#executions-on-historical-bars), Pine scripts can access only a single set of [open](../../reference manual/variables/open.md), [high](../../reference manual/variables/high.md), [low](../../reference manual/variables/low.md) and [close](../../reference manual/variables/close.md) (OHLC) data per bar.
...
#### [Using ​`calc_on_every_tick`​](../6. FAQ/faq_strategies.md#using-calc_on_every_tick)

Strategies running on [realtime bars](../3. Language/language_execution-model.md#executions-on-realtime-bars) can simulate orders partway through a bar by enabling the [calc\_on\_every\_tick](../1. Concepts/concepts_strategies.md#calc_on_every_tick) parameter. This setting allows the strategy to process each tick (incoming price update) and execute trades on the tick after a logical condition occurs.

#### [Using predefined prices](../6. FAQ/faq_strategies.md#using-predefined-prices)

Stop or limit orders at predefined prices _can_ execute orders partway through a bar, even when the strategy does not enable the `calc_on_every_tick` parameter. This method is effective on both realtime _and_ historical data. Even though orders are processed on the close of historical bars, the broker emulator simulates an order fill at the predefined price level, if the broker determines that price has hit that level during the bar.
For information about the assumptions that the broker emulator makes about price movements, see the [Broker emulator](../1. Concepts/concepts_strategies.md#broker-emulator) section of the User Manual.

The following example script uses stop and limit orders to exit a trade partway through a bar. The script calls the [strategy.exit()](../../reference manual/functions/strategy.exit.md) function with the `stop` and `limit` parameters, determining the specific price levels at which the trade will exit.

![image](../images/Strategies-Order-execution-and-management-How-can-i-execute-a-trade-midway-through-a-bar-before-it-fully-closes-1.D1Kusv4G_Z2vOQdd.webp)
...
### [How can I exit a trade in the same bar as it opens?](../6. FAQ/faq_strategies.md#how-can-i-exit-a-trade-in-the-same-bar-as-it-opens)

Sometimes, strategy testers want to be able to exit a trade in the same bar as the entry. By default, if an exit condition occurs during the same bar that a trade is opened, the broker emulator closes the trade at the open of the _next_ bar. To learn why this happens, refer to [this FAQ entry](../6. FAQ/faq_strategies.md#why-are-my-orders-executed-on-the-bar-following-my-triggers).

To override this default behavior, either specify exit prices, or exit with a market order at the bar close.

#### [Specifying exit prices](../6. FAQ/faq_strategies.md#specifying-exit-prices)
...
#### [Using a market order at bar close](../6. FAQ/faq_strategies.md#using-a-market-order-at-bar-close)

Another method to exit a trade in the same bar that it opens is to use a market order at the bar’s close, by setting the `immediately` argument to `true` in the [strategy.close()](../../reference manual/functions/strategy.close.md) function.

In the following example script, if the buy order is opened, the strategy closes the position at the end of the entry bar. Scripts can call the [strategy.close()](../../reference manual/functions/strategy.close.md) function conditionally within a local block if necessary. For simplicity, in this example we apply the command to every entry.
...
## [Advanced order types and conditions](../6. FAQ/faq_strategies.md#advanced-order-types-and-conditions)

### [How can I set stop-loss and take-profit levels as a percentage from my entry point?](../6. FAQ/faq_strategies.md#how-can-i-set-stop-loss-and-take-profit-levels-as-a-percentage-from-my-entry-point)

To set exit orders as a percentage from the entry price, the script needs the average entry price calculated by the broker emulator (which is affected by conditions including multiple entries and slippage). However, the built-in variable [strategy.position\_avg\_price](../../reference manual/variables/strategy.position_avg_price.md) returns [na](../../reference manual/variables/na.md) until the close of the entry bar. This means that take-profit and stop-loss orders based on the entry price can only be placed during the _next_ bar.
...
#### [Using ​`calc_on_order_fills`​](../6. FAQ/faq_strategies.md#using-calc_on_order_fills)

Setting the [calc\_on\_order\_fills](../1. Concepts/concepts_strategies.md#calc_on_order_fills) argument of the [strategy()](../../reference manual/functions/strategy.md) declaration function to `true` recalculates the strategy immediately after simulating an order fill. This setting provides access to data such as the current average price of a position on an unconfirmed bar.

The following example script sets take-profit and stop-loss orders during the entry bar, based on the entry price [strategy.position\_avg\_price](../../reference manual/variables/strategy.position_avg_price.md). The script uses the `calc_on_order_fills` setting to enable this behavior.
...
#### [Using predefined prices](../6. FAQ/faq_strategies.md#using-predefined-prices-1)
...
### [How do I move my stop-loss order to breakeven?](../6. FAQ/faq_strategies.md#how-do-i-move-my-stop-loss-order-to-breakeven)

Moving a stop-loss order to breakeven can be a useful technique to manage risk.

The following example script sets a persistent `stopLoss` variable when the strategy enters a position. The script then updates the stop price to the entry price when the market price gets halfway to the take-profit level. The script calls the [strategy.exit()](../../reference manual/functions/strategy.exit.md) function on every bar to ensure that the broker emulator receives any updates made to the `stopLoss` value. Lastly, it plots the average price according to the [strategy.position\_avg\_price](../../reference manual/variables/strategy.position_avg_price.md) variable for reference.

![image](../images/Strategies-Advanced-order-types-and-conditions-How-do-i-move-my-stop-loss-to-breakeven-1.IzyaMOFu_ZB2m4p.webp)
...
- This strategy uses [strategy.position\_avg\_price](../../reference manual/variables/strategy.position_avg_price.md) as the breakeven level. However, the real breakeven price of a trade is affected by slippage and commission.

### [How do I place a trailing stop loss?](../6. FAQ/faq_strategies.md#how-do-i-place-a-trailing-stop-loss)
...
Strategies can create trailing stops either by using the built-in functionality of the [strategy.exit()](../../reference manual/functions/strategy.exit.md) function or by creating custom trailing stop-loss logic.

Trailing stops set in the [strategy.exit()](../../reference manual/functions/strategy.exit.md) use live price updates in realtime but [assumed price movements](../1. Concepts/concepts_strategies.md#broker-emulator) for historical bars. These assumptions can cause [repainting](../1. Concepts/concepts_repainting.md). This type of trailing stop is therefore potentially more responsive but less accurate.
...
#### [Using built-in trailing stop functionality](../6. FAQ/faq_strategies.md#using-built-in-trailing-stop-functionality)

To set a trailing stop in the [strategy.exit()](../../reference manual/functions/strategy.exit.md) function, specify both _when_ the trail should activate and _how far_ behind price it should trail.
...
After it activates, the stop loss trails behind the bar’s [high](../../reference manual/variables/high.md) or [low](../../reference manual/variables/low.md) price by this distance, defined in ticks using the `trail_offset` parameter.

In the following long-only example script, the [strategy.exit()](../../reference manual/functions/strategy.exit.md) function uses the `trail_points` and `trail_offset` parameters to set a trailing stop. The
stop-loss trails the [high](../../reference manual/variables/high.md), minus the offset points, after it activates. The script creates and plots a separate `trailingStop` variable to visualize the trailing stop price that the function calculates internally, although this is not necessary for the trailing stop to function. We also set a separate stop-loss order to close trades that go too low before they trigger the trailing stop.

![image](../images/Strategies-Advanced-order-types-and-conditions-How-do-i-implement-a-trailing-stop-loss-1.CmsIKens_Zmjqb1.webp)
...
#### [Coding a custom trailing stop](../6. FAQ/faq_strategies.md#coding-a-custom-trailing-stop)

A custom trailing stop can use different activation conditions, and can trail in a different way, to the trailing stop built into the [strategy.exit()](../../reference manual/functions/strategy.exit.md) function. To work correctly, a custom trailing stop must calculate the stop price on each bar that the stop is active, and call the [strategy.exit()](../../reference manual/functions/strategy.exit.md) function on each bar to set the `stop` price.
...
- Because strategies run once per bar, the trailing stop price in this example script updates at the close of each bar. During realtime bars the _previous_ bar’s stop value is used. This approach, while slightly delayed compared to using the built-in trailing stop described in the FAQ entry about how to place a trailing stop loss [using built-in trailing stop functionality](../6. FAQ/faq_strategies.md#using-built-in-trailing-stop-functionality), ensures that the
trailing stop price is not subject to assumptions about intrabar price movements, and thus avoids repainting.

### [How can I set a time-based condition to close out a position?](../6. FAQ/faq_strategies.md#how-can-i-set-a-time-based-condition-to-close-out-a-position)

To close positions after a certain amount of time has passed, track the entry time for each trade and close the position using [strategy.close()](../../reference manual/functions/strategy.close.md) after the timeout.
...
In realtime, the same logic applies unless the strategy uses the [calc\_on\_every\_tick](../1. Concepts/concepts_strategies.md#calc_on_every_tick) parameter, in which case the trade closes as soon as the first tick exceeds the timeout value. Remember that [altering emulator behavior](../1. Concepts/concepts_strategies.md#altering-calculation-behavior) typically introduces [repainting](../1. Concepts/concepts_repainting.md).
...
![image](../images/Strategies-Advanced-order-types-and-conditions-How-can-i-set-a-time-based-condition-to-close-out-a-position-1.n6VCruKe_Z2ePmnM.webp)
...
- The script uses either the time of the bar’s close using the [time\_close](../../reference manual/variables/time_close.md) variable, or the current time from the [timenow](../../reference manual/variables/timenow.md) variable (if the strategy uses the [calc\_on\_every\_tick](../1. Concepts/concepts_strategies.md#calc_on_every_tick) parameter).
- The script uses the [built-in](../3. Language/language_built-ins.md) functions [strategy.opentrades.entry\_time()](../../reference manual/functions/strategy.opentrades.entry_time.md) and [strategy.opentrades.entry\_id()](../../reference manual/functions/strategy.opentrades.entry_id.md) to measure trade duration and identify individual trades.
- The [strategy.close()](../../reference manual/functions/strategy.close.md) function uses the `immediately` argument to simulate trades at the end of the bar that exceeds the timer, rather than waiting for the opening of the next bar. Consequently, when a 120-second timeout is applied and the script runs on a 1-minute chart, it gives the appearance that trades last exactly two bars.

### [How can I configure a bracket order with a specific risk-to-reward (R:R\ ) ratio?](../6. FAQ/faq_strategies.md#how-can-i-configure-a-bracket-order-with-a-specific-risk-to-reward-rr-ratio)

To create a bracket order, define a stop-loss and a take-profit order using a single [strategy.exit()](../../reference manual/functions/strategy.exit.md) call.
...
ratio. The `loss` parameter of the [strategy.exit()](../../reference manual/functions/strategy.exit.md) function is simply the stop distance. The `profit` parameter is the stop distance multiplied by the R:R
...
![image](../images/Strategies-Advanced-order-types-and-conditions-How-can-i-configure-a-bracket-order-with-a-specific-risk-to-reward-rr-ratio-1.Dy3U4-6N_Z1CXCdg.webp)
...
### [How can I risk a fixed percentage of my equity per trade?](../6. FAQ/faq_strategies.md#how-can-i-risk-a-fixed-percentage-of-my-equity-per-trade)
...
1. Calculate monetary _risk per contract_ by multiplying the stop distance in ticks by the monetary value of each tick ( [syminfo.mintick](../../reference manual/variables/syminfo.mintick.md)) and by the number of units each contract represents ( [syminfo.pointvalue](../../reference manual/variables/syminfo.pointvalue.md)).

2. Determine _risk amount_ by multiplying the current equity ( [strategy.equity](../../reference manual/variables/strategy.equity.md)) by the percentage of equity that you want to risk.
...
![image](../images/Strategies-Advanced-order-types-and-conditions-How-can-i-adjust-my-position-size-to-ensure-that-i-risk-a-fixed-percentage-of-my-equity-1.yi_EPCoj_Z21f7ro.webp)
...
## [Strategy optimization and testing](../6. FAQ/faq_strategies.md#strategy-optimization-and-testing)

### [Why did my trade results change dramatically overnight?](../6. FAQ/faq_strategies.md#why-did-my-trade-results-change-dramatically-overnight)

Strategy results can vary over time depending on where the historical data starts. The starting point of the data set aligns with the start of the nearest day, week, month or year, depending on the chart timeframe. Additionally, different TradingView plans provide access to varying amounts of historical bars. Refer to the User Manual entry on [starting points](../1. Concepts/concepts_repainting.md#starting-points) for a discussion of these factors.

For strategies, this means the historical results seen today might change as the dataset’s starting point moves. This can lead to a natural [repainting](../1. Concepts/concepts_repainting.md) of strategy results over time. To reduce the effect of these changes on backtesting, follow these tips:

**Export strategy results**

Regularly [exporting strategy results](https://www.tradingview.com/support/solutions/43000613680-how-can-i-export-strategy-data/) to file maintains a record of performance over time despite changes in historical data. Use the “Export Data” option in the top of the [Strategy Tester](../1. Concepts/concepts_strategies.md#strategy-tester) to export data.
...
### [Why is backtesting on Heikin Ashi and other non-standard charts not recommended?](../6. FAQ/faq_strategies.md#why-is-backtesting-on-heikin-ashi-and-other-non-standard-charts-not-recommended)

Non-standard charts like
[Heikin Ashi](https://www.tradingview.com/support/solutions/43000619436-heikin-ashi/),
[Renko](https://www.tradingview.com/support/solutions/43000502284-renko-charts/),
[Line Break](https://www.tradingview.com/support/solutions/43000502273-line-break-charts/),
[Kagi](https://www.tradingview.com/support/solutions/43000502272-kagi-charts/),
[Point & Figure](https://www.tradingview.com/support/solutions/43000502276-point-and-figure-pnf-charts/), and
[Range Charts](https://www.tradingview.com/support/solutions/43000474007-what-s-range-interval-and-how-do-i-select-it/)
offer unique perspectives on price action. However, these chart types are not suited for [strategy backtesting](../1. Concepts/concepts_strategies.md#notes-on-testing-strategies) or automated trading systems execution, because the prices and time intervals do not match market prices and times.
...
[Point & Figure](https://www.tradingview.com/support/solutions/43000502276-point-and-figure-pnf-charts/), and
[Range Charts](https://www.tradingview.com/support/solutions/43000474007-what-s-range-interval-and-how-do-i-select-it/)
simplify price action, losing some price detail. [Heikin Ashi](https://www.tradingview.com/support/solutions/43000619436-heikin-ashi/) charts calculate synthetic prices for each bar’s
[open](../../reference manual/variables/open.md),
[high](../../reference manual/variables/high.md),
[low](../../reference manual/variables/low.md),
and [close](../../reference manual/variables/close.md)
...
### [How can I backtest deeper into history?](../6. FAQ/faq_strategies.md#how-can-i-backtest-deeper-into-history)
...
For TradingView users with [Premium and higher plans](https://www.tradingview.com/pricing/), the [Deep Backtesting](https://www.tradingview.com/support/folders/43000584695/) feature calculates the strategy on _all_ historical data available for the selected symbol. The results are displayed in the Strategy Tester but are not visible on the chart. The results from Deep Backtesting might be different from results from the [Strategy Tester](../1. Concepts/concepts_strategies.md#strategy-tester) in regular mode, as explained in [this Help Center article](https://www.tradingview.com/support/solutions/43000666266/).

### [How can I backtest multiple symbols?](../6. FAQ/faq_strategies.md#how-can-i-backtest-multiple-symbols)
...
- Export the results from the [Strategy Tester](../1. Concepts/concepts_strategies.md#strategy-tester) and use external tools such as spreadsheet software to compare the performance of a strategy on different symbols.

### [What does Bar Magnifier do?](../6. FAQ/faq_strategies.md#what-does-bar-magnifier-do)

The [Bar Magnifier](../1. Concepts/concepts_strategies.md#bar-magnifier) feature, available for TradingView [Premium and Ultimate](https://www.tradingview.com/pricing/) account holders, significantly enhances the accuracy of order fills in strategy backtests. This tool uses data from lower timeframes to obtain more detailed price movement within a bar, which can result in more precise
order fills. When selected, Bar Magnifier mode replaces the assumptions that the [broker emulator](../1. Concepts/concepts_strategies.md#broker-emulator) must make about price movement using only a single set of OHLC values for each historical bar.
...
To fully appreciate the effectiveness of Bar Magnifier, refer to the script demonstrations in the section about [Bar Magnifier](../1. Concepts/concepts_strategies.md#bar-magnifier) in the User Manual.

## [Advanced features and integration](../6. FAQ/faq_strategies.md#advanced-features-and-integration)

### [Can my strategy script place orders with TradingView brokers?](../6. FAQ/faq_strategies.md#can-my-strategy-script-place-orders-with-tradingview-brokers)
...
### [How can I add a time delay between orders?](../6. FAQ/faq_strategies.md#how-can-i-add-a-time-delay-between-orders)
...
- Define the delay duration, whether in time units (minutes, hours, days) or a number of bars. For time-based delays, convert the chosen time unit into milliseconds, because Pine [time variables](../1. Concepts/concepts_time.md#time-variables) use milliseconds.
- Check the [time](../../reference manual/variables/time.md) or [bar\_index](../../reference manual/variables/bar_index.md) of the last trade using [strategy.closedtrades.exit\_time()](../../reference manual/functions/strategy.closedtrades.exit_time.md) or [strategy.closedtrades.exit\_bar\_index()](../../reference manual/functions/strategy.closedtrades.exit_bar_index.md).
...
To keep the chart clean, the script calls the [strategy.close()](../../reference manual/functions/strategy.close.md) function to close active trades after they have been open for 10 bars. The script uses background shading, labels and arrows to illustrate the trade entries and exits.

![image](../images/Strategies-Advanced-features-and-integration-How-can-i-implement-a-time-delay-between-orders-1.Ln04vrEQ_ZYWJ3j.webp)
...
### [How can I calculate custom statistics in a strategy?](../6. FAQ/faq_strategies.md#how-can-i-calculate-custom-statistics-in-a-strategy)

To track metrics other than the default metrics that the Strategy Tester tracks, strategies can calculate custom statistics. These calculations might need to detect order executions, track closed trades, monitor entries into trades, and assess whether a trade is active. Changes in [built-in](../3. Language/language_built-ins.md#built-in-variables) variables such as [strategy.opentrades](../../reference manual/variables/strategy.opentrades.md) and [strategy.closedtrades](../../reference manual/variables/strategy.closedtrades.md) can track the execution of orders.
...
![image](../images/Strategies-Advanced-features-and-integration-How-can-i-calculate-custom-statistics-in-a-strategy-1.BCecXRri_KBDrw.webp)
...
- The strategy incorporates [trading costs](../1. Concepts/concepts_strategies.md#simulating-trading-costs). Failing to account for these costs can lead to an unrealistic perception of strategy performance and diminish the credibility of test results.
- We round the
[open](../../reference manual/variables/open.md),
[high](../../reference manual/variables/high.md),
[low](../../reference manual/variables/low.md) and
[close](../../reference manual/variables/close.md)
(OHLC) built-in variables to the symbol’s precision. This rounding ensures that any statistics the script calculates align within the [Strategy Tester](../1. Concepts/concepts_strategies.md#strategy-tester) and with strategy order-related built-in variables.
- The script creates global variables for the changes in built-in variables for open and closed trades so that the [ta.change](../../reference manual/functions/ta.change.md) function is called on every bar for consistency.

### [How do I incorporate leverage into my strategy?](../6. FAQ/faq_strategies.md#how-do-i-incorporate-leverage-into-my-strategy)
...
For example, setting a 20% margin ratio means that the trader’s balance funds only 20% of the position’s value, allowing positions up to five times the account balance. A margin ratio of 20% is therefore the same as 5:1
leverage. With an available balance of $10,000 and a 20% margin setting, a strategy can open positions up to $50,000 in value.

Pine Script strategies can simulate trading with leverage by specifying margin requirements for long and short positions. Users can adjust the “Margin for long positions” and Margin for short positions” in the strategy’s “Properties” tab. Programmers can set the default margin in the script using the `margin_long` and `margin_short` parameters in the [strategy()](../../reference manual/functions/strategy.md) declaration function.
...
### [Can you hedge in a Pine Script strategy?](../6. FAQ/faq_strategies.md#can-you-hedge-in-a-pine-script-strategy)
...
### [Can I connect my strategies to my paper trading account?](../6. FAQ/faq_strategies.md#can-i-connect-my-strategies-to-my-paper-trading-account)
...
Strategies can customize [order fill alerts](../1. Concepts/concepts_strategies.md#strategy-alerts) to include detailed results and performance metrics in the alert strings, providing a record of the strategy’s theoretical fills and overall performance in realtime.

## [Troubleshooting and specific issues](../6. FAQ/faq_strategies.md#troubleshooting-and-specific-issues)

### [Why are no trades executed after I add the strategy to the chart?](../6. FAQ/faq_strategies.md#why-are-no-trades-executed-after-i-add-the-strategy-to-the-chart)
...
If a valid script that uses the [strategy()](../../reference manual/functions/strategy.md) declaration statement is running but is not placing any orders, consider the following potential problems and their solutions:

**Lack of order placement commands**

The strategy must use either the [strategy.order()](../../reference manual/functions/strategy.order.md) or [strategy.entry()](../../reference manual/functions/strategy.entry.md) order [placement commands](../1. Concepts/concepts_strategies.md#order-placement-and-cancellation) to place orders. Add [log.info](../../reference manual/functions/log.info.md) messages and review the Pine Logs to check whether the conditions to run the commands are met.

**Insufficient capital**

Verify that the strategy has enough initial capital to cover the position sizes it attempts to open. Remember, the cost of entering a futures contract position is the chart price multiplied by the [syminfo.pointvalue](../../reference manual/variables/syminfo.pointvalue.md), which can be significantly greater than the chart price. For a quick fix, increase the initial capital to a very high value in the _Properties_ tab.
...
### [Why does my strategy not place any orders on recent bars?](../6. FAQ/faq_strategies.md#why-does-my-strategy-not-place-any-orders-on-recent-bars)
...
Some programmers define entry conditions that rely on having no positions currently open. Make sure to explicitly close trades by specifying corresponding exit conditions for all trades. Without explicit instructions to close an open position using [strategy.close()](../../reference manual/functions/strategy.close.md) or [strategy.exit()](../../reference manual/functions/strategy.exit.md) commands, the strategy might display only a single entry order early in the chart’s history and in the _List of Trades_ tab. If trades are not closed, they do not generate results in the _Overview_.

### [Why is my strategy repainting?](../6. FAQ/faq_strategies.md#why-is-my-strategy-repainting)
...
- The [calc\_on\_every\_tick](../1. Concepts/concepts_strategies.md#calc_on_every_tick) setting causes a strategy to recalculate with every price update, which may cause orders and alerts to trigger during the formation of a bar in realtime. By contrast, on historical bars, calculations are performed at the close of the bar.
- The [calc\_on\_order\_fills](../1. Concepts/concepts_strategies.md#calc_on_order_fills) setting causes a strategy to recalculate immediately after simulating an order fill. For example, this feature is particularly useful for strategies that rely on entry prices to set exit prices on the entry bar, rather than waiting for the bar to close, such as the first example script in the FAQ entry [How can I set stop-loss and take-profit levels as a percentage from my entry point using `calc_on_order_fills`?](../6. FAQ/faq_strategies.md#using-calc_on_order_fills) However, using this setting can introduce _lookahead bias_ into the strategy, leading to potentially unrealistic outcomes. For instance, if a strategy’s entry conditions are met within a bar that also triggers an exit, the strategy would execute an entry order within the same bar on the next tick. On historical bars, such entries could occur at any of the bar’s [open](../../reference manual/variables/open.md), [high](../../reference manual/variables/high.md), [low](../../reference manual/variables/low.md), or [close](../../reference manual/variables/close.md) (OHLC) prices, resulting in entry prices that are unrealistically favorable.
- Since strategies and their alerts execute at the close of a historical bar, the next possible moment for an entry order to be filled is the beginning of the next bar. However, the [process\_orders\_on\_close](../1. Concepts/concepts_strategies.md#process_orders_on_close) setting causes the strategy to use the close price of the bar where the condition is met for its order prices instead. See the FAQ entry [Why are my orders executed on the bar following my triggers?](../6. FAQ/faq_strategies.md#why-are-my-orders-executed-on-the-bar-following-my-triggers) for more information.

To avoid repainting, set the [calc\_on\_every\_tick](../1. Concepts/concepts_strategies.md#calc_on_every_tick), [calc\_on\_order\_fills](../1. Concepts/concepts_strategies.md#calc_on_order_fills), and [process\_orders\_on\_close](../1. Concepts/concepts_strategies.md#process_orders_on_close) parameters to `false` in the [strategy()](../../reference manual/functions/strategy.md) declaration statement.
...
Ensure that strategies use only fixed values from a higher timeframe, as described in [Avoiding repainting](../1. Concepts/concepts_other-timeframes-and-data.md#avoiding-repainting).

Although these are the most common causes of repainting in strategies, they are not the only causes. For additional information, refer to the section on [repainting](../1. Concepts/concepts_repainting.md) in the User Manual.

### [How do I turn off alerts for stop loss and take profit orders?](../6. FAQ/faq_strategies.md#how-do-i-turn-off-alerts-for-stop-loss-and-take-profit-orders)
...
[Previous 
**Programming**](../6. FAQ/faq_programming.md) [Next 
**Strings and formatting**](../6. FAQ/faq_strings-and-formatting.md)