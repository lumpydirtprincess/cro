![](../6. FAQ/faq_strings-and-formatting.md)

# [Strings and formatting](../6. FAQ/faq_strings-and-formatting.md#strings-and-formatting)

## [How can I place text on the chart?](../6. FAQ/faq_strings-and-formatting.md#how-can-i-place-text-on-the-chart)

Scripts can display text using the following methods:

- The [plotchar()](../../reference manual/functions/plotchar.md) or [plotshape()](../../reference manual/functions/plotshape.md) functions for static text, which doesn’t change.
- [Labels](../2. Visuals/visuals_text-and-shapes.md#labels) and [boxes](../2. Visuals/visuals_lines-and-boxes.md#boxes) for dynamic text, which can vary bar to bar.
- [Tables](../2. Visuals/visuals_tables.md) for more complex text (static or dynamic) that stays in the same region of the chart.

### [Plotting text](../6. FAQ/faq_strings-and-formatting.md#plotting-text)

The [plotchar()](../../reference manual/functions/plotchar.md) and [plotshape()](../../reference manual/functions/plotshape.md) functions can display fixed text on bars:

- A single [plotchar()](../../reference manual/functions/plotchar.md) function call can print a string using the `text` parameter, but only one character using the `char` parameter. To plot only the text and not the character, set `char` to `""`.
- A [plotshape()](../../reference manual/functions/plotshape.md) function call can print a string using the `text` parameter. To plot only the text and not the shape, set the `color` (for the shape) to [na](../../reference manual/variables/na.md) and the `textcolor` to something visible.

Plots appears on the bar where the script calls the function, by default, but scripts can offset a plot by a dynamic number of bars to the left or right. On the Y axis, the plots appear above/below the bar, at the top/bottom of the chart, or at an arbitrary price level. Scripts can call a [plotchar()](../../reference manual/functions/plotchar.md) or [plotshape()](../../reference manual/functions/plotshape.md) function on any number of bars and it counts as a single plot towards the [plot limit](../4. Writing_Scripts/writing_limitations.md#plot-limits).
...
To print different text depending on a logical condition, use two function calls and control them using the `series` parameter. Note that even if the `series` for one or both of the function calls is never true during a script’s execution, and so no shape, character or text is ever plotted, _both_ functions still count towards the [plot limit](../4. Writing_Scripts/writing_limitations.md#plot-limits).

The following script corrects the earlier example, and shows the use of both [plotchar()](../../reference manual/functions/plotchar.md) and [plotshape()](../../reference manual/functions/plotshape.md) to display text:
...
### [Labels](../6. FAQ/faq_strings-and-formatting.md#labels)

[Labels](../2. Visuals/visuals_text-and-shapes.md#labels) are particularly useful for displaying text that can change from one bar to another. The `text` parameter of the [label.new()](../../reference manual/functions/label.new.md) function takes a “series string”, so it can change whenever necessary.

Labels do not count towards the [plot limit](../4. Writing_Scripts/writing_limitations.md#plot-limits), but there is a separate limit of how many labels can display on the chart. By default, up to approximately 50 of the _most recent_ labels appear on the chart. Programmers can adjust this limit up to 500 by setting the `max_labels_count` parameter in the [indicator()](../../reference manual/functions/indicator.md) or [strategy()](../../reference manual/functions/strategy.md) functions.

The parameters to the [label.new()](../../reference manual/functions/label.new.md) function for text, color, etc., take “series” arguments. This makes labels much more flexible than plots.

The following example script displays the same information as the previous script, but using labels. The background to the labels is transparent (set to [na](../../reference manual/variables/na.md)) in this example, to more closely match the style of the previous scripts.
...
As well as showing historical information, labels can also be used to show only the latest information on the current bar. The following example script displays the value of RSI in a different color depending on whether it is above or below 50, for the most recent bar only. This is not possible using [plotchar()](../../reference manual/functions/plotchar.md) or [plotshape()](../../reference manual/functions/plotshape.md), because the text is fixed, and too many plots would be required to plot every value separately.
...
### [Boxes](../6. FAQ/faq_strings-and-formatting.md#boxes)

[Boxes](../2. Visuals/visuals_lines-and-boxes.md#boxes) can also display text on the chart, by providing the text to the `text` parameter of the [box.new()](../../reference manual/functions/box.new.md) function. Boxes work with text in a similar way to labels, but with some additional features.
...
Text contained in the box can automatically wrap if it reaches the border of the box, if the `text_wrap` parameter is set to [text.wrap\_auto](../../reference manual/constants/text.wrap_auto.md). Additionally, scripts can align the text inside the box along the vertical and horizontal axes. Using the `text_halign` and `text_valign` parameters of [box.new()](../../reference manual/functions/box.new.md), text can display at one of the nine possible positions inside of the box.
...
![image](../images/Strings-and-formatting-Boxes.DGNthdsU_Z1uBDUD.webp)
...
### [Tables](../6. FAQ/faq_strings-and-formatting.md#tables)

[Tables](../2. Visuals/visuals_tables.md) are useful to display information in a fixed position on the chart. Whereas plots and labels can easily show historical information because they are, or can be, linked to specific bars, table contents do not change as users move the cursor over past chart bars. This makes tables best suited for showing _current_ information.
...
## [How can I position text on either side of a single bar?](../6. FAQ/faq_strings-and-formatting.md#how-can-i-position-text-on-either-side-of-a-single-bar)
...
The following example script draws three labels on the chart’s last bar, with different `style` and `textalign` values. User inputs control whether individual labels appear, and the central label is off by default for readability. If the input to hide the background is enabled, the color is set to [na](../../reference manual/variables/na.md) so that it does not appear. Note that the proper way to do this is to cast it to a color by using `color(na)`.

![image](../images/Strings-and-formatting-How-can-i-position-text-on-either-side-of-a-single-bar-1.DUFIx4_a_2eGvJY.webp)
...
## [How can I stack plotshape() text?](../6. FAQ/faq_strings-and-formatting.md#how-can-i-stack-plotshape-text)
...
![image](../images/Strings-and-formatting-How-can-i-lift-plotshape-text-up-1.BRcQQ6eU_Z2vQ3Az.webp)

## [How can I print a value at the top right of the chart?](../6. FAQ/faq_strings-and-formatting.md#how-can-i-print-a-value-at-the-top-right-of-the-chart)

Refer to the [Placing a single value in a fixed position](../2. Visuals/visuals_tables.md#placing-a-single-value-in-a-fixed-position) section of the [Tables](../2. Visuals/visuals_tables.md) page. The example in that section uses a single-cell table to display a string representation of a value in the top-right corner of the chart.

## [How can I split a string into characters?](../6. FAQ/faq_strings-and-formatting.md#how-can-i-split-a-string-into-characters)

The [str.split()](../../reference manual/functions/str.split.md) function splits a string into parts and stores the parts in an array.
...
[Previous 
**Strategies**](../6. FAQ/faq_strategies.md) [Next 
**Techniques**](../6. FAQ/faq_techniques.md)