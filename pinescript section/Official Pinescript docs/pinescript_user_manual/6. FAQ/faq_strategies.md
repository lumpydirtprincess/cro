...
# [Strategies](https://www.tradingview.com/pine-script-docs/faq/strategies/\#strategies)
...
Add [order placement commands](https://www.tradingview.com/pine-script-docs/concepts/strategies/#order-placement-and-cancellation) for simulating orders. Use logical conditions from the initial indicator to trigger the commands in the converted strategy.
...
When using a group of orders whose OCA type is [strategy.oca.reduce](https://www.tradingview.com/pine-script-docs/concepts/strategies/#strategyocareduce), we recommend ensuring that the total size of all exit orders, after any reductions, matches the size of the initial entry orders. Matching the sizes guarantees that the strategy closes the position entirely without leaving any part open or inadvertently opening a new position in the opposite direction.
...
#### [Using ​`calc_on_every_tick`​](https://www.tradingview.com/pine-script-docs/faq/strategies/\#using-calc_on_every_tick)
...
For information about the assumptions that the broker emulator makes about price movements, see the [Broker emulator](https://www.tradingview.com/pine-script-docs/concepts/strategies/#broker-emulator) section of the User Manual.
...
#### [Using a market order at bar close](https://www.tradingview.com/pine-script-docs/faq/strategies/\#using-a-market-order-at-bar-close)
...
#### [Using ​`calc_on_order_fills`​](https://www.tradingview.com/pine-script-docs/faq/strategies/\#using-calc_on_order_fills)
...
### [How can I configure a bracket order with a specific risk-to-reward (R:R\ ) ratio?](https://www.tradingview.com/pine-script-docs/faq/strategies/\#how-can-i-configure-a-bracket-order-with-a-specific-risk-to-reward-rr-ratio)
...
ratio. The `loss` parameter of the [strategy.exit()](https://www.tradingview.com/pine-script-reference/v6/#fun_strategy.exit) function is simply the stop distance. The `profit` parameter is the stop distance multiplied by the R:R
...
### [How can I backtest multiple symbols?](https://www.tradingview.com/pine-script-docs/faq/strategies/\#how-can-i-backtest-multiple-symbols)
...
order fills. When selected, Bar Magnifier mode replaces the assumptions that the [broker emulator](https://www.tradingview.com/pine-script-docs/concepts/strategies/#broker-emulator) must make about price movement using only a single set of OHLC values for each historical bar.
...
## [Advanced features and integration](https://www.tradingview.com/pine-script-docs/faq/strategies/\#advanced-features-and-integration)
...
(OHLC) built-in variables to the symbol’s precision. This rounding ensures that any statistics the script calculates align within the [Strategy Tester](https://www.tradingview.com/pine-script-docs/concepts/strategies/#strategy-tester) and with strategy order-related built-in variables.
...
### [Can I connect my strategies to my paper trading account?](https://www.tradingview.com/pine-script-docs/faq/strategies/\#can-i-connect-my-strategies-to-my-paper-trading-account)
...
Ensure that strategies use only fixed values from a higher timeframe, as described in [Avoiding repainting](https://www.tradingview.com/pine-script-docs/concepts/other-timeframes-and-data/#avoiding-repainting).
...
[Previous\\
**Programming**](https://www.tradingview.com/pine-script-docs/faq/programming) [Next\\
