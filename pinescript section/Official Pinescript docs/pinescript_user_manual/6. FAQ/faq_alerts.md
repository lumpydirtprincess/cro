...
# [Alerts](../6. FAQ/faq_alerts.md#alerts)

## [How do I make an alert available from my script?](../6. FAQ/faq_alerts.md#how-do-i-make-an-alert-available-from-my-script)

In indicator scripts, there are two ways to define triggers for [alerts](../1. Concepts/concepts_alerts.md):

- Using the [alertcondition()](../../reference manual/functions/alertcondition.md) function
- Using the [alert()](../../reference manual/functions/alert.md) function

In strategy scripts, there are also two ways to define alert triggers:

- Using the [alert()](../../reference manual/functions/alert.md) function
- Using [order fill events](../1. Concepts/concepts_alerts.md#order-fill-events)
...
## [How are the types of alerts different?](../6. FAQ/faq_alerts.md#how-are-the-types-of-alerts-different)

### [Usability](../6. FAQ/faq_alerts.md#usability)

Any script can include calls to the [alertcondition()](../../reference manual/functions/alertcondition.md) and [alert()](../../reference manual/functions/alert.md) functions within their code. However, [alertcondition()](../../reference manual/functions/alertcondition.md) calls have no effect unless the script is an _indicator_. [Libraries](../1. Concepts/concepts_libraries.md) can _export_ functions containing [alert()](../../reference manual/functions/alert.md) calls, but they cannot issue alert triggers directly.

[Order fill](../1. Concepts/concepts_alerts.md#order-fill-events) alert triggers are available only from [strategies](../1. Concepts/concepts_strategies.md).

### [Options for creating alerts](../6. FAQ/faq_alerts.md#options-for-creating-alerts)

Each [alertcondition()](../../reference manual/functions/alertcondition.md) call in an indicator script defines one distinct trigger and one corresponding option in the “Condition” dropdown menu of the “Create Alert” dialog box. If the user wants multiple alerts, they must create each one _separately_.

By contrast, if a script includes one or more [alert()](../../reference manual/functions/alert.md) function calls, only _one_ option appears in the “Condition” dropdown menu, titled “Any alert() function call”. Selecting this option creates a _single alert_ that activates based on the occurrences of _any_ executed [alert()](../../reference manual/functions/alert.md) call.

Similarly, for strategy scripts, the “Order fills and and alert() function calls” or “Order fills only” option in the “Condition” dropdown menu creates an alert that fires when _any_ [order fill event](../1. Concepts/concepts_alerts.md#order-fill-events) occurs.

### [How alerts activate](../6. FAQ/faq_alerts.md#how-alerts-activate)

The [alertcondition()](../../reference manual/functions/alertcondition.md) function operates exclusively in an indicator’s [global scope](../6. FAQ/faq_programming.md#what-does-scope-mean). Scripts cannot include calls to this function within any _local block_, such as the _indented_ code within an [if](../../reference manual/keywords/if.md) structure. The function triggers an alert when its specified `condition` is `true`. Users can set the allowed _frequency_ of the alert trigger using the “Frequency” field in the “Create Alert” dialog box.

The [alert()](../../reference manual/functions/alert.md) function has no `condition` parameter. Scripts trigger the alerts on any [alert()](../../reference manual/functions/alert.md) call based on each call’s `freq` argument. Therefore, programmers typically include such calls within the local scopes of [conditional structures](../3. Language/language_conditional-structures.md) to control when they execute.

Order fill alert triggers are available from [strategies](../1. Concepts/concepts_strategies.md) _automatically_ without requiring extra code. However, programmers can customize the default alert messages. These alerts fire on [order fill events](../1. Concepts/concepts_alerts.md#order-fill-events), which occur when the [broker emulator](../1. Concepts/concepts_strategies.md#broker-emulator) fills a strategy’s _orders_.

### [Messages](../6. FAQ/faq_alerts.md#messages)

The `message` parameter of the [alertcondition()](../../reference manual/functions/alertcondition.md) function populates the “Message” field of the “Create Alert” dialog box with a default message, which script users can customize to suit their alert needs. It accepts a “const string” argument, meaning its value cannot change after compilation. However, the argument can include [placeholders](https://www.tradingview.com/support/solutions/43000531021-how-to-use-a-variable-value-in-alert/) to make the message’s information _dynamic_.

The `message` parameter of the [alert()](../../reference manual/functions/alert.md) function accepts a “series string” argument, allowing programmers to create _dynamic messages_ that can include “string” representations of a script’s calculated values. Unlike [alertcondition()](../../reference manual/functions/alertcondition.md), this function does not populate a “Message” field in the “Create Alert” dialog box, and it does not process placeholders. Programmers can allow users to customize [alert()](../../reference manual/functions/alert.md) messages by creating [inputs](../1. Concepts/concepts_inputs.md) in the script’s settings.

Order fill alerts have a default message that describes a strategy’s order fill event. This default message contains strategy-specific [placeholders](https://www.tradingview.com/support/solutions/43000531021-how-to-use-a-variable-value-in-alert/), which the alert replaces with current strategy information each time it fires. Programmers can override the default message using the `//@strategy_alert_message` [compiler annotation](../3. Language/language_script-structure.md#compiler-annotations), which allows text and strategy placeholders, but _not_ script variables. Script users can edit the default message from the “Message” field in the “Create Alert” dialog.
...
### [Limitations](../6. FAQ/faq_alerts.md#limitations)

The [alertcondition()](../../reference manual/functions/alertcondition.md) function has some limitations:
...
- Every [alertcondition()](../../reference manual/functions/alertcondition.md) call contributes to the script’s [plot count](../4. Writing_Scripts/writing_limitations.md#plot-limits).
...
By contrast, all calls to the [alert()](../../reference manual/functions/alert.md) function count as **one** alert, regardless of the number of calls in the code. In addition, [alert()](../../reference manual/functions/alert.md) calls _do not_ contribute to a script’s plot count.

Similarly, if a user creates a strategy alert based on [order fill events](../1. Concepts/concepts_alerts.md#order-fill-events), it counts as **one** alert, even though it can fire multiple times with distinct messages from different order executions.

### [Example ​`alertcondition()`​ alert](../6. FAQ/faq_alerts.md#example-alertcondition-alert)

The script below demonstrates a simple [alertcondition()](../../reference manual/functions/alertcondition.md) call that triggers alerts when the current bar’s [close](../../reference manual/variables/close.md) is above the value from the previous chart bar. It also uses a [plotshape()](../../reference manual/functions/plotshape.md) call to indicate each bar where the `triggerCondition` occurred:
...
See [this section](../1. Concepts/concepts_alerts.md#alertcondition-events) of the [Alerts](../1. Concepts/concepts_alerts.md) page to learn more.

### [Example ​`alert()`​ alert](../6. FAQ/faq_alerts.md#example-alert-alert)

This example uses the `vstop()` function from our [ta](https://www.tradingview.com/script/BICzyhq0-ta/) library to calculate a volatility stop value and trend information based on the Average True Range ( [ATR](https://www.tradingview.com/support/solutions/43000501823-average-true-range-atr/)). The `stopValue` trails behind the chart’s [close](../../reference manual/variables/close.md) to form a trend-following system.

The script triggers an alert with an [alert()](../../reference manual/functions/alert.md) call each time the trend direction changes. The alert’s message is a “series string” that shows the trend’s new direction and the current stop value. An additional alert occurs whenever the `stopValue` moves in the current trend direction, with a message containing the updated value:
...
See [this section](../1. Concepts/concepts_alerts.md#script-alerts) of the [Alerts](../1. Concepts/concepts_alerts.md) page for more information.

### [Example strategy alert](../6. FAQ/faq_alerts.md#example-strategy-alert)

This example strategy places a [market order](../1. Concepts/concepts_strategies.md#market-orders) with [strategy.entry()](../../reference manual/functions/strategy.entry.md) and a stop-loss and take-profit (bracket) order with [strategy.exit()](../../reference manual/functions/strategy.exit.md) when a 5-bar moving average crosses over a 10-bar moving average. The stop-loss price is 1% below the current [close](../../reference manual/variables/close.md) and the take-profit price is 2% above the [close](../../reference manual/variables/close.md). Order fill alerts occur when the broker emulator fills an entry or exit order. Both [order placement commands](../1. Concepts/concepts_strategies.md#order-placement-and-cancellation) include unique `alert_message` arguments that combine placeholders and “string” representations of the `limit` and `stop` values to output details like the trade action, position size, chart symbol, and order prices:
...
For more information about order fill events, see [this section](../1. Concepts/concepts_alerts.md#order-fill-events) of the [Alerts](../1. Concepts/concepts_alerts.md) page. To learn more about how strategy scripts work, see the [Strategies](../1. Concepts/concepts_strategies.md) page.

## [If I change my script, does my alert change?](../6. FAQ/faq_alerts.md#if-i-change-my-script-does-my-alert-change)
...
## [Why aren’t my alerts working?](../6. FAQ/faq_alerts.md#why-arent-my-alerts-working)
...
If an alert fires at a different time than expected, _repainting_ might be the cause. Refer to the [Repainting](../1. Concepts/concepts_repainting.md) page for more information.
...
- Attempting to store more than 100,000 elements within a [collection](../3. Language/language_type-system.md#collections)
- Trying to access an item from a collection at an _out-of-bounds_ index
- Referencing historical values of a time series outside its allocated [memory buffer](https://www.tradingview.com/pine-script-docs/error-messages/#the-requested-historical-offset-x-is-beyond-the-historical-buffers-limit-y)
- Using [loops](../3. Language/language_loops.md) that take longer than 500 ms to complete their iterations

See [this page](https://www.tradingview.com/pine-script-docs/error-messages/) for additional details about common error messages and troubleshooting tips.

## [Why is my alert firing at the wrong time?](../6. FAQ/faq_alerts.md#why-is-my-alert-firing-at-the-wrong-time)

Sometimes, alerts may fire when users do not expect according to what their script displays on the chart. [Repainting](../1. Concepts/concepts_repainting.md) is the typical cause of such issues.

A chart’s realtime and historical bars often rely on _different_ [data feeds](../1. Concepts/concepts_other-timeframes-and-data.md#data-feeds). Data providers may retroactively adjust the reported values on realtime bars, which the displayed data reflects _after_ users refresh their charts or restart their scripts. Such adjustments can cause discrepancies where a triggered alert’s timing may not align with the script’s output after reloading it.
...
Most scripts have [fluid data values](../1. Concepts/concepts_repainting.md#fluid-data-values) that update after new ticks during an unconfirmed realtime bar and finalize after the bar closes. Consequently, an alert that fires on an open bar may not reflect the _final state_ of the condition after the bar’s confirmation. Set the alert’s frequency to “Once Per Bar Close” to avoid this issue.

**Using \`calc\_on\_every\_tick\` in strategies**

When a strategy script includes `calc_on_every_tick = true` in its declaration statement or the user selects the “On every tick” option in the “Recalculate” section of the strategy’s [properties](https://www.tradingview.com/support/solutions/43000628599-strategy-properties/), it recalculates on _every_ price update in the realtime data. This behavior can cause strategies to repaint because historical bars do not contain the same information as realtime bars. See [this section](../1. Concepts/concepts_strategies.md#altering-calculation-behavior) of the [Strategies](../1. Concepts/concepts_strategies.md) page to learn more.

**Incorrect usage of \`request.security()\` calls**

Using [request.security()](../../reference manual/functions/request.security.md) calls to fetch data from alternative timeframes can cause discrepancies on historical bars that scripts **cannot** reproduce on realtime bars. Ensure you follow the best practices for _non-repainting_ data requests to avoid such discrepancies, especially with higher-timeframe data. See the [Avoiding repainting](../1. Concepts/concepts_other-timeframes-and-data.md#avoiding-repainting) section of the [Other timeframes and data](../1. Concepts/concepts_other-timeframes-and-data.md) page and the [Higher-timeframe requests](https://www.tradingview.com/script/W1YpYcOI-Higher-timeframe-requests/) publication from PineCoders for more information.

## [Can I use variable messages with alertcondition()?](../6. FAQ/faq_alerts.md#can-i-use-variable-messages-with-alertcondition)

The `message` parameter of the [alertcondition()](../../reference manual/functions/alertcondition.md) function requires a “const string” argument, which **cannot change** after compilation. However, the “string” can include [placeholders](../1. Concepts/concepts_alerts.md#placeholders), which an alert substitutes with corresponding dynamic values from a script each time it fires.

The script below demonstrates two [alertcondition()](../../reference manual/functions/alertcondition.md) calls whose `message` arguments include placeholders for dynamic values. Each time alerts from these triggers occur, the message displays information about the current chart’s exchange, symbol, price, and volume:
...
## [How can I include values that change in my alerts?](../6. FAQ/faq_alerts.md#how-can-i-include-values-that-change-in-my-alerts)

The method for including dynamic values in alert messages varies with the type of alert trigger:

- The [alertcondition()](../../reference manual/functions/alertcondition.md) function accepts a “const string” `message` argument that can contain placeholders for dynamic values. See [Can I use variable messages with \`alertcondition()\`?](../6. FAQ/faq_alerts.md#can-i-use-variable-messages-with-alertcondition) for more information.
- The [alert()](../../reference manual/functions/alert.md) function accepts “series string” `message` arguments, which allows the convenient creation of dynamic messages that use a script’s calculated values. See [this section](../6. FAQ/faq_alerts.md#example-alert-alert) for an example.
- Order fill alerts can use “series string” values and placeholders. Refer to the example [here](../6. FAQ/faq_alerts.md#example-strategy-alert).

## [How can I get custom alerts on many symbols?](../6. FAQ/faq_alerts.md#how-can-i-get-custom-alerts-on-many-symbols)
...
Here is an example incorporating three symbols. The `checkForAlert()` function calls [request.security()](../../reference manual/functions/request.security.md) to fetch data from a specified context and evaluate the user-defined `checkForRsiConditions()` function using that data. Then, the function calls [alert()](../../reference manual/functions/alert.md) using the result to create an alert trigger. The script calls this function three times, creating distinct alert triggers for each specified symbol:
...
- This script uses the [alert()](../../reference manual/functions/alert.md) function because [alertcondition()](../../reference manual/functions/alertcondition.md) is not allowed within _local scopes_.

## [How can I trigger an alert for only the first instance of a condition?](../6. FAQ/faq_alerts.md#how-can-i-trigger-an-alert-for-only-the-first-instance-of-a-condition)
...
For alerts with [alertcondition()](../../reference manual/functions/alertcondition.md) triggers, setting them to fire once using the “Only Once” option in the “Create Alert” dialog box is not an optimal solution because it requires _manual_ reactivation each time an alert occurs. Alerts from the [alert()](../../reference manual/functions/alert.md) function do not have an “Only Once” frequency option. The programmer must use conditional logic to ensure the call executes at the appropriate time.
...
The example script below manages separate bullish and bearish states, and it colors the background to represent each state. When a bullish or bearish state first occurs, an [alert()](../../reference manual/functions/alert.md) call executes and the script plots a triangle on the chart. It also plots smaller triangles to show where other signals occur within a state, which do not trigger additional alerts:

// ![image](../images/Alerts-How-can-i-trigger-an-alert-only-once-when-the-condition-is-true-the-first-time-1.D3rDfnca_Z2rl2QW.webp)
...
## [How can I run my alert on a timer or delay?](../6. FAQ/faq_alerts.md#how-can-i-run-my-alert-on-a-timer-or-delay)
...
The Pine script below implements a _time-delayed_ alert, which is subject to the limitations above. When the current [close](../../reference manual/variables/close.md) is higher than a moving average, a delay counter starts. After the delay passes, the alert fires once, and another alert _cannot_ fire until the timer resets. Users can specify whether the timer resets on each bar using the script’s `resetInput`:

![image](../images/Alerts-How-can-i-run-my-alert-on-a-timer-or-delay-1.K7lVQTwy_w62nO.webp)
...
This script relies on variables declared with the [varip](../../reference manual/keywords/varip.md) keyword, which do not revert to their last committed states during realtime bar calculations. See [this section](../3. Language/language_variable-declarations.md#varip) of the User Manual to learn more about using this keyword. To learn about how _rollback_ works, see the [Execution model](../3. Language/language_execution-model.md) page.

## [How can I create JSON messages in my alerts?](../6. FAQ/faq_alerts.md#how-can-i-create-json-messages-in-my-alerts)
...
Use [placeholders](../1. Concepts/concepts_alerts.md#placeholders) in the alert message, such as `{{close}}` and `{{volume}}`, to add _dynamic_ values to the JSON. The alert instance replaces the placeholders with corresponding values when it fires. This method can create richer alerts, especially for [strategies](../1. Concepts/concepts_strategies.md), which have [extra placeholders](https://www.tradingview.com/support/solutions/43000481368-strategy-alerts/) for their calculated values. See [this section](../6. FAQ/faq_alerts.md#example-strategy-alert) above for an example.
...
## [How can I send alerts to Discord?](../6. FAQ/faq_alerts.md#how-can-i-send-alerts-to-discord)
...
The script example below uses [placeholders](../1. Concepts/concepts_alerts.md#placeholders) to dynamically populate alert messages with script values, including the new high or low price, and the chart’s symbol and timeframe:
...
For more information about dynamic values in alert messages, refer to [How can I include values that change in my alerts?](../6. FAQ/faq_alerts.md#how-can-i-include-values-that-change-in-my-alerts).

To learn about using JSON format in script alerts, see [How can I create JSON messages in my alerts?](../6. FAQ/faq_alerts.md#how-can-i-create-json-messages-in-my-alerts).

## [How can I send alerts to Telegram?](../6. FAQ/faq_alerts.md#how-can-i-send-alerts-to-telegram)
...
[Previous 
**General**](../6. FAQ/faq_general.md) [Next 
**Data structures**](../6. FAQ/faq_data-structures.md)