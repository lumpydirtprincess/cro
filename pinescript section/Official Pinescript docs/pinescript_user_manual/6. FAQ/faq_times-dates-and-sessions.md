![](../6. FAQ/faq_times-dates-and-sessions.md)

# [Times, dates, and sessions](../6. FAQ/faq_times-dates-and-sessions.md#times-dates-and-sessions)

## [How can I get the time of the first bar in the dataset?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-get-the-time-of-the-first-bar-in-the-dataset)

The following example script initializes a variable using the [var](../3. Language/language_variable-declarations.md#var) keyword on the first bar and then never updates it again.
The variable stores the value of the [time](../../reference manual/functions/time.md) built-in, which represents the time of the bar open in UNIX format (milliseconds since
00:00:00
...
## [How can I convert a time to a date-time string?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-convert-a-time-to-a-date-time-string)

The built-in function [str.format\_time()](../../reference manual/functions/str.format_time.md) translates a UNIX timestamp into a string representation of date and time.
...
To improve readability, we use a custom function, `timeToString()`, which calls [str.format\_time()](../../reference manual/functions/str.format_time.md) and applies a consistent format, instead of specifying the format for every function call.

The format `"YYYY.MM.dd @ HH:mm:ss"` is similar to the ISO 8601 date and time representation, but with periods to separate parts of the date, and an `@` symbol between the date and time. For more formatting customization options, consult the [str.format\_time()](../../reference manual/functions/str.format_time.md) documentation.

![image](../images/Times-dates-sessions-How-can-i-convert-a-time-to-a-date-time-string.D2uzseAs_ZyKMHS.webp)
...
## [How can I find out how many days are in the current month?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-find-out-how-many-days-are-in-the-current-month)
...
![image](../images/Times-dates-sessions-How-can-i-know-how-many-days-are-in-the-current-month-1.B9TubEEu_Z2wr448.webp)
...
## [How can I detect the chart’s last day?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-detect-the-charts-last-day)

Scripts can detect the last day on a chart in different ways. However, because the [last\_bar\_time](../../reference manual/variables/last_bar_time.md)
and [timenow](../../reference manual/variables/timenow.md) variables constantly update to mirror the time of the newest bar, each subsequent realtime bar is _also_ interpreted as if it is the chart’s last day.

### [Using ​`timeframe.change`​](../6. FAQ/faq_times-dates-and-sessions.md#using-timeframechange)

This example script checks for all of these conditions:

- The daily timeframe begins a new day.
- The difference between the time of the last chart bar (retrieved using [last\_bar\_time](../../reference manual/variables/last_bar_time.md))
and the current bar’s time (from [time](../../reference manual/variables/time.md)) is less than the time in one day.
...
### [Using ​`timenow`​](../6. FAQ/faq_times-dates-and-sessions.md#using-timenow)

This script uses the [timenow](../../reference manual/variables/timenow.md) variable to retrieve the current time — not the time of the bar on which the script is executing, but the time of the latest chart update. We pass this time as input to the built-in functions [year()](../../reference manual/functions/year.md), [month()](../../reference manual/functions/month.md), and [dayofmonth()](../../reference manual/functions/dayofmonth.md) to calculate the year, month, and day of the month for that time.
We compare these calculated values with the built-in variables [year](../../reference manual/variables/year.md), [month](../../reference manual/variables/month.md), and [dayofmonth](../../reference manual/variables/dayofmonth.md), which store the year, month, and day of the month of the current chart bar.
...
## [How can I get bar’s trading date on symbols with an overnight session?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-get-bars-trading-date-on-symbols-with-an-overnight-session)
...
In Pine Script, the opening time of a bar is represented by the variable [time](../../reference manual/variables/time.md), which also returns the date and time without adjustment. Variables that return date-related values, like [dayofmonth](../../reference manual/variables/dayofmonth.md), [weekofyear](../../reference manual/variables/weekofyear.md), etc., are calculated based on this opening time, which can lead to unexpected behavior on overnight symbols.

The example script below outputs the value of the built-in [dayofmonth](../../reference manual/variables/dayofmonth.md) variable onto a chart as a label. The following screenshot shows the output of this script on two symbols: one stock and one Forex. We added a vertical line on the chart using the drawing tools to highlight the bar that represents May 22nd.
...
” symbol, the label shows `21` on the bar marked May 22nd. This is because the trading for May 22 actually starts on May 21, at 17:00
in the exchange’s time zone, and the [dayofmonth](../../reference manual/variables/dayofmonth.md) variable uses this time to determine the day of the month for this bar.

![image](../images/Times-dates-sessions-How-can-i-get-bars-trading-date-on-overnight-symbols-1.BMFIXxaR_15rvo2.webp)
...
There is a way to avoid this issue. Each date-related variable like [dayofmonth](../../reference manual/variables/dayofmonth.md) has a corresponding function with the same name, e.g., [dayofmonth()](../../reference manual/functions/dayofmonth.md). The function takes two parameters, `time` and `timezone`, which together allow you to specify the exact timestamp to convert to a date.

In addition, the [time\_tradingday](../../reference manual/variables/time_tradingday.md) variable returns the timestamp of 00:00
UTC of the _trading day_ the bar belongs to, regardless of the bar’s actual opening time. You can pass this timestamp to the [dayofmonth()](../../reference manual/functions/dayofmonth.md) function along with the `"UTC"` time zone to extract the date from the trading day of the bar, instead of its opening time. Below, we update our example script to use this method:

![image](../images/Times-dates-sessions-How-can-i-get-bars-trading-date-on-overnight-symbols-2.CbFg8vvY_Z2v0A8i.webp)
...
- Here, we add `"UTC"` as the time zone for the [dayofmonth()](../../reference manual/functions/dayofmonth.md) calculation, because [time\_tradingday](../../reference manual/variables/time_tradingday.md) always returns the trading day as 00:00
...
## [How can I plot a value starting n months or years back?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-plot-a-value-starting-n-months-or-years-back)

The [timestamp()](../../reference manual/functions/timestamp.md) function can accept negative argument values and convert them to accurate dates.
For instance, a negative month value deducts the corresponding number of months from the outcome.

Our example script calculates the date three months prior to the current time, using the [timestamp()](../../reference manual/functions/timestamp.md) function to create a timestamp that is the configured amount of time prior to the time now, as measured using [timenow](../../reference manual/variables/timenow.md).
As the script executes on each bar, it compares the bar [time](../../reference manual/variables/time.md) with the previously calculated timestamp. When the time of a bar matches our specified target date, we assign the high of that bar as our value to plot and color the background red for demonstration.

![image](../images/Times-dates-sessions-How-can-i-plot-a-value-starting-n-months-years-back-1.DmQSNam2_Zx5DYI.webp)
...
## [How can I track highs and lows for a specific timeframe?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-track-highs-and-lows-for-a-specific-timeframe)

The following example script tracks the high and low values of a timeframe that the user selects. Our script avoids using [request.security()](../../reference manual/functions/request.security.md) calls, which are resource-intensive.

![image](../images/Times-dates-sessions-How-can-i-track-highs-lows-for-a-specific-timeframe-1.CzQ5vZps_Z2vFuTY.webp)
...
- We detect the beginning of a new timeframe by passing the period input to the [timeframe.change()](../../reference manual/functions/timeframe.change.md) function.
- The script resets the `hi` and `lo` variables at the beginning of a new timeframe and then trails `hi` up and `lo` down.
- We change the color of plots to [na](../../reference manual/variables/na.md) every time a new timeframe begins, to visually distinguish between periods.

## [How can I track the highs and lows within a specific session or time of day?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-track-the-highs-and-lows-within-a-specific-session-or-time-of-day)

To find if a time is within a specific session, pass a [time-based session string](../1. Concepts/concepts_sessions.md#time-based-sessions) to the [time()](../../reference manual/functions/time.md) function, which retrieves the UNIX time of the current bar based on a given timeframe and session. If the time point falls outside the session, the function returns [na](../../reference manual/variables/na.md).

In the following example script, on the first bar that the time is in session, we set the `hi` and `lo` variables to the source inputs provided by the user ( [high](../../reference manual/variables/high.md) and [low](../../reference manual/variables/low.md) by default). During the session we trail the `hi` value up and the `lo` value down. The script also highlights the background red when a bar’s time falls outside the session time.

![image](../images/Times-dates-sessions-How-can-i-track-the-highs-lows-within-a-specific-session-or-time-of-day-1.CKWpgvoE_Z2f1IWk.webp)
...
- We use the 3-parameter version of the [time()](../../reference manual/functions/time.md) function to ascertain whether we are within user-specified hours and time zone for tracking highs and lows.
- We pass a session input and an optional time zone to the [time()](../../reference manual/functions/time.md) function.

## [How can I make an entire custom session visible?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-make-an-entire-custom-session-visible)

Scripts can use custom time-based sessions by defining a session string, as explained in the [Time-based sessions](../1. Concepts/concepts_sessions.md#time-based-sessions) section of the [Sessions](../1. Concepts/concepts_sessions.md) page.
...
- The script uses [time()](../../reference manual/functions/time-1.md) and [time\_close()](../../reference manual/functions/time_close.md) functions in loops, with positive and negative values for the `bars_back` parameter to search forwards and backwards in time.

## [How can I track highs and lows for periods within a bar?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-track-highs-and-lows-for-periods-within-a-bar)
...
- Next, we modify the symbol’s ticker using [ticker.modify()](../../reference manual/functions/ticker.modify.md) to include extended session data, ensuring that the script identifies premarket highs and lows even when the chart uses regular trading hours.
- Finally, we pass the `hiLoBetweenTime()` function as the `expression` of the [request.security()](../../reference manual/functions/request.security.md) function, with the modified ticker as the `symbol`.
The [request.security()](../../reference manual/functions/request.security.md) function evaluates the `hiLoBetweenTime()` function _within the context of the lower timeframe_ to calculate the session’s highs and lows.

![image](../images/Times-dates-sessions-How-can-i-track-highs-lows-between-specific-intrabar-hours-1.BEXeNwQA_Z1eu0TE.webp)
...
- The script raises an error using [runtime.error()](../../reference manual/functions/runtime.error.md) if the chosen lower timeframe for intrabar inspection is not shorter than the main chart’s timeframe. Including error-checking such as this when working with timeframes makes the script more robust.

## [How can I detect a specific date and time?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-detect-a-specific-date-and-time)

The simplest way to detect a specific date and time is to check the [time](../../reference manual/variables/time.md) variable, which returns the timestamp of the current bar’s open. If this matches a certain timestamp, then the script has detected that specific date and time.
...
![image](../images/Times-dates-sessions-How-can-i-detect-a-specific-date-time-1.BRXAWfjw_Z1oPUTR.webp)
...
- We use the built-in variables [year](../../reference manual/variables/year.md), [month](../../reference manual/variables/month.md), [dayofmonth](../../reference manual/variables/dayofmonth.md), [hour](../../reference manual/variables/hour.md), and [minute](../../reference manual/variables/minute.md), which store the time of the current bar’s open.
- We use the built-in functions [year()](../../reference manual/functions/year.md), [month()](../../reference manual/functions/month.md), [dayofmonth()](../../reference manual/functions/dayofmonth.md), [hour()](../../reference manual/functions/hour.md), and [minute()](../../reference manual/functions/minute.md) to retrieve the time of the current bar’s close, and to evaluate the time that is input by the user.

## [How can I know the date when the highest value was found?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-know-the-date-when-the-highest-value-was-found)

To determine the date on which the highest value occurred, the following example script uses the [ta.highest()](../../reference manual/functions/ta.highest.md) function to find the highest value within a certain lookback period, and the [ta.highestbars()](../../reference manual/functions/ta.highestbars.md) function to find how many bars back this value occurred.
If the highest point occurs on the current bar, the number of bars back, or _offset_, is zero. In our example script, when this condition is met, we update labels showing the time at the respective highest or lowest price.

![image](../images/Times-dates-sessions-How-can-i-know-the-date-when-the-highest-value-was-found-1.ScXUxlvD_pzLOb.webp)
...
- We declare the high and low labels with [var](../3. Language/language_variable-declarations.md#var) but supply [na](../../reference manual/variables/na.md) values for the x and y coordinates, so that the labels do not appear until these parameters are defined.
...
## [How can I detect bars opening at a specific hour?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-detect-bars-opening-at-a-specific-hour)
...
We check whether the built-in [hour](../../reference manual/variables/hour.md) variable is equal to eighteen and the minute variable is equal to zero.

**Detect the start of the session**

Using the [time()](../../reference manual/functions/time.md) function with a [time-based session string](../1. Concepts/concepts_sessions.md#time-based-sessions) returns the time of the current bar if it is within the session time. If it’s outside the session time, the function returns [na](../../reference manual/variables/na.md).
We determine the first bar that falls within the session by verifying that the current returned time is not [na](../../reference manual/variables/na.md) but was [na](../../reference manual/variables/na.md) one bar earlier.

**Check current time against a timestamp**

We generate a timestamp using the [timestamp()](../../reference manual/functions/timestamp.md) function by specifying 18 hours and 0 minutes, as well as the current day, month, and year. We then verify whether the bar’s time matches this timestamp.
...
- To ensure that the numbers do not plot on top of one another, we use the technique described in the FAQ entry [How can I stack plotshape text?](../6. FAQ/faq_strings-and-formatting.md#how-can-i-stack-plotshape-text)

## [Can I time the duration of a condition?](../6. FAQ/faq_times-dates-and-sessions.md#can-i-time-the-duration-of-a-condition)

Normally, variables reset every time the script performs a new iteration. This process is called _rollback_ and is described in the [Execution model](../3. Language/language_execution-model.md#executions-on-realtime-bars) page of the User Manual.

By contrast, variables declared with [varip](../../reference manual/keywords/varip.md) _preserve_ their values between realtime updates. This behavior allows scripts to track conditions during bars.

The following example script uses the `secondsSince()` function from the PineCoders’ [time](https://www.tradingview.com/script/tyeeNU9I-Time/) library. This function uses variables declared with [varip](../../reference manual/keywords/varip.md) to track the number of seconds that elapse within a realtime bar while a condition remains true.
Our script waits for price to move a user-defined number of ticks from the [open](../../reference manual/variables/open.md), and then starts a timer. When the appropriate number of seconds elapses, it triggers an alert. A table displays these conditions in real time.

![image](../images/Times-dates-sessions-Can-i-time-the-duration-of-a-condition-1.BgWHsjTH_ZqXduQ.webp)
...
For more information on the `secondsSince()` function and the use of [varip](../../reference manual/keywords/varip.md) variables, consult the PineCoders’ [Using `varip` variables](https://www.tradingview.com/script/ppQxBISk-Using-varip-variables-PineCoders/) publication.

## [How can I identify the nth occurrence of a weekday in the month?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-identify-the-nth-occurrence-of-a-weekday-in-the-month)
...
![image](../images/Times-dates-sessions-How-can-i-identify-the-nth-occurance-of-a-weekday-in-the-month-1.-p0cMTpr_Z2lkrUM.webp)
...
- We use an [enum input](../1. Concepts/concepts_inputs.md#enum-input) to specify the target weekday, and convert it to an integer using a [switch structure](../3. Language/language_conditional-structures.md#switch-structure).
...
## [How can I count down the remaining time in a bar?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-count-down-the-remaining-time-in-a-bar)
...
We subtract [timenow](../../reference manual/variables/timenow.md) from [time\_close](../../reference manual/variables/time_close.md) to calculate the time remaining in the current bar, and then display the result in a table.
...
![image](../images/Times-dates-sessions-How-can-i-implement-a-countdown-timer-1.f7Zg0trE_Z10slpl.webp)
...
- We use the [str.format\_time()](../../reference manual/functions/str.format_time.md) function to present the timestamp in a format similar to the price scale countdown timer. The function uses a conditional format so that we display hours only if the time remaining is one hour or greater.
...
## [How can I get the week of the month?](../6. FAQ/faq_times-dates-and-sessions.md#how-can-i-get-the-week-of-the-month)

Counting the week of the month in which the current bar occurs is not always straightforward. Some symbols trade overnight, so the time of the bar open is technically the previous day to the bar close, and sometimes the previous week. Because [weekofyear](../../reference manual/variables/weekofyear.md), [month](../../reference manual/variables/month.md), and similar variables use [time](../../reference manual/variables/time.md) (the time of the bar open) in their calculations, they can give unexpected results.
...
The following amended script version uses the functions [weekofyear()](../../reference manual/functions/weekofyear.md) and [month()](../../reference manual/functions/month.md) instead of the equivalent variables, and evaluates the [time\_tradingday](../../reference manual/variables/time_tradingday.md) variable, which, unlike [time](../../reference manual/variables/time.md), returns the beginning time _of the trading day_ that the bar belongs to. Using this variable, and fixing the time zone to “UTC”, avoids problems with sessions that span days.
...
While the preceding examples are useful for understanding the difference between [time](../../reference manual/variables/time.md) and [time\_tradingday](../../reference manual/variables/time_tradingday.md), the simplest solution is to use the [timeframe.change()](../../reference manual/functions/timeframe.change.md) function, which detects changes in a given timeframe. We can rewrite the boolean conditions in our example script as follows:
...
[Previous 
**Techniques**](../6. FAQ/faq_techniques.md) [Next 
**Variables and operators**](../6. FAQ/faq_variables-and-operators.md)